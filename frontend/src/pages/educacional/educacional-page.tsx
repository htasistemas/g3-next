...
const formularioInicial: Formulario = {
  ...
  ano_letivo_id: "", serie_id: "", unidade_id: "", sala_id: "", turno: "INTEGRAL", capacidade_maxima: ""
  , disciplina_id: "", aulas_semanais: "", turma_id: "", professor_id: "", dia_semana: "1", hora_inicio: "", hora_fim: ""
};
...
export function EducacionalPage() {
  ...
  const [buscaAlunoMatricula, setBuscaAlunoMatricula] = useState("");
  const [alunosMatricula, setAlunosMatricula] = useState<EducacionalItem[]>([]);
  const [buscandoAlunoMatricula, setBuscandoAlunoMatricula] = useState(false);
  ...
  useEffect(() => {
    void educacionalService.listarUnidadesEnsino()
      .then(setUnidadesEnsino)
      .catch((erro) => setMensagem(mensagemErro(erro, "Não foi possível carregar as unidades de ensino.")));
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    if (aba !== "matriculas") return;
    const termo = buscaAlunoMatricula.trim();
    if (termo.length < 2) {
      setAlunosMatricula([]);
      return;
    }
    const temporizador = window.setTimeout(() => {
      setBuscandoAlunoMatricula(true);
      void educacionalService.buscarAlunos(termo)
        .then(setAlunosMatricula)
        .catch((erro) => setMensagem(mensagemErro(erro, "Não foi possível buscar os alunos.")))
        .finally(() => setBuscandoAlunoMatricula(false));
    }, 300);
    return () => window.clearTimeout(temporizador);
  }, [aba, buscaAlunoMatricula]);
  ...
  const atualizarMatricula = (campo: keyof MatriculaFormulario, valor: string) => setMatricula((anterior) => ({
    ...anterior,
    [campo]: valor,
    ...(["ano_letivo_id", "etapa_id", "serie_id"].includes(campo) ? { turma_id: "" } : {})
  }));
  const selecionarAlunoDaMatricula = (aluno: EducacionalItem) => {
    atualizarMatricula("aluno_id", String(aluno.id));
    setBuscaAlunoMatricula(texto(aluno, "nome_completo", "nome"));
    setListas((anterior) => ({ ...anterior, alunos: [aluno] }));
    setAlunosMatricula([]);
  };
  ...
  async function salvarEstrutura(event: FormEvent) {
    ...
    else if (recurso === "turmas") payload = { nome, ano_letivo_id: Number(formulario.ano_letivo_id), etapa_id: Number(formulario.etapa_id), serie_id: Number(formulario.serie_id), turno: formulario.turno, capacidade_maxima: Number(formulario.capacidade_maxima), unidade_id: formulario.unidade_id ? Number(formulario.unidade_id) : null, sala_id: formulario.sala_id ? Number(formulario.sala_id) : null, professor_responsavel_id: null, professor_responsavel_nome: null, status: "ATIVA" };
  ...
  async function criarMatricula(event: FormEvent) {
    ...
    try {
      ...
      const mensagem = mensagemErro(erro, "Não foi possível criar a matrícula.");
      if (mensagem.toLowerCase().includes("turma") && (mensagem.toLowerCase().includes("compatível") || mensagem.toLowerCase().includes("compativel"))) {
        setMatricula((anterior) => ({ ...anterior, turma_id: "" }));
        setMensagem("A turma selecionada pertence a outro ano letivo, etapa ou série. A turma foi limpa; selecione uma turma compatível com os dados da matrícula.");
      } else {
  ...
  return <AdminPageLayout sectionLabel="Educacional" pageTitle={aba === "parcerias" ? "Parcerias públicas e prestação de contas" : aba === "visao-geral" ? "Visão geral" : aba === "pendencias" ? "Pendências educacionais" : aba === "estrutura" || aba === "fluxo-academico" || aba === "unidades" ? "Estrutura acadêmica" : aba === "vida-academica" ? "Vida acadêmica 360º" : aba === "alunos" || aba === "matriculas" || aba === "responsaveis" ? "Alunos" : aba === "enturmacao" ? "Alunos por turma" : aba === "professores" ? "Professores e equipe pedagógica" : aba === "grade-curricular" ? "Grade curricular" : aba === "horarios" ? "Horários" : aba === "diarios" ? "Diário de classe" : aba === "frequencias" ? "Chamada e frequência" : aba === "planos-aula" ? "Plano de aula" : aba === "planejamentos" ? "Planejamento pedagógico" : aba === "avaliacoes" ? "Avaliações e notas" : aba === "notas" ? "Notas" : aba === "boletins" ? "Boletins" : aba === "historicos" ? "Histórico escolar" : aba === "ocorrencias" ? "Ocorrências" : aba === "agenda" ? "Agenda escolar" : aba === "documentos" ? "Documentos/Declarações" : aba === "relatorios" ? "Relatórios e indicadores" : aba === "transferencias" ? "Transferências" : aba === "autorizacoes" ? "Autorizações" : aba === "rotinas-infantis" ? "Rotina infantil" : "Desenvolvimento infantil"} actions={acoes}>
    {mensagem ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog" aria-modal="true" aria-labelledby="mensagem-educacional-titulo"><div className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-5 shadow-2xl"><div className="flex items-start gap-3"><div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-amber-100 text-amber-700">!</div><div><h2 id="mensagem-educacional-titulo" className="font-semibold text-slate-900">Atenção</h2><p className="mt-1 text-sm leading-6 text-slate-600">{mensagem}</p></div></div><div className="mt-4 flex justify-end"><Button type="button" onClick={() => setMensagem("")}>OK</Button></div></div></div> : null}
    ...
    {aba === "matriculas" ? <BuscaAlunoMatriculaPanel termo={buscaAlunoMatricula} resultados={alunosMatricula} alunoId={matricula.aluno_id} carregando={buscandoAlunoMatricula} onTermoChange={(valor) => { setBuscaAlunoMatricula(valor); if (!valor.trim()) atualizarMatricula("aluno_id", ""); }} onSelecionar={selecionarAlunoDaMatricula} /> : null}
...
function BuscaAlunoMatriculaPanel({
  termo,
  resultados,
  alunoId,
  carregando,
  ...
  termo: string;
  resultados: EducacionalItem[];
  alunoId: string;
  carregando: boolean;
  onTermoChange: (valor: string) => void;
  onSelecionar: (aluno: EducacionalItem) => void;
}) {
  return <Card className="mb-4 border-emerald-200 bg-emerald-50/40 shadow-sm"><CardHeader className="pb-2"><CardTitle className="text-base">Buscar aluno para matrícula</CardTitle></CardHeader><CardContent><div className="relative max-w-2xl"><div className="flex gap-2"><Input value={termo} onChange={(event) => onTermoChange(event.target.value)} placeholder="Digite o nome, código ou ID do aluno" autoComplete="off" aria-label="Buscar aluno para matrícula" /><span className="absolute right-3 top-2.5 text-[var(--g3-muted)]">{carregando ? "..." : <Search className="h-4 w-4" />}</span></div>{resultados.length ? <div className="absolute left-0 right-0 top-11 z-30 max-h-60 overflow-y-auto rounded-lg border border-[var(--g3-border)] bg-[var(--g3-card)] p-1 shadow-xl">{resultados.map((item) => <button type="button" key={item.id} className="flex w-full items-center justify-between gap-3 rounded-md px-3 py-2 text-left text-sm hover:bg-[var(--g3-primary-soft)]" onClick={() => onSelecionar(item)}><span className="min-w-0 truncate font-medium">{texto(item, "nome_completo", "nome")}</span><span className="shrink-0 text-xs text-[var(--g3-muted)]">#{item.id}{item.codigo_beneficiario ? ` · ${item.codigo_beneficiario}` : ""}</span></button>)}</div> : null}</div><p className="mt-2 text-xs text-[var(--g3-muted)]">A busca consulta somente alunos ativos e retorna até 50 resultados. {alunoId ? `Aluno selecionado: #${alunoId}.` : "Digite pelo menos 2 caracteres para começar."}</p></CardContent></Card>;
}
