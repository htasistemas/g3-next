...
type Aba = "visao-geral" | "estrutura" | "alunos" | "matriculas" | "responsaveis" | "unidades" | "enturmacao" | "professores" | "grade-curricular" | "horarios" | "diarios" | "frequencias" | "planos-aula" | "planejamentos" | "avaliacoes" | "notas" | "boletins" | "historicos" | "ocorrencias" | "agenda" | "documentos" | "relatorios" | "rotinas-infantis" | "desenvolvimentos-infantis" | "transferencias" | "autorizacoes" | "fluxo-academico";
...
export function EducacionalPage() {
  ...
  const [matricula, setMatricula] = useState({ aluno_id: "", ano_letivo_id: "", unidade_id: "", sala_id: "", etapa_id: "", serie_id: "", turma_id: "", numero_matricula: "" });
  ...
  useEffect(() => {
    ...
    void educacionalService.listarUnidadesEnsino().then(setUnidadesEnsino).catch((erro) => setMensagem(erro instanceof Error ? erro.message : "Não foi possível carregar as unidades de ensino."));
  }, [aba]);
  ...
  async function criarMatricula(event: FormEvent) {
    ...
    setCarregando(true);
    try {
      await educacionalService.salvar("matriculas", { aluno_id: Number(matricula.aluno_id), ano_letivo_id: Number(matricula.ano_letivo_id), unidade_id: Number(matricula.unidade_id), sala_id: Number(matricula.sala_id), etapa_id: Number(matricula.etapa_id), serie_id: Number(matricula.serie_id), turma_id: matricula.turma_id ? Number(matricula.turma_id) : null, numero_matricula: matricula.numero_matricula.trim(), data_matricula: new Date().toISOString().slice(0, 10), situacao: "ATIVA" });
      setMatricula({ aluno_id: "", ano_letivo_id: "", unidade_id: "", sala_id: "", etapa_id: "", serie_id: "", turma_id: "", numero_matricula: "" });
  ...
  const possuiFormulario = !["visao-geral", "professores", "responsaveis", "unidades"].includes(aba);
  ...
  const abasGrupo = grupo === "alunos"
    ? [{ label: "Alunos", aba: "alunos" }, { label: "Matrículas", aba: "matriculas" }, { label: "Responsáveis e famílias", aba: "responsaveis" }, { label: "Transferências", aba: "transferencias" }, { label: "Autorizações", aba: "autorizacoes" }, { label: "Documentos e declarações", aba: "documentos" }, { label: "Histórico escolar", aba: "historicos" }]
    : grupo === "estrutura"
      ? [{ label: "Estrutura acadêmica", aba: "estrutura" }, { label: "Gestão acadêmica", aba: "fluxo-academico" }, { label: "Grade curricular", aba: "grade-curricular" }, { label: "Horários", aba: "horarios" }, { label: "Unidades escolares", aba: "unidades" }]
      : grupo === "professores"
        ? [{ label: "Professores e equipe", aba: "professores" }, { label: "Planejamento pedagógico", aba: "planejamentos" }, { label: "Plano de aula", aba: "planos-aula" }, { label: "Diário de classe", aba: "diarios" }, { label: "Frequência e chamada", aba: "frequencias" }, { label: "Avaliações e notas", aba: "avaliacoes" }, { label: "Boletins e resultados", aba: "boletins" }]
        : grupo === "gestao"
          ? [{ label: "Ocorrências", aba: "ocorrencias" }, { label: "Agenda escolar", aba: "agenda" }, { label: "Rotina infantil", aba: "rotinas-infantis" }, { label: "Desenvolvimento infantil", aba: "desenvolvimentos-infantis" }]
          : [];

  return <AdminPageLayout sectionLabel="Educacional" pageTitle={aba === "visao-geral" ? "Visão geral" : aba === "estrutura" || aba === "fluxo-academico" || aba === "unidades" ? "Estrutura acadêmica" : aba === "alunos" || aba === "matriculas" || aba === "responsaveis" ? "Alunos" : aba === "enturmacao" ? "Alunos por turma" : aba === "professores" ? "Professores e equipe pedagógica" : aba === "grade-curricular" ? "Grade curricular" : aba === "horarios" ? "Horários" : aba === "diarios" ? "Diário de classe" : aba === "frequencias" ? "Chamada e frequência" : aba === "planos-aula" ? "Plano de aula" : aba === "planejamentos" ? "Planejamento pedagógico" : aba === "avaliacoes" ? "Avaliações e notas" : aba === "notas" ? "Notas" : aba === "boletins" ? "Boletins" : aba === "historicos" ? "Histórico escolar" : aba === "ocorrencias" ? "Ocorrências" : aba === "agenda" ? "Agenda escolar" : aba === "documentos" ? "Documentos/Declarações" : aba === "relatorios" ? "Relatórios e indicadores" : aba === "transferencias" ? "Transferências" : aba === "autorizacoes" ? "Autorizações" : aba === "rotinas-infantis" ? "Rotina infantil" : "Desenvolvimento infantil"} actions={acoes}>
    ...
    {abasGrupo.length ? <div className="float-left mb-4 mr-4 w-full max-w-[220px] rounded-lg border border-[var(--g3-border)] bg-[var(--g3-card)] p-2"><div className="space-y-1.5">{abasGrupo.map((item, index) => <button key={item.label} type="button" onClick={() => item.aba === "unidades" || item.aba === "responsaveis" ? navigate(item.aba === "unidades" ? "/cadastros/unidades-assistenciais?tipo_unidade=ENSINO" : "/cadastros/vinculo-familiar") : navigate(`/educacional?grupo=${grupo}&aba=${item.aba}`)} className={classeBotaoAbaLateral(aba === item.aba)}><span className={classeNumeroAbaLateral(aba === item.aba)}>{index + 1}</span><span className="min-w-0 break-words">{item.label}</span></button>)}</div></div> : null}
