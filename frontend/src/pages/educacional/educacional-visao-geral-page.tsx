...
import { AlertTriangle, ArrowDownRight, ArrowUpRight, BarChart3, CalendarCheck2, Check, ClipboardCheck, Eye, EyeOff, GraduationCap, Settings2, UsersRound } from "lucide-react";
...
export function EducacionalVisaoGeralPage({ resumo: inicial, carregando }: Props) {
  ...
  return <div className="space-y-4">
    <div className="flex items-end gap-3 rounded-lg border border-[var(--g3-border)] bg-[var(--g3-card-soft)] p-3 shadow-sm">
      ...
      <Button className="shrink-0" type="button" onClick={() => void atualizarIndicadores()} disabled={carregando}>Atualizar indicadores</Button>
      </div>
      <div className="relative"><Button type="button" variant="outline" onClick={() => setConfiguracaoAberta((atual) => !atual)}><Settings2 className="mr-2 h-4 w-4" />Configurar painel</Button>{configuracaoAberta ? <div className="absolute right-0 top-11 z-20 w-64 rounded-xl border border-slate-200 bg-white p-3 shadow-xl"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Indicadores visíveis</p>{WIDGETS.map((item) => <label key={item.id} className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-2 text-sm hover:bg-emerald-50"><input type="checkbox" checked={exibirWidget(item.id)} onChange={() => atualizarWidgets(item.id)} className="h-4 w-4 rounded border-slate-300 text-emerald-700" /><span className="flex-1">{item.label}</span>{exibirWidget(item.id) ? <Eye className="h-3.5 w-3.5 text-emerald-700" /> : <EyeOff className="h-3.5 w-3.5 text-slate-400" />}</label>)}<button type="button" className="mt-2 flex w-full items-center justify-center gap-1 rounded-md border border-emerald-200 px-2 py-1.5 text-xs font-medium text-emerald-800 hover:bg-emerald-50" onClick={() => setConfiguracaoAberta(false)}><Check className="h-3.5 w-3.5" />Concluir</button></div> : null}</div>
    </div>
