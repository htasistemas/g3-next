...
export function EducacionalVisaoGeralPage({ resumo: inicial, carregando }: Props) {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {card("Alunos ativos", String(numero(resumo.alunos_ativos)), "cadastros educacionais ativos", <UsersRound className="h-5 w-5" />, "?grupo=alunos&aba=alunos")}
      {card("Matrículas ativas", String(numero(resumo.matriculas_ativas)), `${numero(matriculas.pendente)} pendentes`, <ClipboardCheck className="h-5 w-5" />, "?grupo=alunos&aba=alunos")}
      ...
      {card("Alunos em risco", String(risco), `${criticos} críticos · ${Math.max(0, risco - criticos)} em atenção`, <AlertTriangle className="h-5 w-5" />, "?grupo=alunos&aba=alunos", true)}
      {card("Risco de evasão", String(numero(resumo.risco_evasao)), "cinco ou mais faltas registradas", <AlertTriangle className="h-5 w-5" />, "?grupo=alunos&aba=alunos", true)}
    ...
    <Card><CardHeader><CardTitle>Central de pendências</CardTitle></CardHeader><CardContent className="space-y-2">{pendencias.map((item) => <button type="button" key={item.chave} onClick={() => navigate(destinoPendencia(item.destino))} className="grid w-full grid-cols-[110px_1fr_60px] items-center gap-2 rounded-md border border-[var(--g3-border)] bg-[var(--g3-card-soft)] p-3 text-left text-sm hover:border-emerald-600"><span className={item.prioridade === "Crítica" ? "font-semibold text-red-700" : item.prioridade === "Atenção" ? "font-semibold text-amber-700" : "font-semibold text-emerald-700"}>{item.prioridade}</span><span>{item.descricao}</span><strong className="text-right">{item.total}</strong></button>)}{!pendencias.length ? <p className="text-sm text-[var(--g3-muted)]">Nenhuma pendência educacional calculada para os filtros atuais.</p> : null}</CardContent></Card>
