...
import { AlertTriangle, Book, BookOpen, BookPlus, List, Plus, Printer, Save, Search, Trash2, Undo2, X } from "lucide-react";
...
function filtrarLivrosPorTermo(livros: BibliotecaLivro[], valor: string) {
  ...
  return livros.filter((item) => item.status === "ATIVO").filter((item) => !termo || `${item.codigo} ${item.titulo} ${item.autor} ${item.isbn ?? ""}`.toLocaleLowerCase("pt-BR").includes(termo));
}
...
function localizarLivroPorDescricao(livros: BibliotecaLivro[], valor: string) {
  ...
  return exato ?? (candidatos.length === 1 ? candidatos[0] : null);
}
...
export function BibliotecaPage() {
  ...
  return (
    <>
      <AdminPageLayout tabs={abas} activeTab={abaAtiva} onChangeTab={(tabId) => setAbaAtiva(tabId as AbaId)} actions={acoes} activeTitle={abas.find((item) => item.id === abaAtiva)?.label} codeBadge={livroSelecionadoId ?? emprestimoSelecionadoId ? `Código: ${livroSelecionadoId ?? emprestimoSelecionadoId}` : "Novo"}>
        {abaAtiva === "visao" ? <section className="space-y-4"><Card className="overflow-hidden border-emerald-200/80 bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-500 text-white shadow-[0_28px_70px_-36px_rgba(6,182,212,0.9)]"><CardContent className="space-y-6 px-6 py-8 text-center"><div className="space-y-2"><p className="text-xs font-semibold uppercase tracking-[0.28em] text-white/80">Biblioteca</p><h2 className="text-2xl font-semibold sm:text-3xl">Painel moderno do acervo</h2><p className="mx-auto max-w-3xl text-sm leading-6 text-white/85">Acompanhe o desempenho da biblioteca, a disponibilidade do acervo e a movimentação de empréstimos em tempo real.</p></div><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl border border-white/20 bg-white/15 px-4 py-4 backdrop-blur"><p className="text-xs uppercase tracking-[0.2em] text-white/75">Títulos</p><p className="mt-2 text-3xl font-semibold">{formatarNumero(totalTitulos)}</p><p className="mt-1 text-xs text-white/80">{formatarNumero(totalExemplares)} exemplares no total</p></div><div className="rounded-2xl border border-white/20 bg-white/15 px-4 py-4 backdrop-blur"><p className="text-xs uppercase tracking-[0.2em] text-white/75">Disponíveis</p><p className="mt-2 text-3xl font-semibold">{formatarNumero(exemplaresDisponiveis)}</p><p className="mt-1 text-xs text-white/80">{taxaDisponibilidade}% do acervo pronto para empréstimo</p></div><div className="rounded-2xl border border-white/20 bg-white/15 px-4 py-4 backdrop-blur"><p className="text-xs uppercase tracking-[0.2em] text-white/75">Operação</p><p className="mt-2 text-3xl font-semibold">{formatarNumero(emprestimosPendentes.length)}</p><p className="mt-1 text-xs text-white/80">pendências monitoradas por {nomeResponsavelAtual || "usuário atual"}</p></div></div></CardContent></Card><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{cardsVisao.map((card) => { const Icone = card.icon; return <Card key={card.titulo} className={`overflow-hidden border-white/70 bg-gradient-to-br ${card.classe}`}><CardHeader className="items-center space-y-2 pb-2 text-center"><div className="rounded-full border border-white/70 bg-white/70 p-3 shadow-sm"><Icone className="h-6 w-6" /></div><CardTitle className="text-sm uppercase tracking-[0.22em] opacity-70">{card.titulo}</CardTitle></CardHeader><CardContent className="space-y-2 text-center"><p className="text-3xl font-semibold leading-none">{card.valor}</p><p className="text-sm font-medium">{card.detalhe1}</p><p className="text-xs opacity-70">{card.detalhe2}</p></CardContent></Card>; })}</div></section> : null}
        {abaAtiva === "livros" ? <section className="space-y-3"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><div className="space-y-1"><Label>Código *</Label><Input value={livroForm.codigo} onChange={(event) => setLivroForm((atual) => ({ ...atual, codigo: event.target.value }))} disabled={carregandoAcoes} /></div><div className="space-y-1 xl:col-span-2"><Label>ISBN</Label><div className="flex flex-col gap-2 sm:flex-row"><Input value={livroForm.isbn ?? ""} inputMode="numeric" placeholder="9788545702870" onChange={(event) => setLivroForm((atual) => ({ ...atual, isbn: normalizarIsbn(event.target.value) }))} disabled={carregandoAcoes} /><Button type="button" variant="default" onClick={() => void buscarLivroPorIsbn()} disabled={carregandoAcoes} className="sm:min-w-[150px] border border-emerald-300 bg-emerald-200 text-emerald-950 shadow-sm hover:bg-emerald-300 active:bg-emerald-500 active:text-white">{consultarIsbnMutation.isPending ? "Buscando ISBN..." : "Buscar ISBN"}</Button></div></div><div className="space-y-1"><Label>Status</Label><Select value={livroForm.status} onChange={(event) => setLivroForm((atual) => ({ ...atual, status: event.target.value as BibliotecaLivroCadastro["status"] }))} disabled={carregandoAcoes}><option value="ATIVO">Ativo</option><option value="INATIVO">Inativo</option></Select></div><div className="space-y-1 xl:col-span-2"><Label>Título *</Label><Input value={livroForm.titulo} onChange={(event) => setLivroForm((atual) => ({ ...atual, titulo: event.target.value }))} disabled={carregandoAcoes} /></div><div className="space-y-1 xl:col-span-2"><Label>Autor *</Label><Input value={livroForm.autor} onChange={(event) => setLivroForm((atual) => ({ ...atual, autor: event.target.value }))} disabled={carregandoAcoes} /></div><div className="space-y-1"><Label>Editora</Label><Input value={livroForm.editora ?? ""} onChange={(event) => setLivroForm((atual) => ({ ...atual, editora: event.target.value }))} disabled={carregandoAcoes} /></div><div className="space-y-1"><Label>Ano de publicação</Label><Input type="number" value={livroForm.anoPublicacao ?? ""} onChange={(event) => setLivroForm((atual) => ({ ...atual, anoPublicacao: event.target.value ? Number(event.target.value) : null }))} disabled={carregandoAcoes} /></div><div className="space-y-1"><Label>Categoria</Label><Input value={livroForm.categoria ?? ""} onChange={(event) => setLivroForm((atual) => ({ ...atual, categoria: event.target.value }))} disabled={carregandoAcoes} /></div><div className="space-y-1"><Label>Localização</Label><Input value={livroForm.localizacao ?? ""} onChange={(event) => setLivroForm((atual) => ({ ...atual, localizacao: event.target.value }))} disabled={carregandoAcoes} /></div><div className="space-y-1"><Label>Qtd. total</Label><Input type="number" value={livroForm.quantidadeTotal} onChange={(event) => setLivroForm((atual) => ({ ...atual, quantidadeTotal: Number(event.target.value) || 0 }))} disabled={carregandoAcoes} /></div><div className="space-y-1"><Label>Qtd. disponível</Label><Input type="number" value={livroForm.quantidadeDisponivel} onChange={(event) => setLivroForm((atual) => ({ ...atual, quantidadeDisponivel: Number(event.target.value) || 0 }))} disabled={carregandoAcoes} /></div><div className="space-y-1"><Label>Estado do livro</Label><Select value={livroForm.estadoLivro ?? ""} onChange={(event) => setLivroForm((atual) => ({ ...atual, estadoLivro: event.target.value }))} disabled={carregandoAcoes}><option value="">Selecione</option>{opcoesEstadoLivro.map((opcao) => <option key={opcao} value={opcao}>{opcao}</option>)}</Select></div><div className="space-y-1 md:col-span-2 xl:col-span-4"><Label>Observações</Label><Textarea rows={3} value={livroForm.observacoes ?? ""} onChange={(event) => setLivroForm((atual) => ({ ...atual, observacoes: event.target.value }))} disabled={carregandoAcoes} /></div></div><div className="space-y-1"><Label>Buscar livro</Label><Input placeholder="Título, autor, código ou ISBN" value={buscaLivro} onChange={(event) => setBuscaLivro(event.target.value)} disabled={carregandoAcoes} /></div><div className="overflow-x-auto rounded-lg border border-[var(--g3-border)]"><table className="min-w-full text-sm"><thead className="bg-[var(--g3-primary-soft)] text-[var(--g3-active)]"><tr><th className="px-3 py-2 text-left">Código</th><th className="px-3 py-2 text-left">Título</th><th className="px-3 py-2 text-left">Autor</th><th className="px-3 py-2 text-left">ISBN</th><th className="px-3 py-2 text-left">Disponíveis</th><th className="px-3 py-2 text-right">Ações</th></tr></thead><tbody>{livrosFiltrados.length ? livrosFiltrados.map((item, index) => <tr key={item.id} className={`border-t border-[var(--g3-border)] ${index % 2 === 0 ? "bg-[var(--g3-card)]" : "bg-[var(--g3-primary-soft)]/35"}`}><td className="px-3 py-2">{item.codigo}</td><td className="px-3 py-2">{item.titulo}</td><td className="px-3 py-2">{item.autor}</td><td className="px-3 py-2">{item.isbn ?? "---"}</td><td className="px-3 py-2">{item.quantidadeDisponivel}</td><td className="px-3 py-2 text-right"><Button size="sm" variant="outline" onClick={() => selecionarLivro(item.id)} disabled={carregandoAcoes}>Selecionar</Button></td></tr>) : <tr><td colSpan={6} className="px-3 py-4 text-center">{livrosQuery.isLoading ? "Carregando livros..." : "Nenhum livro encontrado."}</td></tr>}</tbody></table></div></section> : null}
