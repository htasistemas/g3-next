  ...
  async function fecharCompetenciaAtual() {
    try {
      ...
      setPopup({
        tipo: 'sucesso',
        titulo: 'Mês fechado',
        texto: `A competência ${formatarCompetencia(resposta.competencia)} foi fechada com sucesso. A abertura de ${formatarCompetencia(resposta.proximaCompetencia)} já foi preparada com os saldos fechados.`
      });
    } catch (error: any) {
      setPopup({
        tipo: 'erro',
        titulo: 'Erro no fechamento',
        texto: error?.response?.data?.message ?? 'Não foi possível fechar a competência informada.'
      });
    }
  }
