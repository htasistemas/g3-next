...
import {
  mapAuditoriaRowParaResponse,
  mapUsuarioRowParaResponse,
  type AuditoriaRow,
  type UsuarioRow
} from "../usuario.mapper.js";
import type {
...
export class UsuarioRepository {
  ...
  private async buscarAuditoriaPorUsuarioId(
    ...
    try {
      const rows = await prisma.$queryRaw<AuditoriaRow[]>(Prisma.sql`
        SELECT
          a.id::text,
          a.acao,
          a.usuario_id,
  ...
  private async atualizarCamposComplementaresTx(
    ...
    await tx.$executeRawUnsafe(
      ...
      normalizeDigits(input.telefone) ?? null,
      normalizeDigits(input.cpf) ?? null,
      trimOrUndefined(input.matricula) ?? null,
      trimOrUndefined(input.setor) ?? null,
      trimOrUndefined(input.unidade) ?? null,
  ...
  private async listarPermissoesUsuarioTx(
    tx: Prisma.TransactionClient,
    usuarioId: bigint
  ): Promise<string[]> {
  ...
  private async validarDuplicidades(input: {
    nome_usuario: string;
    email: string;
    cpf?: string;
    ignorar_id?: bigint;
    ...
    const loginRepetido = duplicidades.some(
      (item) => item.nome_usuario.toLowerCase() === input.nome_usuario.toLowerCase()
    );
    if (loginRepetido) {
      throw new AppError("Ja existe um usuario com este login.", 409);
    }

    const emailRepetido = duplicidades.some(
    ...
    if (input.cpf) {
      const cpfRepetido = duplicidades.some(
        (item) => normalizeDigits(item.cpf) === input.cpf
      );
      if (cpfRepetido) {
  ...
  private mapStatusPersistido(rawStatus?: string | null): UsuarioStatus {
