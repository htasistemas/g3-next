...
import {
  mapBancoEmpregosAvaliacao,
  mapBancoEmpregosCandidato,
  mapBancoEmpregosDocumento,
  mapBancoEmpregosHistorico,
  mapBancoEmpregosProcesso,
  mapBancoEmpregosVaga
} from "../banco-empregos.mapper.js";
import {
  bancoEmpregosAvaliacaoInputSchema,
  bancoEmpregosCandidatoFiltersSchema,
  bancoEmpregosCandidatoInputSchema,
  bancoEmpregosDashboardFiltersSchema,
  bancoEmpregosDocumentoUploadSchema,
  bancoEmpregosHistoricoFiltersSchema,
  bancoEmpregosProcessoFiltersSchema,
  bancoEmpregosProcessoInputSchema,
  bancoEmpregosVagaFiltersSchema,
  bancoEmpregosVagaInputSchema
...
type ExportacaoFormato = "csv" | "pdf";
type CartaTipo = "encaminhamento" | "recomendacao" | "comprovante" | "ficha";
...
export class BancoEmpregosService {
  ...
  private normalizarPayload(rawInput: unknown) {
    ...
    if (typeof resultado.email === "string") resultado.email = normalizarEmail(resultado.email);
    if (typeof resultado.telefone === "string") resultado.telefone = normalizarTelefone(resultado.telefone);
    if (typeof resultado.whatsapp === "string") resultado.whatsapp = normalizarTelefone(resultado.whatsapp);
    if (typeof resultado.cep === "string") resultado.cep = normalizarCep(resultado.cep);

    const normalizarColecao = (campo: string) => {
      if (!Array.isArray(resultado[campo])) return;
      resultado[campo] = (resultado[campo] as unknown[]).map((item) =>
        item && typeof item === "object"
          ? normalizarObjetoTexto(item as Record<string, unknown>, mapaCamposTextoBancoEmpregos)
          : item
      );
    };
  ...
  private extrairSugestoesCurriculo(textoExtraido?: string | null, nomeArquivo?: string | null) {
  ...
  private gerarCsvExportacao(filenameBase: string, cabecalho: string[], linhas: string[][]) {
  ...
  private async gerarPdfDocumentoInstitucional(input: {
    titulo: string;
    instituicao: Awaited<ReturnType<ReportsRepository["obterInstituicaoRelatorio"]>>;
    detalhes: string[];
    linhas: string[];
  }) {
