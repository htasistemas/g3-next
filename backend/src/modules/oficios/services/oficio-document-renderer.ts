...
export type OficioDocumentoInstituicao = {
  ...
  email?: string;
  logoUrl?: string;
  rodapePadrao?: OficioRodapePadrao;
};

export type OficioDocumentoLayout = {
  numeroOficio: string;
  cidadeUf?: string;
  dataExtenso: string;
  destinatarioInstituicao?: string;
  destinatarioTratamento?: string;
  destinatarioNome?: string;
  destinatarioCargo?: string;
  assunto?: string;
  corpoTexto?: string;
  informacoesComplementares?: OficioCampoComplementar[];
  fechamento?: string;
...
function fitTextToWidth(doc: PDFKit.PDFDocument, value: string, maxWidth: number) {
  ...
  async render(layout: OficioDocumentoLayout): Promise<Buffer> {
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      const garantirEspaco = (alturaNecessaria: number) => {
        if (doc.y + alturaNecessaria <= limiteInferior()) return;
        doc.addPage();
      };
      ...
      const escreverParagrafo = (texto: string, alinhamento: "left" | "justify" = "justify") => {
      ...
      const desenharComplementares = () => {
        const items = (layout.informacoesComplementares ?? []).filter(
          (item) => toSafeText(item.rotulo) && toSafeText(item.valor)
        );
        if (!items.length) return;

        doc.font(fonteCorpo).fontSize(11);
        const alturas = items.map((item) =>
          doc.heightOfString(`${toSafeText(item.rotulo)}: ${toSafeText(item.valor)}`, {
      ...
      for (let index = 0; index < pageRange.count; index += 1) {
        ...
        drawCenteredSingleLine(
          doc,
          fitTextToWidth(doc, truncateText(linha1, 260), larguraRodape),
          left(),
          larguraRodape,
          top
        );
        drawCenteredSingleLine(
          doc,
          fitTextToWidth(doc, truncateText(linha2, 320), larguraRodape),
          left(),
          larguraRodape,
          top + 10
        );
        drawCenteredSingleLine(
          doc,
          fitTextToWidth(doc, truncateText(linha3, 320), larguraRodape),
          left(),
          larguraRodape,
          top + 20
        );
