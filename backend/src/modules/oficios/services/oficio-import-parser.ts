...
const ROTULOS_COMPLEMENTARES: Array<{ aliases: string[]; rotulo: string }> = [
...
let ocrWorkerPromise: Promise<Awaited<ReturnType<typeof Tesseract.createWorker>>> | undefined;
let filaOcr: Promise<void> = Promise.resolve();
...
function normalizarTextoDocumento(valor?: string | null) {
  return (valor ?? "")
    .replace(/\u00a0/g, " ")
...
function limparMarcadoresPagina(valor?: string | null) {
...
function tamanhoTextoUtil(valor?: string | null) {
  return limparMarcadoresPagina(valor).replace(/\s+/g, "").length;
}

function quebrarLinhas(texto: string): LinhaDocumento[] {
...
function extrairExtensao(nomeArquivo: string) {
...
function extrairNumeroOficio(linhas: LinhaDocumento[]) {
  const linha = linhas.find((item) => item.normalized.includes("oficio"));
  if (!linha) return undefined;

  const match =
    linha.raw.match(/of[ií]cio\s*(?:n[ºo°.]?\s*)?(.+)/i) ??
    linha.raw.match(/n[ºo°.]?\s*(.+)/i);

  return match?.[1]?.trim() || undefined;
}

function extrairCidadeUfEData(linhas: LinhaDocumento[]) {
...
function extrairTratamentoENome(valor?: string) {
  const texto = valor?.trim();
  if (!texto) {
    return { saudacao: undefined, para: undefined };
  }

  const prefixo = PREFIXOS_TRATAMENTO.find((item) =>
    normalizarBusca(texto).startsWith(normalizarBusca(item))
  );
...
function extrairValorPorRotulo(linhas: LinhaDocumento[], rotulos: string[]) {
...
function ehLinhaRodape(valor?: string) {
...
function ehLinhaEnderecoRodape(valor?: string) {
  const texto = normalizarBusca(valor);
  if (!texto) return false;
...
function pareceLinhaInstitucionalRodape(valor?: string) {
  const texto = normalizarBusca(valor);
  if (!texto) return false;

  return (
    texto.startsWith("unidade ") ||
...
function ehSiglaInstitucionalCurta(valor?: string) {
  const texto = (valor ?? "").trim();
...
function ehLinhaAssinaturaDecorativa(valor?: string) {
  const texto = (valor ?? "").trim();
  return /^[_\-=\s]{3,}$/.test(texto);
}

function pareceNomePessoa(valor?: string) {
  const texto = (valor ?? "").trim();
  if (!texto || texto.length < 6 || texto.length > 80) return false;
  if (/[,:;]/.test(texto) || /\d/.test(texto)) return false;
  ...
  if (
    /(secretari|diretor|coordenador|assistente|supervisor|presidente|prefeito|gerente|procurador|assessor|tecnic|analista|professor|psicolog|enfermeir)/.test(
      textoNormalizado
    )
  ) {
    return false;
...
function pareceCargoAssinatura(valor?: string) {
  const texto = normalizarBusca(valor);
  if (!texto || texto.length < 6) return false;
  if (/\d{4,}/.test(texto)) return false;

  return /(secretari|diretor|coordenador|assistente|supervisor|presidente|prefeito|gerente|procurador|assessor|tecnic|analista|professor|psicolog|enfermeir|desenvolvimento social|cidadania)/.test(
    texto
  );
}

function encontrarInicioRodape(linhas: LinhaDocumento[]) {
  const inicioJanela = Math.max(linhas.length - 12, 0);
  const candidatos = linhas
    .map((item, index) => ({ item, index }))
    .filter(
      ({ item, index }) =>
        index >= inicioJanela && (ehLinhaRodape(item.raw) || ehLinhaEnderecoRodape(item.raw))
    )
    .map(({ index }) => index);
  ...
  while (indice > inicioJanela) {
    ...
    if (
      pareceLinhaInstitucionalRodape(anterior) ||
...
function extrairAssinatura(bloco: string[]) {
  const candidatos = bloco
    .map((raw, index) => ({ raw: raw.trim(), index }))
    .filter(({ raw }) => {
      if (!raw) return false;
      if (ehLinhaAssinaturaDecorativa(raw)) return false;
      if (ehLinhaRodape(raw) || ehLinhaEnderecoRodape(raw) || pareceLinhaInstitucionalRodape(raw)) return false;
      if (FECHAMENTOS_PADRAO.some((regex) => regex.test(raw))) return false;
      return true;
    });
...
function juntarLinhasComoParagrafos(linhas: string[]) {
...
function extrairComplementares(bloco: LinhaDocumento[]) {
  const campos: CampoComplementar[] = [];
...
async function executarOcrSerial<T>(tarefa: () => Promise<T>) {
  const execucao = filaOcr.then(tarefa, tarefa);
  filaOcr = execucao.then(
    () => undefined,
    () => undefined
  );
  return execucao;
...
async function silenciarAvisosPdf<T>(tarefa: () => Promise<T>) {
...
async function prepararImagemParaOcr(buffer: Buffer) {
  return sharp(buffer)
    .grayscale()
    .normalize()
    .sharpen()
...
async function extrairTextoPdfPorOcr(arquivo: ArquivoImportacao) {
  const avisos: string[] = [
  ...
  if (totalPaginas > OCR_MAX_PAGINAS) {
    avisos.push(
      `O OCR processou as ${OCR_MAX_PAGINAS} primeiras paginas de ${totalPaginas}. Revise se o oficio continua depois disso.`
    );
  }
  ...
  for (let pagina = 1; pagina <= paginasProcessadas; pagina += 1) {
    ...
    const resultado = await executarOcrSerial(() =>
      worker.recognize(imagemPreparada, { rotateAuto: true })
    );
...
async function extrairTextoArquivo(arquivo: ArquivoImportacao) {
  ...
  if (arquivo.tipoMime === "application/msword" || extensao === "doc") {
    throw new AppError(
      "Arquivo Word antigo (.doc) ainda nao suportado. Salve como .docx e envie novamente.",
      400
    );
  }
...
function extrairAssunto(linhas: LinhaDocumento[]) {
...
function inferirAssuntoPeloCorpo(linhasCorpo: string[]) {
  ...
  const indiceBase = Math.max(
    0,
    linhasFiltradas.findIndex((item) => /(venho|encaminh|solicit|inform|comunico|requer)/i.test(item))
  );
...
function paginaConcluiDocumento(textoPagina: string) {
  const linhas = quebrarLinhas(textoPagina);
  const indiceFechamento = linhas.findIndex((item) =>
    FECHAMENTOS_PADRAO.some((regex) => regex.test(item.raw))
  );
...
export class OficioImportParser {
  async importar(arquivo?: ArquivoImportacao): Promise<OficioImportacaoResultado> {
    ...
    if (!texto) {
      throw new AppError(
        "Nao foi possivel ler texto util do arquivo enviado. Verifique se o PDF possui texto selecionavel.",
        400
      );
    }

    if (
      (arquivo.tipoMime === "application/pdf" || extrairExtensao(arquivo.nomeArquivo) === "pdf") &&
    ...
    const destinatarioInstituicao =
      destinatarioInstituicaoBruto && destinatarioInstituicaoBruto.length >= 4
    ...
    const blocoAssinaturaBruto =
      indiceFechamento >= 0 ? linhas.slice(indiceFechamento + 1, indiceFimRodape).map((item) => item.raw) : [];
    ...
    const fechamentoLinhas = indiceFechamento >= 0
      ? [
          ...
          ...blocoAssinaturaBruto.filter(
            (item, index) =>
              !indicesAssinatura.has(index) &&
