...
import type {
  ...
  OficioImagemRow,
  OficioInput,
  OficioPdfAssinadoInput,
  OficioRow,
  OficioTramiteInput,
  OficioTramiteRow
} from "../oficios.types.js";

type TransactionClient = Prisma.TransactionClient;
type OficioSchemaInfo = {
  possuiDestinatarioResponsavel: boolean;
  possuiDestinatarioCargo: boolean;
};

function normalizarDestinatarios(input: OficioInput) {
  const destinatario =
    trimOrUndefined(input.identificacao.destinatario ?? undefined) ??
    trimOrUndefined(input.conteudo.para ?? undefined) ??
    "Nao informado";

  const destinatarioResponsavel =
    trimOrUndefined(input.identificacao.destinatarioResponsavel ?? undefined) ?? destinatario;

  const destinatarioCargo =
    trimOrUndefined(input.identificacao.destinatarioCargo ?? undefined) ??
...
export class OficiosRepository {
  ...
  async buscarPorId(id: bigint) {
    ...
    const tramites = await prisma.$queryRaw<OficioTramiteRow[]>(Prisma.sql`
      SELECT
        id,
        oficio_id,
        data,
  ...
  async adicionarImagem(oficioId: bigint, input: OficioImagemInput) {
    ...
    const rows = await prisma.$queryRaw<OficioImagemRow[]>(Prisma.sql`
      SELECT
        id,
        oficio_id,
        nome_arquivo,
