...
export function mapAutorizacaoCompraResumoToResponse(row: AutorizacaoCompraRow) {
  return {
    id: Number(row.id),
...
export function mapAutorizacaoCompraItemToResponse(row: AutorizacaoCompraItemRow) {
  return {
    id: Number(row.id),
...
export function mapAutorizacaoCompraCotacaoToResponse(
  row: AutorizacaoCompraCotacaoRow,
  extras?: {
    menorCotacaoId?: bigint | null;
    cotacaoVencedoraId?: bigint | null;
    indicadoresFornecedor?: Record<number, unknown>;
  }
) {
  return {
    id: Number(row.id),
    ...
    situacaoCadastral: row.situacao_cadastral ?? undefined,
    inicioAtividade: toIsoDate(row.inicio_atividade),
    enderecoCartaoCnpj: row.endereco_cartao_cnpj ?? undefined,
    valor: row.valor,
...
export function mapReservaBancariaToResponse(row: AutorizacaoCompraReservaRow) {
  return {
    id: Number(row.id),
    autorizacaoCompraId: Number(row.autorizacao_compra_id),
...
export function mapAutorizacaoCompraAprovacaoToResponse(
  row: AutorizacaoCompraAprovacaoRow,
  nivel?: AutorizacaoCompraNivelAprovacaoRow
) {
  return {
    id: Number(row.id),
    autorizacaoCompraId: Number(row.autorizacao_compra_id),
...
export function mapAutorizacaoCompraNivelToResponse(
  nivel: AutorizacaoCompraNivelAprovacaoRow,
  aprovacoes: AutorizacaoCompraAprovacaoRow[]
) {
...
export function mapAutorizacaoCompraHistoricoToResponse(row: AutorizacaoCompraHistoricoRow) {
  return {
    id: Number(row.id),
    autorizacaoCompraId: Number(row.autorizacao_compra_id),
    acao: row.acao,
...
export function mapAutorizacaoCompraIntegracaoToResponse(row: AutorizacaoCompraIntegracaoRow) {
  return {
    id: Number(row.id),
    autorizacaoCompraId: Number(row.autorizacao_compra_id),
    tipo: row.tipo,
...
export function mapArquivoResumoToResponse(row: ArquivoResumoRow) {
  return {
    id: Number(row.id),
...
export function mapAutorizacaoCompraDetalheToResponse(input: {
  compra: AutorizacaoCompraRow;
  itens: AutorizacaoCompraItemRow[];
  niveis: AutorizacaoCompraNivelAprovacaoRow[];
  aprovacoes: AutorizacaoCompraAprovacaoRow[];
  cotacoes: AutorizacaoCompraCotacaoRow[];
  reservas: AutorizacaoCompraReservaRow[];
  historico: AutorizacaoCompraHistoricoRow[];
  integracoes: AutorizacaoCompraIntegracaoRow[];
  anexos?: ArquivoResumoRow[];
  indicadoresFornecedor?: Record<number, unknown>;
}) {
  ...
  return {
    ...
    niveisAprovacao: input.niveis.map((nivel) =>
      mapAutorizacaoCompraNivelToResponse(nivel, input.aprovacoes)
    ),
    aprovacoes: input.aprovacoes.map((aprovacao) =>
      mapAutorizacaoCompraAprovacaoToResponse(
        aprovacao,
        input.niveis.find((nivel) => nivel.id === aprovacao.nivel_id)
      )
    ),
    cotacoes: input.cotacoes.map((cotacao) =>
      mapAutorizacaoCompraCotacaoToResponse(cotacao, {
...
export function mapFornecedorByCnpj(row?: { cnpj: string | null; razao_social: string | null }) {
  return {
    cnpj: row?.cnpj ?? undefined,
    razaoSocial: row?.razao_social ?? undefined,
