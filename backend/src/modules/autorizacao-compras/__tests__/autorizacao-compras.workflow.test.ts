...
import type { AutorizacaoCompraCotacaoRow, AutorizacaoCompraNivelAprovacaoRow } from "../autorizacao-compras.types.js";
...
test("determina aprovacoes cumulativas conforme a alcada do valor", () => {
  ...
  })) satisfies AutorizacaoCompraNivelAprovacaoRow[];

  const niveisObrigatorios = determinarNiveisObrigatorios(18000, niveis);

  assert.deepEqual(
    niveisObrigatorios.map((nivel) => nivel.codigo),
...
test("calcula sugestao de vencedor pelo menor preco", () => {
  ...
  ] satisfies AutorizacaoCompraCotacaoRow[];
