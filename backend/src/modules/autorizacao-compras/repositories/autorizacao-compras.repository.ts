...
export class AutorizacaoComprasRepository {
  ...
  async criar(input: AutorizacaoCompraInput) {
    ...
    `);
    const id = rows[0]?.id;
    if (!id) throw new AppError("Nao foi possivel criar autorizacao de compra.", 500);
    return this.buscarPorIdOuFalhar(id);
  }

  async atualizar(id: bigint, input: AutorizacaoCompraInput) {
  ...
  async listarCotacoes(autorizacaoId: bigint) {
    ...
    return prisma.$queryRaw<AutorizacaoCompraCotacaoRow[]>(Prisma.sql`
      SELECT
        id,
  ...
  async criarCotacao(autorizacaoId: bigint, input: AutorizacaoCompraCotacaoInput) {
    await this.buscarPorIdOuFalhar(autorizacaoId);
    const rows = await prisma.$queryRaw<Array<{ id: bigint }>>(Prisma.sql`
    ...
    `);
    const id = rows[0]?.id;
    if (!id) throw new AppError("Nao foi possivel salvar cotacao.", 500);
    const cotacoes = await this.listarCotacoes(autorizacaoId);
    const cotacao = cotacoes.find((item) => item.id === id);
    if (!cotacao) throw new AppError("Cotacao nao encontrada apos inclusao.", 500);
    return cotacao;
  }

  async removerCotacao(autorizacaoId: bigint, cotacaoId: bigint) {
    await this.buscarPorIdOuFalhar(autorizacaoId);
    await prisma.$executeRaw(Prisma.sql`
  ...
  async registrarReservaBancaria(autorizacaoId: bigint, input: ReservaBancariaInput) {
    await this.buscarPorIdOuFalhar(autorizacaoId);
    const rows = await prisma.$queryRaw<Array<{ id: bigint }>>(Prisma.sql`
    ...
    `);
    const id = rows[0]?.id;
    if (!id) throw new AppError("Nao foi possivel registrar reserva bancaria.", 500);
    const reservas = await this.listarReservas(autorizacaoId);
    const reserva = reservas.find((item) => item.id === id);
    if (!reserva) throw new AppError("Reserva bancaria nao encontrada apos inclusao.", 500);
    return reserva;
  }

  async removerReservaBancaria(autorizacaoId: bigint, contaId: bigint) {
    await this.buscarPorIdOuFalhar(autorizacaoId);
    await prisma.$executeRaw(Prisma.sql`
  ...
  async listarReservas(autorizacaoId: bigint) {
    return prisma.$queryRaw<AutorizacaoCompraReservaRow[]>(Prisma.sql`
      SELECT
        id,
        autorizacao_compra_id,
        conta_bancaria_id,
  ...
  async gerarAutorizacaoPagamento(autorizacaoId: bigint, input: AutorizacaoPagamentoInput) {
    const registro = await this.buscarPorIdOuFalhar(autorizacaoId);
    const numero =
      registro.autorizacao_pagamento_numero ?? `AP-${autorizacaoId.toString().padStart(6, "0")}`;
