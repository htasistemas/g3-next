...
import {
  ...
  mapAutorizacaoCompraToResponse,
  mapFornecedorByCnpj,
...
export class AutorizacaoComprasService {
  ...
  async criarCotacao(rawId: string, rawInput: unknown) {
    const id = this.parseId(rawId);
  ...
  async removerCotacao(rawId: string, rawCotacaoId: string) {
    const id = this.parseId(rawId);
  ...
  async registrarReservaBancaria(rawId: string, rawInput: unknown) {
    const id = this.parseId(rawId);
  ...
  async removerReservaBancaria(rawId: string, rawContaId: string) {
    const id = this.parseId(rawId);
    const contaId = this.parseId(rawContaId);
    await this.repository.removerReservaBancaria(id, contaId);
  }

  async gerarAutorizacaoPagamento(rawId: string, rawInput: unknown) {
    const id = this.parseId(rawId);
