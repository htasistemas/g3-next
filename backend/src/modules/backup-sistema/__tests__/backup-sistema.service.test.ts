...
function criarRunnerFalso(registros: Array<{ comando: string; args: string[] }>) {
  return async (comando: string, args: string[]) => {
...
test("gera e restaura backup do banco com metadados e comandos corretos", async () => {
  ...
  const comandos: Array<{ comando: string; args: string[] }> = [];
  let disconnects = 0;
  let connects = 0;
  ...
  const metadata = JSON.parse(await readFile(metadataPath, "utf-8")) as { restauradoEm?: string | null };
