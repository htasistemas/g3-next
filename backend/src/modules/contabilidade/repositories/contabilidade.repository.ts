...
export class ContabilidadeRepository {
  ...
  async buscarContaBancariaPorId(id: bigint) {
    const rows = await prisma.$queryRaw<ContaBancariaRow[]>(Prisma.sql`
      SELECT
        id,
        banco,
        agencia,
  ...
  async criarContaBancaria(input: ContaBancariaInput) {
    ...
    `);
    const id = rows[0]?.id;
    if (!id) throw new AppError("Nao foi possivel criar conta bancaria.", 500);
    return this.buscarContaBancariaPorIdOuFalhar(id);
  }

  async atualizarContaBancaria(id: bigint, input: ContaBancariaInput) {
  ...
  async removerContaBancaria(id: bigint) {
    await this.buscarContaBancariaPorIdOuFalhar(id);
    await prisma.$executeRaw(Prisma.sql`
  ...
  async buscarLancamentoPorId(id: bigint) {
    const rows = await prisma.$queryRaw<LancamentoFinanceiroRow[]>(Prisma.sql`
      SELECT
        id,
        tipo,
        descricao,
  ...
  async buscarLancamentoPorIdOuFalhar(id: bigint) {
    const lancamento = await this.buscarLancamentoPorId(id);
    if (!lancamento) throw new AppError("Lancamento financeiro nao encontrado.", 404);
    return lancamento;
  }

  async criarLancamento(input: LancamentoFinanceiroInput) {
    const rows = await prisma.$queryRaw<Array<{ id: bigint }>>(Prisma.sql`
    ...
    `);
    const id = rows[0]?.id;
    if (!id) throw new AppError("Nao foi possivel criar lancamento.", 500);
    return this.buscarLancamentoPorIdOuFalhar(id);
  }

  async atualizarLancamento(id: bigint, input: LancamentoFinanceiroInput) {
  ...
  async atualizarSituacaoLancamento(id: bigint, status: string) {
    await this.buscarLancamentoPorIdOuFalhar(id);
    await prisma.$executeRaw(Prisma.sql`
      UPDATE lancamento_financeiro
      SET
  ...
  async removerLancamento(id: bigint) {
    await this.buscarLancamentoPorIdOuFalhar(id);
    await prisma.$executeRaw(Prisma.sql`
  ...
  async pagarLancamento(id: bigint, dataPagamento?: string | null) {
  ...
  async buscarMovimentacaoPorId(id: bigint) {
    const rows = await prisma.$queryRaw<MovimentacaoFinanceiraRow[]>(Prisma.sql`
      SELECT
        m.id,
        m.tipo,
        m.descricao,
  ...
  async buscarMovimentacaoPorIdOuFalhar(id: bigint) {
    const movimentacao = await this.buscarMovimentacaoPorId(id);
    if (!movimentacao) throw new AppError("Movimentacao financeira nao encontrada.", 404);
    return movimentacao;
  }

  async criarMovimentacao(input: MovimentacaoFinanceiraInput) {
    ...
    `);
    const id = rows[0]?.id;
    if (!id) throw new AppError("Nao foi possivel criar movimentacao.", 500);
    return this.buscarMovimentacaoPorIdOuFalhar(id);
  }

  async atualizarMovimentacao(id: bigint, input: MovimentacaoFinanceiraInput) {
  ...
  async removerMovimentacao(id: bigint) {
    await this.buscarMovimentacaoPorIdOuFalhar(id);
    await prisma.$executeRaw(Prisma.sql`
  ...
  async buscarEmendaPorId(id: bigint) {
    const rows = await prisma.$queryRaw<EmendaImpositivaRow[]>(Prisma.sql`
      SELECT
        id,
        identificacao,
        referencia_legal,
  ...
  async buscarEmendaPorIdOuFalhar(id: bigint) {
    const emenda = await this.buscarEmendaPorId(id);
    if (!emenda) throw new AppError("Emenda impositiva nao encontrada.", 404);
    return emenda;
  }
  ...
  async criarEmenda(input: EmendaImpositivaInput) {
    ...
    `);
    const id = rows[0]?.id;
    if (!id) throw new AppError("Nao foi possivel criar emenda.", 500);
