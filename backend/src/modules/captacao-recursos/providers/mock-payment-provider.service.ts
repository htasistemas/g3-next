...
function gerarQrCodeSvg(seed: string) {
  ...
  for (let y = 0; y < grid; y += 1) {
    for (let x = 0; x < grid; x += 1) {
      ...
      if (fill) {
        rects.push(
          `<rect x="${(x + margin) * moduleSize}" y="${(y + margin) * moduleSize}" width="${moduleSize}" height="${moduleSize}" rx="1" ry="1" />`
        );
      }
    }
  }
...
export class MockPaymentProviderService implements PaymentProviderInterface {
  getProviderName() {
    return "mock-g3n";
  }
  ...
  async cancelCharge(_reference: string) {
    return { status: "cancelado" };
  }

  async refundCharge(_reference: string) {
    return { status: "estornado" };
  }

  async createRecurring(input: PaymentChargeInput) {
    return this.createCharge({ ...input, recurring: true });
  }

  async cancelRecurring(_reference: string) {
