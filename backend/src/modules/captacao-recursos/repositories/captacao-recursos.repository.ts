...
import { toOptionalDate, toStringId, trimOrUndefined } from "../../../utils/string-utils.js";
import { captacaoPermissions } from "../captacao-recursos.types.js";

type SqlRow = Record<string, unknown>;
type ListaResultado = { rows: SqlRow[]; total: number };
...
function toPgDate(value?: string | null) {
...
function tenantFilter(alias: string, tenantId?: string | null) {
...
export async function ensureCaptacaoRecursosEstrutura() {
  if (!ensurePromise) {
    ensurePromise = (async () => {
      ...
      `);
      await prisma.$executeRawUnsafe(
        "CREATE UNIQUE INDEX IF NOT EXISTS captacao_configuracoes_tenant_unique_idx ON captacao_configuracoes (tenant_id) WHERE tenant_id IS NOT NULL;"
      );
    })().catch((error) => {
      ensurePromise = null;
      throw error;
...
export class CaptacaoRecursosRepository {
  ...
  private async query<T extends SqlRow = SqlRow>(sql: string, params: unknown[] = []) {
  ...
  async listarDoadores(filters: Record<string, unknown>, tenantId?: string): Promise<ListaResultado> {
    ...
    if (trimOrUndefined(String(filters.termo ?? ""))) {
      ...
      parts.push(
        `(d.nome ILIKE ${param} OR COALESCE(d.nome_fantasia, '') ILIKE ${param} OR COALESCE(d.email_principal, '') ILIKE ${param} OR COALESCE(d.cpf_cnpj_norm, '') ILIKE ${param})`
      );
    }
    ...
    if (trimOrUndefined(String(filters.responsavel ?? ""))) {
      parts.push(
        `COALESCE(d.responsavel_relacionamento, '') ILIKE ${push(`%${String(filters.responsavel)}%`)}`
      );
    }
    ...
    const countRows = await this.query<{ total: bigint }>(
  ...
  async buscarDoadorPorIdOuFalhar(id: bigint, tenantId?: string) {
    const rows = await this.query(
      `
        SELECT d.*,
               COALESCE(stats.quantidade_doacoes, 0) AS quantidade_doacoes,
  ...
  async buscarDoadorDuplicado(
    ...
    const params: unknown[] = [];
    const parts: string[] = ["deleted_at IS NULL", tenantFilter("", tenantId)];
    const push = (value: unknown) => {
      params.push(value);
      return `$${params.length}`;
    };

    const comparadores: string[] = [];
    if (documentoNorm) comparadores.push(`cpf_cnpj_norm = ${push(documentoNorm)}`);
    if (emailNorm) comparadores.push(`email_principal_norm = ${push(emailNorm)}`);
    if (!comparadores.length) return undefined;
  ...
  async salvarDoador(id: bigint | null, uuid: string, input: Record<string, unknown>, userId?: bigint, tenantId?: string) {
    const payload = {
  ...
  async inativarDoador(id: bigint, userId?: bigint, tenantId?: string) {
    const row = (
      await this.query(
        `
          UPDATE captacao_doadores
  ...
  async listarCampanhas(filters: Record<string, unknown>, tenantId?: string): Promise<ListaResultado> {
    await this.ajustarCampanhasEncerradas();
    const params: unknown[] = [];
  ...
  async buscarCampanhaPorIdOuFalhar(id: bigint, tenantId?: string) {
    const rows = await this.query(
      `
  ...
  async salvarCampanha(id: bigint | null, uuid: string, input: Record<string, unknown>, userId?: bigint, tenantId?: string) {
    const params = [
      uuid,
      input.nome,
    ...
    if (!row) throw new AppError("Nao foi possivel salvar a campanha.", 500);
  ...
  async alterarStatusCampanha(id: bigint, status: string, userId?: bigint, tenantId?: string) {
    const row = (
      await this.query(
        `
          UPDATE captacao_campanhas
    ...
    if (!row) throw new AppError("Campanha nao encontrada.", 404);
    return row;
  }

  async recalcularMetricasCampanha(campanhaId: bigint, userId?: bigint, tenantId?: string) {
    await this.exec(
      `
  ...
  async listarDoacoes(filters: Record<string, unknown>, tenantId?: string): Promise<ListaResultado> {
    await this.ajustarCampanhasEncerradas();
    const params: unknown[] = [];
    const parts = ["d.deleted_at IS NULL", tenantFilter("d", tenantId)];
    const pagina = Math.max(1, Number(filters.pagina ?? 1) || 1);
    ...
    if (trimOrUndefined(String(filters.termo ?? ""))) {
      ...
      const param = push(termo);
      parts.push(
        `(d.numero_doacao ILIKE ${param} OR COALESCE(doadores.nome, '') ILIKE ${param} OR COALESCE(campanhas.nome, '') ILIKE ${param})`
      );
    }
    if (trimOrUndefined(String(filters.campanhaId ?? ""))) parts.push(`d.campanha_id = ${push(filters.campanhaId)}`);
    if (trimOrUndefined(String(filters.doadorId ?? ""))) parts.push(`d.doador_id = ${push(filters.doadorId)}`);
    if (trimOrUndefined(String(filters.formaPagamento ?? ""))) parts.push(`d.forma_pagamento = ${push(filters.formaPagamento)}`);
    if (trimOrUndefined(String(filters.situacao ?? ""))) parts.push(`d.situacao = ${push(filters.situacao)}`);
    if (trimOrUndefined(String(filters.origem ?? ""))) parts.push(`d.origem = ${push(filters.origem)}`);
    if (trimOrUndefined(String(filters.tipoDoacao ?? ""))) parts.push(`d.tipo_doacao = ${push(filters.tipoDoacao)}`);
    if (trimOrUndefined(String(filters.periodoInicio ?? ""))) parts.push(`d.data_hora >= ${push(String(filters.periodoInicio))}::date`);
    if (trimOrUndefined(String(filters.periodoFim ?? ""))) parts.push(`d.data_hora < (${push(String(filters.periodoFim))}::date + INTERVAL '1 day')`);
    const where = parts.join(" AND ");
  ...
  async buscarDoacaoPorIdOuFalhar(id: bigint, tenantId?: string) {
    const rows = await this.query(
      `
        SELECT
          d.*,
    ...
    if (!rows[0]) throw new AppError("Doacao nao encontrada.", 404);
    return rows[0];
  }
  ...
  async salvarRecorrencia(
    ...
    if (!rows[0]) throw new AppError("Nao foi possivel salvar a recorrencia.", 500);
    return rows[0];
  }

  async salvarDoacao(id: bigint | null, uuid: string, numeroDoacao: string, input: Record<string, unknown>, userId?: bigint, tenantId?: string) {
    const params = [
      uuid,
      numeroDoacao,
    ...
    if (!rows[0]) throw new AppError("Nao foi possivel salvar a doacao.", 500);
    return rows[0];
  }

  async alterarSituacaoDoacao(id: bigint, situacao: string, userId?: bigint, tenantId?: string, extras?: { txid?: string | null; linkPagamento?: string | null; comprovanteGerado?: boolean }) {
    const rows = await this.query(
      `
        UPDATE captacao_doacoes
           SET situacao = $2,
  ...
  async registrarEventoDoacao(doacaoId: bigint, tipoEvento: string, descricao: string, payloadJson?: Record<string, unknown> | null, userId?: bigint, tenantId?: string) {
    await this.exec(
      `
  ...
  async salvarTransacaoCartao(doacaoId: bigint, result: Record<string, unknown>, userId?: bigint, tenantId?: string) {
    await this.exec(
      `
  ...
  async salvarTransacaoBoleto(doacaoId: bigint, result: Record<string, unknown>, userId?: bigint, tenantId?: string) {
    await this.exec(
      `
  ...
  async salvarComprovante(
    doacaoId: bigint,
    data: { uuid: string; doadorId?: bigint | null; campanhaId?: bigint | null; numeroComprovante: string; codigoValidacao: string; arquivoCaminho?: string | null; mensagemAgradecimento?: string | null },
    userId?: bigint,
    tenantId?: string
  ) {
    const rows = await this.query(
  ...
  async marcarComprovanteEnviado(id: bigint, userId?: bigint, tenantId?: string) {
    await this.exec(
      `
        UPDATE captacao_comprovantes
  ...
  async buscarComprovantePorDoacao(doacaoId: bigint, tenantId?: string) {
    const rows = await this.query(
      `
  ...
  async listarComprovantes(filters: Record<string, unknown>, tenantId?: string): Promise<ListaResultado> {
    const params: unknown[] = [];
    ...
    if (trimOrUndefined(String(filters.campanhaId ?? ""))) parts.push(`c.campanha_id = ${push(filters.campanhaId)}`);
    if (trimOrUndefined(String(filters.doadorId ?? ""))) parts.push(`c.doador_id = ${push(filters.doadorId)}`);
    ...
    const countRows = await this.query<{ total: bigint }>(`SELECT COUNT(*)::BIGINT AS total ${baseFrom}`, params.slice(0, params.length - 2));
    return { rows, total: Number(countRows[0]?.total ?? 0) };
  }

  async obterConfiguracoes(tenantId?: string) {
    if (tenantId) {
      await this.exec(
        `
          INSERT INTO captacao_configuracoes (
  ...
  async salvarConfiguracoes(input: Record<string, unknown>, userId?: bigint, tenantId?: string) {
  ...
  async registrarLog(entidadeTipo: string, entidadeId: bigint | null, acao: string, descricao: string, detalhesJson?: Record<string, unknown> | null, userId?: bigint, tenantId?: string) {
    await this.exec(
      `
  ...
  async listarLogs(limit = 200, tenantId?: string) {
  ...
  async listarDashboardBase(filters: Record<string, unknown>, tenantId?: string) {
  ...
  async obterDoadorPortalPorCredenciais(emailNorm: string, documentoNorm?: string) {
  ...
  async criarAcessoPortal(doadorId: bigint, token: string, metadata: { email?: string; ip?: string; userAgent?: string }, userId?: bigint, tenantId?: string) {
  ...
  async obterAcessoPortalValido(token: string) {
    const rows = await this.query(
      `
  ...
  async listarRecorrenciasPorDoador(doadorId: bigint, tenantId?: string) {
    return this.query(
      `
  ...
  async listarComprovantesPorDoador(doadorId: bigint, tenantId?: string) {
    return this.query(
      `
  ...
  async atualizarDadosPortalDoador(doadorId: bigint, input: { email?: string; emailNorm?: string; telefone?: string; telefoneNorm?: string; whatsapp?: string; whatsappNorm?: string; cidade?: string; uf?: string }, tenantId?: string) {
    const rows = await this.query(
      `
        UPDATE captacao_doadores
    ...
    if (!rows[0]) throw new AppError("Doador nao encontrado.", 404);
    return rows[0];
  }

  async cancelarRecorrenciaPortal(recorrenciaId: bigint, doadorId: bigint) {
    const rows = await this.query(
      `
        UPDATE captacao_recorrencias
    ...
    if (!rows[0]) throw new AppError("Recorrencia nao encontrada.", 404);
    return rows[0];
  }
}
