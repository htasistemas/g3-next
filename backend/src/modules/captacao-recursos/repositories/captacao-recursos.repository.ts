...
export class CaptacaoRecursosRepository {
  ...
  async listarDoadores(filters: Record<string, unknown>): Promise<ListaResultado> {
  ...
  async buscarDoadorPorIdOuFalhar(id: bigint) {
    const rows = await this.query(
      `
        SELECT d.*,
               COALESCE(stats.quantidade_doacoes, 0) AS quantidade_doacoes,
  ...
  async salvarDoador(id: bigint | null, uuid: string, input: Record<string, unknown>, userId?: bigint) {
    const payload = {
  ...
  async inativarDoador(id: bigint, userId?: bigint) {
    const row = (
      await this.query(
        `
          UPDATE captacao_doadores
  ...
  async listarCampanhas(filters: Record<string, unknown>): Promise<ListaResultado> {
    await this.ajustarCampanhasEncerradas();
    const params: unknown[] = [];
  ...
  async buscarCampanhaPorIdOuFalhar(id: bigint) {
    const rows = await this.query(
      `
  ...
  async salvarCampanha(id: bigint | null, uuid: string, input: Record<string, unknown>, userId?: bigint) {
    const params = [
      uuid,
      input.nome,
  ...
  async alterarStatusCampanha(id: bigint, status: string, userId?: bigint) {
    const row = (
      await this.query(
        `
          UPDATE captacao_campanhas
  ...
  async recalcularMetricasCampanha(campanhaId: bigint, userId?: bigint) {
    await this.exec(
      `
  ...
  async listarDoacoes(filters: Record<string, unknown>): Promise<ListaResultado> {
    await this.ajustarCampanhasEncerradas();
    const params: unknown[] = [];
    const parts = ["d.deleted_at IS NULL"];
    const pagina = Math.max(1, Number(filters.pagina ?? 1) || 1);
  ...
  async buscarDoacaoPorIdOuFalhar(id: bigint) {
    const rows = await this.query(
      `
        SELECT
          d.*,
  ...
  async salvarDoacao(id: bigint | null, uuid: string, numeroDoacao: string, input: Record<string, unknown>, userId?: bigint) {
    const params = [
      uuid,
      numeroDoacao,
  ...
  async alterarSituacaoDoacao(id: bigint, situacao: string, userId?: bigint, extras?: { txid?: string | null; linkPagamento?: string | null; comprovanteGerado?: boolean }) {
    const rows = await this.query(
      `
        UPDATE captacao_doacoes
           SET situacao = $2,
  ...
  async registrarEventoDoacao(doacaoId: bigint, tipoEvento: string, descricao: string, payloadJson?: Record<string, unknown> | null, userId?: bigint) {
    await this.exec(
      `
  ...
  async listarEventosDoacao(doacaoId: bigint) {
    return this.query(`SELECT * FROM captacao_doacoes_eventos WHERE doacao_id = $1 ORDER BY created_at DESC, id DESC`, [doacaoId]);
  }

  async salvarTransacaoPix(doacaoId: bigint, result: Record<string, unknown>, userId?: bigint) {
    await this.exec(
      `
  ...
  async salvarTransacaoCartao(doacaoId: bigint, result: Record<string, unknown>, userId?: bigint) {
    await this.exec(
      `
  ...
  async salvarTransacaoBoleto(doacaoId: bigint, result: Record<string, unknown>, userId?: bigint) {
    await this.exec(
      `
  ...
  async marcarComprovanteEnviado(id: bigint, userId?: bigint) {
    await this.exec(
      `
  ...
  async buscarComprovantePorDoacao(doacaoId: bigint) {
    const rows = await this.query(
      `
  ...
  async listarComprovantes(filters: Record<string, unknown>): Promise<ListaResultado> {
    const params: unknown[] = [];
    const parts = ["1 = 1"];
    const pagina = Math.max(1, Number(filters.pagina ?? 1) || 1);
    const limite = Math.min(100, Math.max(1, Number(filters.limite ?? 20) || 20));
  ...
  async salvarConfiguracoes(input: Record<string, unknown>, userId?: bigint) {
  ...
  async registrarLog(entidadeTipo: string, entidadeId: bigint | null, acao: string, descricao: string, detalhesJson?: Record<string, unknown> | null, userId?: bigint) {
    await this.exec(
      `
  ...
  async listarDashboardBase(filters: Record<string, unknown>) {
  ...
  async criarAcessoPortal(doadorId: bigint, token: string, metadata: { email?: string; ip?: string; userAgent?: string }, userId?: bigint) {
  ...
  async listarDoacoesPorDoador(doadorId: bigint) {
    return this.query(
      `
  ...
  async listarRecorrenciasPorDoador(doadorId: bigint) {
    return this.query(
      `
  ...
  async listarComprovantesPorDoador(doadorId: bigint) {
    return this.query(
      `
  ...
  async atualizarDadosPortalDoador(doadorId: bigint, input: { email?: string; emailNorm?: string; telefone?: string; telefoneNorm?: string; whatsapp?: string; whatsappNorm?: string; cidade?: string; uf?: string }) {
    const rows = await this.query(
      `
        UPDATE captacao_doadores
