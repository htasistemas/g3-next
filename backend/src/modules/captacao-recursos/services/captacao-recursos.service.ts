...
import {
  mapCaptacaoCampanha,
  mapCaptacaoComprovante,
  mapCaptacaoConfiguracoes,
  mapCaptacaoDoacao,
  mapCaptacaoDoador,
  mapCaptacaoLog,
...
import {
  ...
  captacaoConfiguracoesSchema,
  captacaoDoacaoInputSchema,
...
import type { CaptacaoConfiguracoesInput } from "../captacao-recursos.types.js";
...
function gerarCodigoValidacao(seed: string) {
...
function normalizarDoadorPayload(input: ReturnType<typeof captacaoDoadorInputSchema.parse>) {
  const documento = trimOrUndefined(input.cpfCnpj);
  const digits = documento?.replace(/\D/g, "");
  const cpfCnpjNorm =
    digits?.length === 11 ? normalizarCpf(digits) : digits?.length === 14 ? normalizarCnpj(digits) : digits;
  return {
    ...input,
    cpfCnpj: documento,
    cpfCnpjNorm,
...
export class CaptacaoRecursosService {
  ...
  async getDashboard(rawFilters: unknown, rawTenantId?: string) {
    ...
    const soma = (items: typeof doacoes) => items.reduce((acc, item) => acc + item.valorLiquido, 0);
    ...
    const agruparPor = <T extends string>(items: typeof doacoes, key: (item: typeof doacoes[number]) => T) => {
  ...
  async saveDoacao(rawInput: unknown, userId?: string, rawId?: string, rawTenantId?: string) {
    ...
    const recorrencia = payload.recorrencia
      ? await this.repository.salvarRecorrencia(
          ...
          randomUUID(),
          payload.doadorId ? parseId(payload.doadorId) : null,
    ...
    const row = await this.repository.salvarDoacao(
      currentId,
      rawId ? String(rawId) : randomUUID(),
      rawId ? `DOA-${rawId}` : gerarNumero("DOA"),
      {
        ...payload,
  ...
  private async gerarBufferComprovante(doacao: ReturnType<typeof mapCaptacaoDoacao>, mensagemAgradecimento?: string, rawTenantId?: string) {
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      if (instituicao.cnpj) doc.text(`CNPJ: ${instituicao.cnpj}`, { align: "center" });
      if (instituicao.enderecoCompleto) doc.text(instituicao.enderecoCompleto, { align: "center" });
      doc.moveDown();
      ...
      if (doacao.campanhaNome) doc.text(`Campanha: ${doacao.campanhaNome}`);
      if (doacao.txid) doc.text(`TXID / referência: ${doacao.txid}`);
      doc.moveDown();
  ...
  async emitirComprovante(rawDoacaoId: string, userId?: string, rawTenantId?: string) {
    ...
    const comprovante = await this.repository.salvarComprovante(
      parseId(rawDoacaoId),
      {
        uuid: randomUUID(),
  ...
  async reenviarComprovante(rawDoacaoId: string, userId?: string, rawTenantId?: string) {
    ...
    const destinatario = trimOrUndefined(String((comprovanteRow as Record<string, unknown>).email_principal ?? ""));
    if (!destinatario) {
  ...
  async portalLogin(rawInput: unknown, metadata: { ip?: string; userAgent?: string } = {}) {
    const payload = captacaoPortalLoginSchema.parse(rawInput ?? {}) as {
      email: string;
      documento: string;
    };
  ...
  async atualizarPortalDoador(rawToken: string, rawInput: unknown) {
    const { token } = captacaoPortalTokenSchema.parse({ token: rawToken });
    const acesso = await this.repository.obterAcessoPortalValido(token);
    if (!acesso) throw new AppError("Sessão do portal expirada.", 401);
    const tenantId = parseTenant(typeof acesso.tenant_id === "string" ? acesso.tenant_id : undefined);
  ...
  async criarDoacaoPortal(rawToken: string, rawInput: unknown) {
    const { token } = captacaoPortalTokenSchema.parse({ token: rawToken });
    const acesso = await this.repository.obterAcessoPortalValido(token);
    if (!acesso) throw new AppError("Sessão do portal expirada.", 401);
    const tenantId = parseTenant(typeof acesso.tenant_id === "string" ? acesso.tenant_id : undefined);
