...
import {
  mensagemDestinatarioBuscaSchema,
  mensagemEnvioInputSchema,
  mensagemHistoricoFiltrosSchema,
  mensagemModeloFiltrosSchema,
  mensagemModeloInputSchema,
...
import type {
  ...
  MensagemVariavelDisponivel
} from "../mensagens-personalizadas.types.js";

const placeholdersDisponiveis: MensagemVariavelDisponivel[] = [
...
const taxonomiasBase: MensagemTaxonomiaInput[] = [
...
function formatarDataPtBr(valor?: string | Date | null) {
  if (!valor) return "";
...
function limparTelefoneWhatsapp(telefone?: string) {
  if (!telefone) return "";
...
function mapTaxonomia(row: MensagemTaxonomiaRow) {
  return {
    id: row.id.toString(),
    tipo: row.tipo,
    nome: row.nome,
...
function deduplicarStrings(valores: string[]) {
  return Array.from(new Set(valores.map((item) => item.trim()).filter(Boolean)));
}

function mapModelo(row: MensagemModeloRow) {
  ...
  const canalPermitido =
    canais.includes("WHATSAPP") && canais.includes("EMAIL")
      ? "AMBOS"
      : (canais[0] as "WHATSAPP" | "EMAIL" | undefined) ?? "WHATSAPP";
...
function mapHistorico(row: MensagemHistoricoRow) {
  return {
    id: row.id.toString(),
...
function destinatarioPermiteCanal(
  modelo: ReturnType<typeof mapModelo>,
...
function montarSaudacao(destinatario: MensagemDestinatarioDetalhe) {
  return `Olá, ${destinatario.primeiroNome || destinatario.nome}.`;
}

function montarAssinatura(contexto: Record<string, string>) {
  return contexto.instituicao || "Equipe G3 Next";
}

function substituirPlaceholders(texto: string, contexto: Record<string, string>) {
  return texto.replace(/\{([a-z0-9_]+)\}/gi, (_match, key) => contexto[key] ?? "");
}

function serializarSeguro(value: unknown) {
  try {
...
export class MensagensPersonalizadasService {
  ...
  private parseTenant(rawTenantId?: string | null) {
  ...
  async obterSuporte(rawTenantId?: string | null) {
  ...
  async listarTaxonomias(actor?: MensagemAtor) {
  ...
  async criarTaxonomia(rawInput: unknown, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
  ...
  async atualizarTaxonomia(rawId: string, rawInput: unknown, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
    const input = mensagemTaxonomiaInputSchema.parse(rawInput);
  ...
  async excluirTaxonomia(rawId: string, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
  ...
  async listarModelos(rawFiltros: unknown, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
  ...
  async obterModelo(rawId: string, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
  ...
  async criarModelo(rawInput: unknown, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
  ...
  async atualizarModelo(rawId: string, rawInput: unknown, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
    const input = mensagemModeloInputSchema.parse(rawInput);
  ...
  async duplicarModelo(rawId: string, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
    const original = await this.obterModelo(rawId, actor);
    const input: MensagemModeloInput = {
      ...
      tags: original.tags,
      tiposDestinatario: original.tiposDestinatario as MensagemDestinatarioTipo[],
      canalPermitido: original.canalPermitido as "WHATSAPP" | "EMAIL" | "AMBOS",
      mensagemBase: original.mensagemBase,
      variaveisPermitidas: original.variaveisPermitidas,
      status: original.status as "ATIVA" | "INATIVA",
  ...
  async atualizarStatusModelo(rawId: string, rawStatus: unknown, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
  ...
  async excluirModelo(rawId: string, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
  ...
  async listarHistorico(rawFiltros: unknown, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
  ...
  async buscarDestinatarios(rawQuery: unknown, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
  ...
  async enviarMensagem(rawInput: unknown, actor?: MensagemAtor) {
    const tenantId = this.parseTenant(actor?.tenant_id);
    await this.garantirBase(tenantId);
  ...
  private async garantirBase(rawTenantId?: string | null) {
  ...
  private async resolverModeloPreview(
    ...
    input: { canal: MensagemCanalEnvio; destinatarioTipo: MensagemDestinatarioTipo },
    tenantId: string,
    actor?: MensagemAtor
  ) {
    if (!modeloId) {
      ...
      const primeiroCompativel = candidatos.map(mapModelo).find((modelo) =>
        destinatarioPermiteCanal(modelo, input.destinatarioTipo, input.canal)
      );
  ...
  private async resolverDestinatario(tipo: MensagemDestinatarioTipo, id: string, tenantId: string) {
  ...
  private montarContexto(
    destinatario: MensagemDestinatarioDetalhe,
    actor?: MensagemAtor,
    ...
    return {
      ...
      data: formatarDataPtBr(extra.data as string | undefined) || formatarDataPtBr(agora),
      hora:
        typeof extra.hora === "string" && extra.hora.trim().length
          ? extra.hora.trim()
      ...
      data_registro:
        formatarDataPtBr((extra.data_registro as string | undefined) ?? destinatario.dataRegistro) ||
...
export async function ensureMensagensPersonalizadasBase(rawTenantId: string) {
  const tenantId = String(rawTenantId ?? "").trim();
  if (!tenantId) {
    throw new AppError("Tenant da sessao nao identificado.", 401);
  }
...
const modelosBaseParte1 = (
  taxonomiaId: (tipo: MensagemTaxonomiaInput["tipo"], nome: string) => string | null
): MensagemModeloSeedInput[] => [
  {
    ...
    mensagemBase:
      "Olá, {primeiro_nome}. Seu cadastro foi recebido com sucesso. Sempre que precisarmos, entraremos em contato pelos canais informados.",
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Olá, {primeiro_nome}. Seja bem-vindo(a) à equipe. Qualquer necessidade, procure o setor {setor}.",
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Olá, {primeiro_nome}. Agradecemos por aceitar contribuir conosco. Em breve você receberá as próximas orientações.",
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Olá. Registramos sua parceria com a instituição {instituicao}. Conte conosco para os próximos passos.",
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Olá, {primeiro_nome}. Obrigado por apoiar nossa instituição. Sua participação fortalece nossas ações sociais.",
    variaveisPermitidas: ["{primeiro_nome}"],
    mensagemPadraoSistema: true,
    mensagemPersonalizadaUsuario: false,
    mensagemSugeridaIa: true,
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Olá, {primeiro_nome}. Este é um lembrete do seu {atendimento} em {data}. Caso precise, responda esta mensagem.",
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Olá, {primeiro_nome}. Confirmamos o recebimento da doação {descricao_doacao}. Muito obrigado pelo apoio.",
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Registramos a doação {descricao_doacao} no valor de {valor_doacao} em {data}. Este texto pode ser usado como comprovante informativo.",
...
const modelosBaseParte2 = (
...
): MensagemModeloSeedInput[] => [
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Sua inscrição no curso {curso} foi registrada em {data}. Guarde esta mensagem como comprovante informativo.",
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Confirmamos a realização do {atendimento} em {data}, às {hora}.",
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Seu cadastro foi atualizado em {data}. Se houver alguma divergência, entre em contato conosco.",
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Informamos uma atualização importante referente às atividades da instituição em {data}.",
    variaveisPermitidas: ["{data}"],
    mensagemPadraoSistema: true,
    mensagemPersonalizadaUsuario: false,
    mensagemSugeridaIa: true,
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Aviso interno: verifique a atualização sobre {setor} e confirme ciência quando necessário.",
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Lembramos seu comparecimento em {data}, às {hora}. Leve os documentos necessários, se houver.",
    variaveisPermitidas: ["{data}", "{hora}"],
    mensagemPadraoSistema: true,
    mensagemPersonalizadaUsuario: false,
    mensagemSugeridaIa: true,
  ...
  },
  {
    ...
    canalPermitido: "AMBOS",
    mensagemBase:
      "Informamos o cancelamento do atendimento previsto para {data}. Assim que possível, entraremos em contato para novo alinhamento.",
    variaveisPermitidas: ["{data}"],
    mensagemPadraoSistema: true,
    mensagemPersonalizadaUsuario: false,
    mensagemSugeridaIa: true,
