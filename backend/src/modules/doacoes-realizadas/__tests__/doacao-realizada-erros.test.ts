...
import {
  calcularSaldoMovimentacaoDoacao,
  tratarErroPersistenciaDoacaoRealizada
} from "../repositories/doacao-realizada.repository.js";

function criarErroPrisma(
  code: string,
  meta: Record<string, unknown> = {},
  message = "Falha de persistencia"
) {
...
test("deve traduzir violacao de unicidade em conflito da entrega", () => {
  assert.throws(
    () =>
      tratarErroPersistenciaDoacaoRealizada(
        criarErroPrisma("P2002", { target: ["doacao_realizada_item"] }, "Unique constraint failed")
      ),
    (error) =>
...
test("deve traduzir referencia invalida em erro de validacao", () => {
  assert.throws(
    () =>
      tratarErroPersistenciaDoacaoRealizada(
        criarErroPrisma("P2003", { field_name: "almoxarifado_item_id" }, "Foreign key violation")
      ),
    (error) =>
      error instanceof AppError &&
...
test("deve traduzir registro ausente em not found", () => {
  assert.throws(
    () => tratarErroPersistenciaDoacaoRealizada(criarErroPrisma("P2025", {}, "Record not found")),
    (error) =>
      error instanceof AppError &&
