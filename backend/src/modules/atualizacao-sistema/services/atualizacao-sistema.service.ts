...
import {
  aplicarAtualizacaoSistemaSchema,
  atualizarConfigAtualizacaoSistemaSchema,
  atualizacaoSistemaChangelogSchema,
  atualizacaoSistemaManifestoSchema,
  rollbackAtualizacaoSistemaSchema
} from "../atualizacao-sistema.schema.js";
...
} from "./atualizacao-sistema.utils.js";
...
type BackupMetadata = {
  ...
  versaoNova: string;
  packageName: string;
  arquivos: BackupFileMetadata[];
};

type ContextoExecucaoAtualizacao = {
  execucaoId: string;
  usuarioExecucao: string;
  modo: AtualizacaoSistemaModo;
  manifesto: AtualizacaoSistemaManifesto;
  versaoInstaladaAntes: string;
};

type ContextoExecucaoRollback = {
  execucaoId: string;
  usuarioExecucao: string;
  historicoBase: AtualizacaoSistemaHistoricoItem;
  versaoInstaladaAntes: string;
};
...
function normalizarDataIso(valor?: string | null) {
  if (!valor?.trim()) return new Date().toISOString().slice(0, 10);
  return valor.trim().slice(0, 10);
}

function escapePowerShell(valor: string) {
...
async function coletarArquivosRecursivos(
  diretorio: string,
  raizBase = diretorio
): Promise<string[]> {
...
function filtrarArquivosPayload(arquivos: string[]) {
  return arquivos.filter((relativePath) => {
    ...
    if (!primeiraParte) return false;
    if (primeiraParte === "migrations" || primeiraParte === "rollback") return false;
    if (primeiraParte === "metadata.json" || primeiraParte === "manifest.json") return false;
    return true;
  });
}

export class AtualizacaoSistemaService {
  private readonly repository = new AtualizacaoSistemaRepository();

  private readonly paths = obterAtualizacaoSistemaPaths();

  private async garantirEstruturaArquivos() {
    ...
    if (!existsSync(this.paths.arquivoManifesto)) {
      const manifestoPadrao = {
        ...
        releaseType: "stable"
      } satisfies AtualizacaoSistemaManifesto;
      await writeFile(
        this.paths.arquivoManifesto,
        `${JSON.stringify(manifestoPadrao, null, 2)}\n`,
        "utf-8"
      );
    }
    ...
    if (!existsSync(this.paths.arquivoChangelog)) {
      const changelogPadrao = {
        ...
        ] satisfies AtualizacaoSistemaChangelogItem[]
      };
      await writeFile(
        this.paths.arquivoChangelog,
        `${JSON.stringify(changelogPadrao, null, 2)}\n`,
        "utf-8"
      );
    }
  }
  ...
  private async gravarVersaoInstalada(versao: string) {
  ...
  private async lerManifesto() {
    await this.garantirEstruturaArquivos();
    const manifesto = await carregarJson<unknown>(this.paths.arquivoManifesto);
  ...
  async obterChangelog() {
    await this.garantirEstruturaArquivos();
    const changelog = await carregarJson<unknown>(this.paths.arquivoChangelog);
  ...
  async salvarConfig(rawPayload: unknown, usuarioAtualizacao: string) {
  ...
  async listarLogs(rawFilters: { execucaoId?: string; limite?: string | number }) {
  ...
  async aplicarAtualizacao(rawPayload: unknown, usuarioExecucao: string, modoForcado?: AtualizacaoSistemaModo) {
    ...
    if (
      manifesto.minCompatibleVersion &&
      compararVersoes(versaoInstalada, manifesto.minCompatibleVersion) < 0
    ) {
      throw new AppError(
        `A versão instalada ${versaoInstalada} não atende a compatibilidade mínima ${manifesto.minCompatibleVersion}.`,
        409
      );
    }
  ...
  async rollback(rawPayload: unknown, usuarioExecucao: string) {
  ...
  async verificarEAplicarAutomaticamente() {
    ...
    try {
      ...
      const mensagem =
        error instanceof Error ? error.message : "Falha ao iniciar atualização automática.";
      await this.repository.atualizarStatus({
  ...
  private async executarFluxoAtualizacao(contexto: ContextoExecucaoAtualizacao) {
    const inicioMs = Date.now();
    let backupDiretorio: string | null = null;

    try {
      await this.atualizarProgresso(
        contexto.execucaoId,
        "VALIDANDO",
        "Validando manifesto e compatibilidade.",
        10
      );

      const caminhoPacote = await this.localizarOuBaixarPacote(
        contexto.manifesto,
        contexto.execucaoId
      );
      ...
      await this.atualizarProgresso(
        contexto.execucaoId,
        "VALIDANDO",
        "Validando checksum do pacote publicado.",
        20
      );
      ...
      try {
        ...
        await this.atualizarProgresso(
          contexto.execucaoId,
          "BACKUP",
          "Gerando backup dos arquivos impactados.",
          35
        );
        ...
        await this.atualizarProgresso(
          contexto.execucaoId,
          "APLICANDO",
          "Aplicando arquivos do pacote versionado.",
          55
        );
        ...
        await this.atualizarProgresso(
          contexto.execucaoId,
          "MIGRANDO",
          "Executando migrações SQL do pacote.",
          75
        );
        ...
        await this.registrarLog(
          ...
          "INFO",
          "FINALIZACAO",
          `Atualização concluída em ${formatarDuracaoHumana(duracaoMs)}.`,
          {
            backupDiretorio,
            versaoAnterior: contexto.versaoInstaladaAntes,
            versaoNova: contexto.manifesto.latestVersion
      ...
      await this.repository.finalizarExecucao({
        ...
        versaoPublicada: contexto.manifesto.latestVersion,
        atualizacaoDisponivel: existeNovaVersao(
          contexto.versaoInstaladaAntes,
          contexto.manifesto.latestVersion
        ),
        usuarioExecucao: contexto.usuarioExecucao,
  ...
  private async executarFluxoRollback(contexto: ContextoExecucaoRollback) {
    const inicioMs = Date.now();
    ...
    try {
      await this.atualizarProgresso(
        contexto.execucaoId,
        STATUS_ROLLBACK,
        "Restaurando backup da atualização.",
        25
      );
      ...
      await this.atualizarProgresso(
        contexto.execucaoId,
        STATUS_ROLLBACK,
        "Aplicando scripts de rollback do pacote, quando existentes.",
        70
      );

      await this.executarSqls(
        path.join(contexto.historicoBase.backupDiretorio!, "rollback"),
        contexto.execucaoId,
        "ROLLBACK_SQL"
      );
      ...
      await this.registrarLog(
        ...
        "INFO",
        STATUS_ROLLBACK,
        `Rollback concluído em ${formatarDuracaoHumana(duracaoMs)}.`,
        {
      ...
      await this.repository.registrarHistorico({
        ...
        detalhes: {
          historicoBaseId: contexto.historicoBase.id,
          observacao:
            "Rollback restaurou arquivos versionados e executou apenas SQLs presentes na pasta rollback."
        },
      ...
      await this.repository.finalizarExecucao({
        ...
        atualizacaoDisponivel: existeNovaVersao(
          versaoRetorno,
          contexto.historicoBase.versaoNova ?? versaoRetorno
        ),
        usuarioExecucao: contexto.usuarioExecucao,
        responsavelUltimaAtualizacao: contexto.usuarioExecucao,
        registrarUltimaAtualizacao: true
  ...
  private async registrarLog(
    ...
    await appendFile(
      path.join(this.paths.diretorioLogs, `${execucaoId}.log`),
      `${new Date().toISOString()} [${nivel}] [${etapa}] ${mensagem}${
  ...
  private async localizarOuBaixarPacote(
    manifesto: AtualizacaoSistemaManifesto,
    execucaoId?: string
  ) {
    ...
    if (!manifesto.downloadUrl?.trim()) {
      throw new AppError(
        `Pacote ${nomePacote} não localizado em /updates/packages e o manifesto não possui downloadUrl.`,
        409
      );
    }

    await this.registrarLog(
      execucaoId ?? "download-manual",
      "INFO",
      "DOWNLOAD",
      `Baixando pacote ${nomePacote} a partir da origem publicada.`,
      {
  ...
  private async prepararPacoteExtraido(caminhoPacote: string, destino: string) {
  ...
  private async resolverDiretorioPayload(diretorioPacote: string) {
  ...
  private async gerarBackup(input: {
    execucaoId: string;
    versaoAnterior: string;
    versaoNova: string;
    packageName: string;
    diretorioPayload: string;
    diretorioRollback: string;
  }) {
    ...
    const arquivosPayload = filtrarArquivosPayload(
      await coletarArquivosRecursivos(input.diretorioPayload)
    );
    const arquivos: BackupFileMetadata[] = [];
    ...
    const relativeVersionPath = path.relative(
      this.paths.raizRepositorio,
      this.paths.arquivoVersaoInstalada
    );
    arquivos.push({
    ...
    await writeFile(
      path.join(diretorioBackup, "metadata.json"),
  ...
  private async aplicarPayload(diretorioPayload: string) {
  ...
  private async restaurarBackup(diretorioBackup: string) {
    ...
    const metadata = await carregarJson<BackupMetadata>(metadataPath);
