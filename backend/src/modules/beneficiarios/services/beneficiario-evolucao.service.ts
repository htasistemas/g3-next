...
function parseId(rawId: string): bigint {
...
export class BeneficiarioEvolucaoService {
  ...
  async analisarDuplicidade(rawPayload: unknown, ator: AtorBeneficiario) {
    ...
    const conditions: Prisma.Sql[] = [Prisma.sql`b.tenant_id::text = ${tenantId}`];
    if (idIgnorado) conditions.push(Prisma.sql`b.id <> ${idIgnorado}`);
    ...
    if (dataNascimento) {
      ...
      if (data) filtros.push(Prisma.sql`b.data_nascimento = ${data}`);
    }
    if (endereco) {
      filtros.push(Prisma.sql`EXISTS (
    ...
    const duplicidades = rows.map((row) => {
      let score = 0;
      if (cpf && normalizeDigits(String(row.cpf ?? "")) === cpf) score += 45;
      if (telefone && normalizeDigits(String(row.telefone_principal ?? "")) === telefone) score += 10;
      if (dataNascimento && String(row.data_nascimento ?? "").startsWith(dataNascimento)) score += 10;
      if (nomeBusca && normalizarTextoBusca(String(row.nome_completo ?? "")) === nomeBusca) score += 20;
      if (maeBusca && normalizarTextoBusca(String(row.nome_mae ?? "")) === maeBusca) score += 10;
      if (score === 0) score = 35;
      return {
  ...
  async criarRapido(rawPayload: unknown, ator: AtorBeneficiario) {
    const tenantId = parseTenantId(ator.tenantId);
    const payload = (rawPayload && typeof rawPayload === "object" ? rawPayload : {}) as Record<string, unknown>;
    ...
    if (cpf && duplicidades.duplicidades.some((item: any) => item.cpf_mascarado === mascararCpf(cpf))) {
      throw new AppError("Ja existe cadastro com este CPF neste tenant.", 409);
    }

    const codigoRows = await prisma.$queryRaw<Array<{ max_code: number | null }>>(Prisma.sql`
    ...
    const criado = await prisma.$transaction(async (tx) => {
      const pessoaRows = await tx.$queryRaw<Array<{ id: bigint }>>(Prisma.sql`
        INSERT INTO pessoa (
      ...
      const beneficiarioRows = await tx.$queryRaw<Array<{ id: bigint }>>(Prisma.sql`
  ...
  async recalcularCompletude(rawId: string, ator: AtorBeneficiario) {
    const tenantId = parseTenantId(ator.tenantId);
    ...
    const familiaRows = await prisma.$queryRaw<Array<{ id: bigint; nome_familia: string }>>(Prisma.sql`
      ...
      LIMIT 1
    `);
    const consentimentosRows = await prisma.$queryRaw<Array<{ total: number }>>(Prisma.sql`
    ...
    const percentual = Math.min(
      100,
      Math.round(
        Object.entries(pesosPadrao).reduce((total, [grupo, peso]) => {
          const item = grupos[grupo as keyof typeof grupos];
  ...
  async obterCompletude(rawId: string, ator: AtorBeneficiario) {
    const tenantId = parseTenantId(ator.tenantId);
    const id = parseId(rawId);
    const rows = await prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
  ...
  async listarConsentimentos(rawId: string, ator: AtorBeneficiario) {
    const tenantId = parseTenantId(ator.tenantId);
    const id = parseId(rawId);
  ...
  async registrarConsentimento(rawId: string, rawPayload: unknown, ator: AtorBeneficiario) {
    const tenantId = parseTenantId(ator.tenantId);
    const id = parseId(rawId);
    await this.repository.buscarPorIdOuFalhar(id, tenantId);
    const payload = (rawPayload && typeof rawPayload === "object" ? rawPayload : {}) as Record<string, unknown>;
    const tipo = String(payload.tipo ?? "").trim().toUpperCase();
    if (!tipo) throw new AppError("Informe o tipo do consentimento.", 422);
  ...
  async listarAuditoria(rawId: string, ator: AtorBeneficiario, completa = false) {
    const tenantId = parseTenantId(ator.tenantId);
    const id = parseId(rawId);
    await this.repository.buscarPorIdOuFalhar(id, tenantId);
    const rows = await prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
  ...
  async obterResumoFamilia(rawId: string, ator: AtorBeneficiario) {
    const tenantId = parseTenantId(ator.tenantId);
    const id = parseId(rawId);
    await this.repository.buscarPorIdOuFalhar(id, tenantId);
    const rows = await prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
  ...
  async listarPendencias(ator: AtorBeneficiario, rawQuery: unknown) {
    const tenantId = parseTenantId(ator.tenantId);
  ...
  async registrarAtualizacaoGrupos(rawId: string, ator: AtorBeneficiario, origemInformacao = "ATUALIZACAO_ADMINISTRATIVA") {
    const tenantId = parseTenantId(ator.tenantId);
    const id = parseId(rawId);
    const usuarioId = usuarioIdBigInt(ator.usuarioId);
  ...
  async garantirPessoaBeneficiario(rawId: string, ator: AtorBeneficiario) {
    const tenantId = parseTenantId(ator.tenantId);
    const id = parseId(rawId);
    const existente = await prisma.$queryRaw<Array<{ pessoa_id: bigint | null }>>(Prisma.sql`
      SELECT pessoa_id
      FROM cadastro_beneficiario
      WHERE id = ${id}
        AND tenant_id::text = ${tenantId}
  ...
  async registrarAuditoria(rawId: bigint, acao: string, campo: string | null, anterior: unknown, posterior: unknown, ator: AtorBeneficiario) {
    const tenantId = parseTenantId(ator.tenantId);
    await prisma.$executeRaw(Prisma.sql`
  ...
  private calcularGrupo(beneficiario: Record<string, unknown>, campos: Array<[string, string]>) {
    const pendencias: PendenciaCompletude[] = [];
    let preenchidos = 0;
