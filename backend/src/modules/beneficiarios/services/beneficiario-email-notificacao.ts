...
function formatarLabel(campo: string) {
  return LABELS[campo] ?? campo.replace(/_/g, " ").replace(/^\w/, (char) => char.toUpperCase());
}

function formatarDocumento(documento: unknown) {
...
function formatarValor(valor: unknown, campo?: string): string {
  if (valor == null) {
  ...
  if (Array.isArray(valor)) {
    const itens = valor
      .map((item) =>
        campo === "documentos_obrigatorios" ? formatarDocumento(item) : String(item ?? "").trim()
      )
      .filter(Boolean)
...
export function montarResumoAlteracoesBeneficiario(
...
export function obterDestinatariosAlteracaoBeneficiario(
  anterior: Record<string, unknown>,
  atual: Record<string, unknown>
) {
...
export function montarMensagemAlteracoesBeneficiario(
  atual: Record<string, unknown>,
  alteracoes: string[]
) {
