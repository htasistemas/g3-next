...
function mapDocumento(row: TermoDocumentoRow) {
  return {
...
export function mapTermoFomentoToResponse(
  termo: TermoFomentoRow,
  aditivos: TermoAditivoRow[],
  documentos: TermoDocumentoRow[]
) {
  const termoDocumento = documentos.find((item) => item.tipo_documento === "termo");
  const documentosRelacionados = documentos.filter(
    (item) => item.tipo_documento !== "termo" && item.tipo_documento !== "aditivo"
  );
