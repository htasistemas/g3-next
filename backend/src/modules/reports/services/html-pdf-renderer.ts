...
function extractSectionSpacerLines(value: string): number | null {
  ...
  private async renderEstruturado(layout: RelatorioHtmlInput, rodape: RodapeRender): Promise<Buffer> {
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      const desenharTabela = () => {
        ...
        const obterFonteTamanhoColuna = (index: number, cabecalho: boolean) => {
          const coluna = tabela.colunas[index];
          if (cabecalho) return coluna?.fonteTamanhoCabecalho ?? 9;
        ...
        const desenharLinhaTabela = (cels: Array<string | RelatorioTabelaCelula>, cabecalho = false) => {
          ...
          for (let index = 0; index < larguras.length; index += 1) {
            ...
            doc
              ...
              .text(
                coluna?.semQuebra ? fitTextToWidth(doc, textoBase, larguraTexto) : textoBase,
                cursorX + paddingX,
                topoLinha + paddingY,
                {
                  width: larguraTexto,
      ...
      const desenharSecoes = () => {
        ...
        for (const secao of layout.secoes) {
          ...
          if (!tituloSecao || !linhas.some(Boolean)) continue;
