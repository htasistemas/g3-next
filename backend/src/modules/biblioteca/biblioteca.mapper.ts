import type { BibliotecaEmprestimoRow, BibliotecaLivroRow } from "./biblioteca.types.js";

export function mapLivroRowToResponse(row: BibliotecaLivroRow) {
  return {
    id: String(row.id),
    codigo: row.codigo,
    titulo: row.titulo,
...
export function mapEmprestimoRowToResponse(row: BibliotecaEmprestimoRow) {
  return {
    id: String(row.id),
