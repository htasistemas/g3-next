...
import type { EvidenciaPublicaInput, IndicadorPublicoInput, ParceriaPublicaInput } from "./parcerias-publicas.types.js";

const id = (value: number) => BigInt(value);
const data = (value?: string | null) => value ? new Date(`${value}T00:00:00.000Z`) : null;
...
export class ParceriasPublicasRepository {
  async listar(tenantId: string) {
    ...
    const parceriaIds = rows.map((row) => row.id as bigint);
    if (!parceriaIds.length) return parcerias;
    const indicadores = await prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
    ...
    `);
    const indicadorIds = indicadores.map((row) => row.id as bigint);
    const evidencias = indicadorIds.length ? await prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
  ...
  async criarParceria(input: ParceriaPublicaInput, tenantId: string) {
  ...
  async criarIndicador(input: IndicadorPublicoInput, tenantId: string) {
  ...
  async criarEvidencia(input: EvidenciaPublicaInput, tenantId: string) {
  ...
  private async validarTermoUnidade(termoId: number, unidadeId: number, tenantId: string) {
    ...
    `);
    if (!rows[0]) throw new AppError("Termo de fomento ou unidade de ensino não encontrado nesta instituição.", 404);
  }

  private async validarExistencia(tabela: string, registroId: number, tenantId: string, mensagem: string) {
    const rows = await prisma.$queryRaw<Array<{ id: bigint }>>(Prisma.sql`SELECT id FROM ${Prisma.raw(tabela)} WHERE id = ${id(registroId)} AND tenant_id::text = ${tenantId} LIMIT 1`);
    if (!rows[0]) throw new AppError(mensagem, 404);
  }

  private serializar(row: Record<string, unknown> | undefined) { return Object.fromEntries(Object.entries(row ?? {}).map(([key, value]) => [key, typeof value === "bigint" ? String(value) : value instanceof Date ? value.toISOString().slice(0, 10) : value])); }
}
