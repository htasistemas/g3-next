...
function precisaExecutarNovamente(
  ...
  if (frequencia === "diaria") return diff >= 24 * 60 * 60 * 1000;
  if (frequencia === "semanal") return diff >= 7 * 24 * 60 * 60 * 1000;
...
export function iniciarDatasComemorativasScheduler(intervaloMs = 60 * 60 * 1000) {
  ...
  const executar = async () => {
    try {
      ...
      const ultimoLog = logs.find(
        (item) => item.tipo_sync === "feriados" && item.status_execucao !== "erro"
      );

      if (
        !precisaExecutarNovamente(
          configuracoes.frequenciaSincronizacao,
          ultimoLog?.iniciado_em
        )
      ) {
        return;
      }
