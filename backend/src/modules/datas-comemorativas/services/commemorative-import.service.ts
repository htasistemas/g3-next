...
export class CommemorativeImportService {
  ...
  async ensureSeedBase(usuarioId?: string) {
    ...
    const chavesExistentes = new Set(
      existentes.rows.map(
        (item) =>
          `${item.titulo.trim().toLowerCase()}|${item.dia}|${item.mes}|${item.tipo_evento}|${item.abrangencia}`
      )
    );
  ...
  async importFromSeed(usuarioId?: string) {
    return this.upsertBatch(datasComemorativasSeed, "seed", usuarioId);
  }

  async importFromJson(conteudo: string, usuarioId?: string) {
    let parsed: unknown;
    try {
    ...
    return this.upsertBatch(
      itens.map((item) => this.normalizarItem(item)),
      "importacao_json",
      usuarioId
    );
  }

  async importFromCsv(conteudo: string, usuarioId?: string) {
    ...
    const itens = linhas.slice(1).map((linha) => {
      ...
      const registro = cabecalho.reduce<Record<string, string>>((acc, chave, indice) => {
  ...
  async importar(rawInput: unknown, usuarioId?: string) {
  ...
  async upsertBatch(items: DataComemorativaImportItem[], source: string, usuarioId?: string) {
    ...
    const counters: ImportBatchResult = {
      lidos: items.length,
      inseridos: 0,
      atualizados: 0,
      ignorados: 0,
  ...
  private normalizarItem(item: unknown): DataComemorativaImportItem {
    ...
    const recorrenteAnual = this.toBoolean(
      registro.recorrenteAnual ?? registro.recorrente_anual,
      true
    );
    ...
    return {
      ...
      origemReferencia: this.toOptionalString(
        registro.origemReferencia ?? registro.origem_referencia
      ),
      corExibicao: this.toOptionalString(registro.corExibicao ?? registro.cor_exibicao),
      icone: this.toOptionalString(registro.icone),
      prioridadePopup:
        this.toOptionalNumber(registro.prioridadePopup ?? registro.prioridade_popup) ?? 0,
      exibirNoPopup: this.toBoolean(
        registro.exibirNoPopup ?? registro.exibir_no_popup,
        true
      ),
  ...
  private toTipoEvento(value: unknown): TipoEvento {
    const normalized = String(value ?? "comemorativa").trim().toLowerCase();
    if (
      normalized === "feriado_nacional" ||
  ...
  private toAbrangencia(value: unknown): AbrangenciaEvento {
