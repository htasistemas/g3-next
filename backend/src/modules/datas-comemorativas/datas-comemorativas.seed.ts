...
const coresPorTipo: Record<TipoEvento, string> = {
  comemorativa: "#2563eb",
...
const eventosInstitucionais = new Set<string>(["Aniversário da Revelare"]);

function buildSeedKey(mes: number, dia: number, titulo: string) {
  return `${mes}-${dia}-${titulo.trim().replace(/\s+/gu, " ").toLowerCase()}`;
}

const overrides2026 = new Map<string, SeedOverride>([
  [
...
const blocosMensais: SeedMonthBlock[] = [
...
function createSeedItem(mes: number, dia: number, titulo: string): DataComemorativaImportItem {
  ...
  const tipoEvento =
    ...
    (eventosInstitucionais.has(titulo)
      ...
      : feriadosNacionais.has(titulo)
        ...
        : "comemorativa");
  const abrangencia =
    override?.abrangencia ?? (eventosInstitucionais.has(titulo) ? "interna" : "nacional");
...
function parseMonthBlock({ mes, texto }: SeedMonthBlock) {
  const itens: DataComemorativaImportItem[] = [];
...
export const datasComemorativasSeed: DataComemorativaImportItem[] = blocosMensais.flatMap(
  parseMonthBlock
);
