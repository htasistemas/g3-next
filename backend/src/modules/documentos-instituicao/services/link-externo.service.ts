...
type LinkExternoSeed = [nome: string, url: string, tiposRelacionados?: string, observacao?: string];
...
const linksPadrao: LinkExternoSeed[] = [
  [
...
async function ensureLinkExternoEstrutura() {
  ...
  const totalRegistros = await prisma.$queryRawUnsafe<Array<{ count: bigint | number | string }>>(
    'SELECT COUNT(*)::bigint AS count FROM "link_externo_documento"'
  );
