...
function formatarIsoDate(data: Date) {
...
function calcularTendencia(atual: number, anterior: number): DashboardPowerBiCardTrend {
...
export class DashboardPowerBiService {
  ...
  private async gerarPowerBi(
    ...
    const [
      ...
      pendencias,
      casosPrioritarios,
      documentacoesPendentes,
      projetosAtivos,
      acoesColetivas,
      participantesOficinas,
      instituicoesParceiras,
      conveniosAtivos,
      familiasAcompanhamento,
      seriesCadastros,
      seriesAtendimentos,
      statusDistribuicao,
      familiasPorBairro,
      ...
      beneficiosPorMes,
      beneficiosPorTipo,
      beneficiosDeferidosIndeferidos,
      encaminhamentosPorMes,
      encaminhamentosPorTipo,
      encaminhamentosPorInstituicao,
      encaminhamentosPendentesRetorno,
      participacaoPorMes,
      projetosPorAdesao,
      participacaoFaixaEtariaProjeto,
      conveniosPorTipo,
      conveniosPorInstituicao,
      conveniosPorVencimento,
      pendenciasCriticas,
      indicadoresResumo,
      detalhamentos,
      totalFamiliasAnterior,
      totalBeneficiariosAnterior,
      novasInscricoesAnterior,
      totalAtendimentosAnterior,
      totalEncaminhamentosAnterior,
      totalBeneficiosAnterior,
      pendenciasAnterior
    ] = await Promise.all([
    ...
    return {
      ...
      indicadoresResumo: {
        ...
        tempoMedioEntreAtendimentosDias: arredondarUmaCasa(
          indicadoresResumo.tempoMedioEntreAtendimentosDias
        ),
  ...
  private resolverPeriodo(
    filtros: DashboardPowerBiFiltros
  ): Pick<Required<DashboardPowerBiFiltros>, "periodPreset" | "startDate" | "endDate"> {
    const hoje = new Date();
  ...
  private calcularPeriodoAnterior(filtros: Required<DashboardPowerBiFiltros>): Required<DashboardPowerBiFiltros> {
  ...
  private criarFiltrosMesAtual(filtros: Required<DashboardPowerBiFiltros>): Required<DashboardPowerBiFiltros> {
