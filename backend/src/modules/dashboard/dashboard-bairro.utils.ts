...
export function normalizarBairro(valor?: string | null): string {
  return String(valor ?? "")
    .trim()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
export function formatarBairro(valor?: string | null): string {
  const texto = String(valor ?? "").trim();
  return texto ? formatarTextoPadrao(texto) : "Nao informado";
}

export function sqlNormalizarBairro(coluna: string) {
