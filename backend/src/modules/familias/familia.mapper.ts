...
import type { FamiliaDbRecord } from "./repositories/familia.repository.js";

function mapBeneficiarioResumo(
  beneficiario: FamiliaDbRecord["referenciaFamiliar"] | FamiliaDbRecord["membros"][number]["beneficiario"] | null
) {
  if (!beneficiario) {
    return null;
  }
...
export function mapFamiliaToResponse(record: FamiliaDbRecord) {
  return {
