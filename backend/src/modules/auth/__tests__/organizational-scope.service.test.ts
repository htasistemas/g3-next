...
test("escopo de projeto não herda acesso para outro projeto", () => {
  const acesso = { instituicao_id: instituicao, projeto_id: "p-1", escopo: "PROJETO" as const };
...
test("exigirEscopo responde 403 para instituição cruzada", () => {
  ...
  ], { instituicao_id: "instituicao-b" }), (error: unknown) => {
    return error instanceof Error && "statusCode" in error && (error as { statusCode: number }).statusCode === 403;
  });
});
