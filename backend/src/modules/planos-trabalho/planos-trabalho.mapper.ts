...
function splitMetasVinculadas(value?: string | null) {
  return (value ?? "")
    .split("|")
    .map((item) => item.trim())
    .filter(Boolean);
...
export function mapPlanoTrabalhoToResponse(
  ...
  objetivosEspecificos: PlanoObjetivoEspecificoRow[],
  metas: PlanoMetaRow[],
  etapas: PlanoMetaEtapaRow[],
  aplicacaoRecursos: PlanoAplicacaoRecursoRow[],
  desembolso: PlanoDesembolsoRow[],
  checklistPrestacao: PlanoChecklistPrestacaoRow[]
) {
  return {
