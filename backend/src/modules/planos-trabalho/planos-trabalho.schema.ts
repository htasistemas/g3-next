...
const requiredTrimmedString = (message: string, min = 2) =>
  z.preprocess((value) => (typeof value === "string" ? value.trim() : value), z.string().min(min, message));
...
const requiredIsoDate = (message: string) =>
  z.preprocess((value) => (typeof value === "string" ? value.trim() : value), z.string().regex(/^\d{4}-\d{2}-\d{2}$/, message));
...
function exibirData(data: string) {
...
const optionalCpfSchema = z.preprocess(
  ...
  cpfSchema.optional().nullable()
);

const optionalCnpjSchema = z.preprocess(
  (value) => (typeof value === "string" && !value.trim() ? undefined : value),
  cnpjSchema.optional().nullable()
);
...
export const planoTrabalhoInputSchema = z
  .object({
    ...
    uf: z.preprocess(
      ...
      z.string().length(2, "Informe a UF com 2 letras.").optional()
    ).nullable().optional(),
  ...
  .superRefine((input, ctx) => {
    if (input.status !== "RASCUNHO") {
      const obrigatorios: Array<[keyof typeof input, string]> = [
