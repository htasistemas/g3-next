...
import type { ProntuarioAdendoInput, ProntuarioAtendimentoInput, ProntuarioStatus } from "../prontuario.types.js";
...
function toOptionalDateTime(value?: string | null, baseDate?: string | null): Date | undefined {
...
export class ProntuarioRepository {
  async ensureEstrutura() {
    if (!estruturaPromise) {
      estruturaPromise = (async () => {
        for (const comando of estruturaSql) await prisma.$executeRawUnsafe(comando);
      })().catch((error) => {
  ...
  async buscarBeneficiarios(busca: string | undefined, tenantId: string) {
  ...
  async obterContexto(beneficiarioId: bigint, tenantId: string) {
    await this.ensureEstrutura();
    const beneficiarios = await prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
      SELECT b.id, b.codigo, b.nome_completo, b.nome_social, b.nome_mae, b.data_nascimento, b.foto_3x4,
             e.bairro,
             c.telefone_principal,
             cpf_doc.numero_documento AS cpf,
  ...
  async listarAtendimentos(beneficiarioId: bigint, tenantId: string, status?: string[]) {
    await this.ensureEstrutura();
  ...
  async criarAtendimento(beneficiarioId: bigint, input: ProntuarioAtendimentoInput, tenantId: string, usuario: UsuarioActor) {
    await this.ensureEstrutura();
    ...
    if (!id) throw new AppError("Não foi possível iniciar o atendimento.", 500);
  ...
  async atualizarAtendimento(id: bigint, input: ProntuarioAtendimentoInput, tenantId: string, usuario: UsuarioActor) {
    await this.ensureEstrutura();
    const atual = await this.buscarAtendimento(id, tenantId);
    if (!atual) throw new AppError("Prontuário não encontrado.", 404);
    if (atual.status === "FINALIZADO" || atual.status === "CANCELADO") throw new AppError("Atendimentos finalizados não podem ser editados. Use um adendo.", 409);
  ...
  async finalizarAtendimento(id: bigint, tenantId: string, usuario: UsuarioActor) {
    await this.ensureEstrutura();
    const atual = await this.buscarAtendimento(id, tenantId);
    if (!atual) throw new AppError("Prontuário não encontrado.", 404);
    if (atual.status === "FINALIZADO") return atual;
    if (!String(atual.evolucao ?? atual.avaliacao ?? atual.motivo ?? "").trim()) throw new AppError("Informe ao menos a evolução, avaliação ou motivo antes de finalizar.", 422);
    const fim = new Date();
  ...
  async criarAdendo(id: bigint, input: ProntuarioAdendoInput, tenantId: string, usuario: UsuarioActor) {
    await this.ensureEstrutura();
    const atual = await this.buscarAtendimento(id, tenantId);
    if (!atual) throw new AppError("Prontuário não encontrado.", 404);
    if (atual.status !== "FINALIZADO") throw new AppError("Adendos só podem ser incluídos em atendimentos finalizados.", 409);
  ...
  private async buscarAtendimento(id: bigint, tenantId: string) {
    const rows = await prisma.$queryRaw<Array<Record<string, any>>>(Prisma.sql`SELECT pa.*, COALESCE((SELECT json_agg(json_build_object('id', ad.id, 'conteudo', ad.conteudo, 'motivo', ad.motivo, 'usuario_nome', ad.usuario_nome, 'criado_em', ad.criado_em) ORDER BY ad.criado_em DESC) FROM prontuario_adendo ad WHERE ad.atendimento_id = pa.id AND ad.tenant_id::text = ${tenantId}), '[]'::json) AS adendos FROM prontuario_atendimento pa WHERE pa.id = ${id} AND pa.tenant_id::text = ${tenantId} LIMIT 1`);
    return rows[0] ?? null;
  }

  private async validarBeneficiario(id: bigint, tenantId: string) {
    const rows = await prisma.$queryRaw<Array<{ id: bigint }>>(Prisma.sql`SELECT id FROM cadastro_beneficiario WHERE id = ${id} AND tenant_id::text = ${tenantId} LIMIT 1`);
    if (!rows[0]) throw new AppError("Beneficiário não encontrado nesta instituição.", 404);
  }

  private async obterOuCriarProntuario(beneficiarioId: bigint, tenantId: string) {
    const existente = await prisma.$queryRaw<Array<{ id: bigint }>>(Prisma.sql`SELECT id FROM prontuario WHERE beneficiario_id = ${beneficiarioId} AND tenant_id::text = ${tenantId} LIMIT 1`);
    if (existente[0]) return existente[0].id;
    const codigo = `P-${beneficiarioId.toString().padStart(8, "0")}`;
    const criado = await prisma.$queryRaw<Array<{ id: bigint }>>(Prisma.sql`INSERT INTO prontuario (tenant_id, beneficiario_id, numero_prontuario) VALUES (${tenantId}::uuid, ${beneficiarioId}, ${codigo}) ON CONFLICT (tenant_id, beneficiario_id) DO UPDATE SET atualizado_em = NOW() RETURNING id`);
    if (!criado[0]) throw new AppError("Não foi possível criar o prontuário.", 500);
    return criado[0].id;
  }

  private async registrarAuditoria(prontuarioId: bigint, atendimentoId: bigint, acao: string, descricao: string, tenantId: string, usuario: UsuarioActor) {
