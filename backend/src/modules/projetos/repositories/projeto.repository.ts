...
import type {
  ProjetoDashboardFrequenciaRow,
  ProjetoDashboardPrazoRow,
  ProjetoDashboardResumoRow,
  ProjetoFilters,
  ProjetoHistoricoRow,
  ProjetoInput,
  ProjetoRow,
...
export class ProjetoRepository {
  ...
  async listar(filters: ProjetoFilters | Record<string, unknown>, tenantId: string) {
    await this.garantirEstrutura();
    const where = this.buildWhere(filters as ProjetoFilters, tenantId, "p");
    const projetos = await prisma.$queryRaw<ProjetoRow[]>(Prisma.sql`
      SELECT
        p.id,
  ...
  async dashboard(filters: ProjetoFilters | Record<string, unknown>, tenantId: string) {
    await this.garantirEstrutura();
    const where = this.buildWhere(filters as ProjetoFilters, tenantId, "p");
    const [resumo] = await prisma.$queryRaw<ProjetoDashboardResumoRow[]>(Prisma.sql`
      SELECT
    ...
    const porStatus = await prisma.$queryRaw<ProjetoDashboardFrequenciaRow[]>(Prisma.sql`
      ...
      ORDER BY p.status
    `);
    const porPrioridade = await prisma.$queryRaw<ProjetoDashboardFrequenciaRow[]>(Prisma.sql`
    ...
    `);
    const evolucao = await prisma.$queryRaw<ProjetoDashboardFrequenciaRow[]>(Prisma.sql`
      SELECT
        p.nome AS chave,
        CASE
          WHEN COUNT(t.id) = 0 AND p.status = 'CONCLUIDO' THEN 100
      ...
      LIMIT 8
    `);
    const tarefasPorResponsavel = await prisma.$queryRaw<ProjetoDashboardFrequenciaRow[]>(Prisma.sql`
      ...
      LIMIT 8
    `);
    const vencimentos = await prisma.$queryRaw<ProjetoDashboardPrazoRow[]>(Prisma.sql`
  ...
  async buscarPorId(id: bigint, tenantId: string) {
    await this.garantirEstrutura();
    const rows = await prisma.$queryRaw<ProjetoRow[]>(Prisma.sql`
      SELECT
        p.id,
        p.tenant_id::text AS tenant_id,
        p.nome,
  ...
  async buscarPorIdOuFalhar(id: bigint, tenantId: string) {
    ...
    if (!registro) throw new AppError("Projeto não encontrado.", 404);
    return registro;
  }

  async criar(usuarioId: bigint | undefined, usuarioNome: string | undefined, input: ProjetoInput, tenantId: string) {
    await this.garantirEstrutura();
    ...
    const projetoId = await prisma.$transaction(async (tx) => {
      ...
      if (!projetoId) throw new AppError("Não foi possível criar o projeto.", 500);
  ...
  async inativar(id: bigint, usuarioId: bigint | undefined, usuarioNome: string | undefined, tenantId: string) {
    await this.garantirEstrutura();
  ...
  async listarHistorico(projetoId: bigint, tenantId: string) {
    await this.garantirEstrutura();
  ...
  async criarTarefa(
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (!tarefaId) throw new AppError("Não foi possível criar a tarefa do projeto.", 500);
  ...
  private async listarTarefasPorTenant(tenantId: string) {
    return prisma.$queryRaw<ProjetoTarefaRow[]>(Prisma.sql`
      SELECT
        id,
  ...
  private async listarHistoricoPorTenant(tenantId: string) {
    return prisma.$queryRaw<ProjetoHistoricoRow[]>(Prisma.sql`
      SELECT
        id,
        tenant_id::text AS tenant_id,
        projeto_id,
  ...
  private async listarTarefasPorProjeto(projetoId: bigint, tenantId: string) {
    return prisma.$queryRaw<ProjetoTarefaRow[]>(Prisma.sql`
      SELECT
        id,
        tenant_id::text AS tenant_id,
  ...
  private async listarHistoricoProjeto(projetoId: bigint, tenantId: string) {
    return prisma.$queryRaw<ProjetoHistoricoRow[]>(Prisma.sql`
      SELECT
        id,
        tenant_id::text AS tenant_id,
  ...
  private async buscarTarefaPorId(projetoId: bigint, tarefaId: bigint, tenantId: string) {
    await this.garantirEstrutura();
    const row = await prisma.$queryRaw<ProjetoTarefaRow[]>(Prisma.sql`
      SELECT
        id,
        tenant_id::text AS tenant_id,
        projeto_id,
    ...
    `);
    const tarefa = row[0];
    if (!tarefa) throw new AppError("Tarefa do projeto não encontrada.", 404);
    return tarefa;
  }

  private async buscarTarefaPorIdTx(tx: TransactionClient, projetoId: bigint, tarefaId: bigint, tenantId: string) {
    const row = await tx.$queryRaw<ProjetoTarefaRow[]>(Prisma.sql`
      SELECT
        id,
        tenant_id::text AS tenant_id,
        projeto_id,
  ...
  private async touchProjetoTx(tx: TransactionClient, projetoId: bigint, usuarioId: bigint | undefined, tenantId: string) {
    await tx.$executeRaw(Prisma.sql`
      UPDATE projetos
      SET updated_at = NOW(),
          updated_by = ${usuarioId ?? null}
  ...
  private async inserirHistoricoTx(
  ...
  private async validarUnidadeAssistencial(unidadeId: string | undefined, tenantId: string) {
  ...
  private buildWhere(filters: ProjetoFilters, tenantId: string, alias: string) {
    ...
    if (filters.status) conditions.push(`${alias}.status = '${escapeSqlValue(filters.status)}'`);
    if (filters.prioridade) conditions.push(`${alias}.prioridade = '${escapeSqlValue(filters.prioridade)}'`);
    if (filters.area_projeto) conditions.push(`${alias}.area_projeto = '${escapeSqlValue(filters.area_projeto)}'`);
    if (filters.data_inicio_de) conditions.push(`${alias}.data_inicio >= DATE '${escapeSqlValue(filters.data_inicio_de)}'`);
    if (filters.data_inicio_ate) conditions.push(`${alias}.data_inicio <= DATE '${escapeSqlValue(filters.data_inicio_ate)}'`);
    if (filters.prazo_de) conditions.push(`${alias}.prazo_previsto >= DATE '${escapeSqlValue(filters.prazo_de)}'`);
    if (filters.prazo_ate) conditions.push(`${alias}.prazo_previsto <= DATE '${escapeSqlValue(filters.prazo_ate)}'`);
