...
async function main() {
  ...
  const matriculas = [
    ...
    await ensureMatricula({ aluno_id: alunos[5], ano_letivo_id: anos.atual, unidade_id: unidades.infantil, sala_id: salas.infantilA, etapa_id: etapas.infantil, serie_id: series.grupo4, turma_id: turmas.infantil, data_matricula: "2026-02-02", data_inicio: "2026-02-02", turno: "INTEGRAL", origem: "NOVO", transporte_escolar: false, documentacao: { identificacao: true, comprovante_residencia: true }, situacao: "ATIVA", ativo: true }, "MAT-DEMO-0006"),
    await ensureMatricula({ aluno_id: alunos[6], ano_letivo_id: anos.atual, unidade_id: unidades.infantil, sala_id: salas.infantilA, etapa_id: etapas.infantil, serie_id: series.grupo4, turma_id: turmas.infantil, data_matricula: "2026-02-02", data_inicio: "2026-02-02", turno: "INTEGRAL", origem: "TRANSFERENCIA", transporte_escolar: false, documentacao: { identificacao: true, comprovante_residencia: false }, situacao: "ATIVA", ativo: true }, "MAT-DEMO-0007"),
    await ensureMatricula({ aluno_id: alunos[7], ano_letivo_id: anos.atual, unidade_id: unidades.infantil, sala_id: salas.infantilA, etapa_id: etapas.infantil, serie_id: series.grupo4, turma_id: turmas.infantil, data_matricula: "2026-02-02", data_inicio: "2026-02-02", turno: "INTEGRAL", origem: "NOVO", transporte_escolar: false, documentacao: { identificacao: true, comprovante_residencia: true }, situacao: "ATIVA", ativo: true }, "MAT-DEMO-0008")
  ];
  ...
  const vinculos = [
    [professores.professoraAna, turmas.quinto, disciplinas.matematica, unidades.centro],
    [professores.professorBruno, turmas.oitavo, disciplinas.lingua, unidades.centro],
    [professores.professoraClara, turmas.infantil, disciplinas.experiencias, unidades.infantil]
  ];
  for (const [profissionalId, turmaId, disciplinaId, unidadeId] of vinculos) {
    ...
    if (!existente) await prisma.$queryRawUnsafe(
      ...
      TENANT_ID, profissionalId, unidadeId, turmaId, disciplinaId
    );
  }
  ...
  const avaliacaoOitavoId = await ensureAcademic("avaliacoes", { turma_id: turmas.oitavo, disciplina_id: disciplinas.lingua, periodo: "1º bimestre", tipo: "Produção textual", data_avaliacao: "2026-04-11", valor_maximo: 10, peso: 1, status: "ENCERRADA" }, "SELECT id FROM educacional_avaliacao WHERE tenant_id::text = $1 AND turma_id = $2 AND disciplina_id = $3 AND periodo = $4 LIMIT 1", TENANT_ID, turmas.oitavo, disciplinas.lingua, "1º bimestre");
  await ensureAcademic("notas", { avaliacao_id: avaliacaoOitavoId, matricula_id: matriculas[2], valor: 7.8, conceito: null, observacao: "Nota fictícia de demonstração" }, "SELECT id FROM educacional_nota WHERE tenant_id::text = $1 AND avaliacao_id = $2 AND matricula_id = $3 LIMIT 1", TENANT_ID, avaliacaoOitavoId, matriculas[2]);

  const avaliacaoInfantilId = await ensureAcademic("avaliacoes", { turma_id: turmas.infantil, disciplina_id: disciplinas.experiencias, periodo: "1º trimestre", tipo: "Registro descritivo", data_avaliacao: "2026-04-12", valor_maximo: 10, peso: 1, status: "ENCERRADA" }, "SELECT id FROM educacional_avaliacao WHERE tenant_id::text = $1 AND turma_id = $2 AND disciplina_id = $3 AND periodo = $4 LIMIT 1", TENANT_ID, turmas.infantil, disciplinas.experiencias, "1º trimestre");
  const matriculasInfantis = await prisma.$queryRawUnsafe<Array<{ id: bigint }>>("SELECT id FROM educacional_matricula WHERE tenant_id::text = $1 AND turma_id = $2 AND situacao = 'ATIVA' ORDER BY id", TENANT_ID, turmas.infantil);
  for (const matricula of matriculasInfantis) await ensureAcademic("notas", { avaliacao_id: avaliacaoInfantilId, matricula_id: Number(matricula.id), valor: 9, conceito: "Em desenvolvimento", observacao: "Registro fictício de demonstração da educação infantil." }, "SELECT id FROM educacional_nota WHERE tenant_id::text = $1 AND avaliacao_id = $2 AND matricula_id = $3 LIMIT 1", TENANT_ID, avaliacaoInfantilId, Number(matricula.id));
  ...
  for (const [indice, matriculaId] of matriculas.entries()) {
    await repo.gerarBoletimAutomatico({ matricula_id: matriculaId, ano_letivo_id: anos.atual, periodo: indice >= 3 ? "1º trimestre" : "1º bimestre" }, TENANT_ID, actor);
  }
  for (const alunoId of alunos) {
    await repo.gerarHistoricoAutomatico({ aluno_id: alunoId, ano_letivo_id: anos.atual }, TENANT_ID, actor);
  }
