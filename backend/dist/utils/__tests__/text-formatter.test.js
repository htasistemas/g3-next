...
import { formatarEndereco, formatarNomeInstituicao, formatarNomePessoa, formatarTextoCurto, formatarTextoPorCampo, normalizarEspacos } from "../text-formatter.js";
test("formatar nome de pessoa com conectivos e espacos extras", () => {
    assert.equal(formatarNomePessoa("  ADRIANO   DA   SILVA  OLIVEIRA "), "Adriano da Silva Oliveira");
...
test("formatar endereco e instituicao", () => {
    ...
    assert.equal(formatarNomeInstituicao("SECRETARIA DE ASSISTENCIA SOCIAL"), "Secretaria de Assistencia Social");
    assert.equal(formatarNomeInstituicao("CENTRO DE REFERENCIA DO CRAS"), "Centro de Referencia do CRAS");
});
...
test("respeitar campos tecnicos sem alterar capitalizacao", () => {
    ...
    assert.equal(formatarTextoPorCampo("email", "email@example.com", mapaTeste), "email@example.com");
