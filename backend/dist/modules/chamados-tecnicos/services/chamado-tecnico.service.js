...
export class ChamadoTecnicoService {
    repository = new ChamadoTecnicoRepository();
    ...
    async removerVinculo(rawId, rawVinculoId, authUser) {
        const id = this.parseId(rawId);
    ...
    mapChamadoDetalhe(item, contexto) {
        return {
    ...
    usuarioPodeDesenvolver(authUser) {
    ...
    usuarioPodeEncerrarChamado(authUser, chamado) {
        if (!authUser?.id)
            return false;
        return toStringId(chamado.criador_usuario_id) === authUser.id;
    }
    async notificarNovoChamado(chamado, authUser) {
    ...
    async gerarPdfExportacao(chamados) {
