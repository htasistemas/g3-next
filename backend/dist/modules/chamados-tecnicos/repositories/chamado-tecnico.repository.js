...
let parametrosIniciaisPromise = null;
...
function buildWhereClauses(filters, tenantId) {
    const clauses = [
        Prisma.sql `c.ativo = TRUE`,
        Prisma.sql `c.tenant_id::text = ${tenantId}`
    ];
    if (filters.codigo)
        clauses.push(Prisma.sql `c.codigo ILIKE ${`%${filters.codigo}%`}`);
    if (filters.resumo)
        clauses.push(Prisma.sql `c.resumo ILIKE ${`%${filters.resumo}%`}`);
    if (filters.situacao_id)
        clauses.push(Prisma.sql `c.situacao_id = ${BigInt(filters.situacao_id)}`);
    if (filters.tipo_id)
        clauses.push(Prisma.sql `c.tipo_id = ${BigInt(filters.tipo_id)}`);
    if (filters.prioridade_id)
        clauses.push(Prisma.sql `c.prioridade_id = ${BigInt(filters.prioridade_id)}`);
    if (filters.categoria_id)
        clauses.push(Prisma.sql `c.categoria_id = ${BigInt(filters.categoria_id)}`);
    if (filters.sistema_id)
        clauses.push(Prisma.sql `c.sistema_id = ${BigInt(filters.sistema_id)}`);
    if (filters.projeto_id)
        clauses.push(Prisma.sql `c.projeto_id = ${BigInt(filters.projeto_id)}`);
    if (filters.sprint_id)
        clauses.push(Prisma.sql `c.sprint_id = ${BigInt(filters.sprint_id)}`);
    if (filters.criador_usuario_id)
        clauses.push(Prisma.sql `c.criador_usuario_id = ${BigInt(filters.criador_usuario_id)}`);
    if (filters.responsavel_usuario_id)
        clauses.push(Prisma.sql `c.responsavel_usuario_id = ${BigInt(filters.responsavel_usuario_id)}`);
    if (filters.cliente)
        clauses.push(Prisma.sql `COALESCE(c.cliente, '') ILIKE ${`%${filters.cliente}%`}`);
    if (filters.solicitante)
        clauses.push(Prisma.sql `c.solicitante ILIKE ${`%${filters.solicitante}%`}`);
    if (filters.resolucao)
        clauses.push(Prisma.sql `COALESCE(c.resolucao, '') ILIKE ${`%${filters.resolucao}%`}`);
    ...
    if (dataInicio)
        clauses.push(Prisma.sql `c.data_criacao >= ${dataInicio}`);
    const dataFim = parseDate(filters.data_criacao_fim);
    if (dataFim)
        clauses.push(Prisma.sql `c.data_criacao < ${new Date(dataFim.getTime() + 86400000)}`);
    if (filters.inatividade_dias)
        clauses.push(Prisma.sql `c.ultima_atualizacao < NOW() - (${filters.inatividade_dias} * INTERVAL '1 day')`);
...
function buildOrderBy(ordenacao, direcao = "desc") {
...
export class ChamadoTecnicoRepository {
    ...
    async buscarParametroPorId(id, _tenantId) {
        await this.garantirEstrutura();
        await this.garantirParametrosIniciais();
    ...
    async buscarParametroPorChave(tipo, chave, _tenantId) {
        await this.garantirEstrutura();
        await this.garantirParametrosIniciais();
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT id, tipo, chave, nome, descricao, cor, ordem, padrao, sla_horas, ativo, metadados_json, criado_em, atualizado_em
    ...
    async listarUsuariosCatalogo(tenantId) {
        await this.garantirEstrutura();
        return prisma.$queryRaw(Prisma.sql `
    ...
    async salvarParametro(input, _tenantId, id) {
        await this.garantirEstrutura();
        if (input.padrao) {
      ...
      `);
            ...
            if (!atualizado)
                throw new AppError("Parametro nao encontrado.", 404);
            return atualizado;
        }
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        ...
        if (!parametro)
            throw new AppError("Nao foi possivel criar o parametro.", 500);
        return parametro;
    }
    async listar(filters, usuarioId, tenantId) {
        await this.garantirEstrutura();
    ...
    `);
      ...
      ${whereSql}
    `);
        const cards = await prisma.$queryRaw(Prisma.sql `
      SELECT
    ...
    async listarExportacao(filters, usuarioId, tenantId) {
        await this.garantirEstrutura();
        const clauses = buildWhereClauses(filters, tenantId);
        const whereSql = Prisma.sql `WHERE ${Prisma.join(clauses, " AND ")}`;
        const orderBySql = buildOrderBy(filters.ordenacao, filters.direcao);
    ...
    async criar(input, criadorUsuarioId, situacaoId, tenantId) {
        await this.garantirEstrutura();
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        const id = rows[0]?.id;
        if (!id)
            throw new AppError("Nao foi possivel criar o chamado tecnico.", 500);
        await prisma.$executeRaw(Prisma.sql `
    ...
    async atualizar(id, input, tenantId, situacaoId) {
    ...
    async registrarResolucao(id, tenantId, situacaoId, options) {
        await this.buscarPorIdOuFalhar(id, tenantId);
        await prisma.$executeRaw(Prisma.sql `
      UPDATE g3n_chamado_tecnico
      SET
    ...
    async salvarComentario(chamadoId, tenantId, input, usuarioId) {
    ...
    `);
        ...
        if (!comentario)
            throw new AppError("Nao foi possivel registrar o comentario.", 500);
        return comentario;
    }
    async listarComentarios(chamadoId, tenantId) {
        await this.buscarPorIdOuFalhar(chamadoId, tenantId);
        return prisma.$queryRaw(Prisma.sql `
      SELECT *
    ...
    async registrarHistorico(chamadoId, payload) {
    ...
    async listarHistorico(chamadoId, tenantId) {
        await this.buscarPorIdOuFalhar(chamadoId, tenantId);
        return prisma.$queryRaw(Prisma.sql `
      SELECT *
    ...
    async salvarVinculo(chamadoId, tenantId, input, usuarioId) {
        await this.buscarPorIdOuFalhar(chamadoId, tenantId);
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        ...
        if (!vinculo)
            throw new AppError("Nao foi possivel salvar o vinculo do chamado.", 500);
        return vinculo;
    }
    async listarVinculos(chamadoId, tenantId) {
        await this.buscarPorIdOuFalhar(chamadoId, tenantId);
        return prisma.$queryRaw(Prisma.sql `
      SELECT *
    ...
    async removerVinculo(chamadoId, tenantId, vinculoId) {
        await this.buscarPorIdOuFalhar(chamadoId, tenantId);
        await prisma.$executeRaw(Prisma.sql `
    ...
    async listarAnexos(chamadoId, tenantId) {
        await this.buscarPorIdOuFalhar(chamadoId, tenantId);
        return prisma.$queryRaw(Prisma.sql `
    ...
    async buscarAnexoPorId(chamadoId, tenantId, arquivoId) {
        await this.buscarPorIdOuFalhar(chamadoId, tenantId);
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async listarFiltrosSalvos(usuarioId, tenantId) {
        await this.garantirEstrutura();
        return prisma.$queryRaw(Prisma.sql `
      SELECT *
    ...
    async salvarFiltroSalvo(usuarioId, tenantId, input, id) {
        await this.garantirEstrutura();
        if (input.padrao) {
            const filtroExclusaoId = id
                ? Prisma.sql `
    ...
    async removerFiltroSalvo(usuarioId, tenantId, id) {
        await this.garantirEstrutura();
        await prisma.$executeRaw(Prisma.sql `
    ...
    async marcarAcesso(chamadoId, usuarioId, tenantId) {
        await this.buscarPorIdOuFalhar(chamadoId, tenantId);
        await prisma.$executeRaw(Prisma.sql `
    ...
    async marcarComentariosComoLidos(chamadoId, usuarioId, tenantId) {
