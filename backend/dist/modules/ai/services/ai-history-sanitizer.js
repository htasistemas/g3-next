...
function repetirMascara(tamanho) {
...
export function sanitizeAiHistoryText(texto) {
    return texto
...
export function sanitizeAiHistoryValue(valor) {
    if (typeof valor === "string") {
        return sanitizeAiHistoryText(valor);
    }
    if (Array.isArray(valor)) {
        return valor.map((item) => sanitizeAiHistoryValue(item));
    }
    if (valor && typeof valor === "object" && !(valor instanceof Date)) {
        return Object.fromEntries(Object.entries(valor).map(([chave, item]) => [chave, sanitizeAiHistoryValue(item)]));
    }
    return valor;
}
