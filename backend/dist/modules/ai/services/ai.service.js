...
function itemMatchesContext(item, contextText) {
    if (!contextText)
        return false;
    return (item.contextos ?? []).some((contexto) => contextText.includes(normalizeQuery(contexto)));
}
function findKnowledgeAnswer(normalizedQuery) {
    return KNOWLEDGE_BASE.find((item) => item.terms.some((term) => normalizedQuery.includes(normalizeQuery(term))));
}
...
const AI_SUGGESTION_CATEGORIES = [
...
export class AiService {
    ...
    async processQuery(query, userId, context, userName) {
        const ctx = {
            query,
    ...
    getConversationalReply(query, normalizedQuery, userName) {
        const firstName = extractFirstName(userName);
        const saudacaoPersonalizada = firstName
            ? `Olá, ${firstName}.`
            : "Olá.";
        let answer = `${saudacaoPersonalizada} Estou bem e posso te ajudar.\n\n` +
    ...
    getIaUnavailable(query, context) {
    ...
    getFallbackNotUnderstood(query, context) {
