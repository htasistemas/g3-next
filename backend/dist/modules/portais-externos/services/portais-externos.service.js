...
function filtroTenant(alias, tenantId) {
    return tenantId
        ? Prisma.sql `AND ${Prisma.raw(alias)}.tenant_id::text = ${tenantId}`
        : Prisma.empty;
}
function montarToken(tipo, id, tenantId) {
...
export class PortaisExternosService {
    async obterLogoInstituicao(rawSlug) {
    ...
    `;
        ...
        if (!logoUrl)
            throw new AppError("Logomarca não cadastrada para esta instituição.", 404);
    ...
    async acessar(tipo, input) {
        ...
        if (tipo === "voluntario")
            return this.acessarVoluntario(identificador);
        if (tipo === "beneficiario")
            return this.acessarBeneficiarioFamilia(identificador, senha, normalizarTexto(input.tenantId) || undefined);
        if (tipo === "parceiro")
            return this.acessarParceiro(identificador);
        throw new AppError("Portal externo nao reconhecido.", 404);
    }
    async obterTransparencia(rawTenantId, rawSlug) {
    ...
    async acessarVoluntario(identificador) {
    ...
    `);
        ...
        if (!voluntario)
            throw new AppError("Voluntario nao encontrado para os dados informados.", 404);
        return {
            tipo: "voluntario",
    ...
    async acessarBeneficiarioFamilia(identificador, senha, tenantSelecionado) {
        ...
        const candidatos = Array.from(new Map(registros
            .filter((item) => item.tenant_id && (!tenantSelecionado || item.tenant_id === tenantSelecionado))
            .map((item) => [item.tenant_id, item])).values());
        const validos = [];
        for (const candidato of candidatos) {
            if (candidato.senha_hash && await bcrypt.compare(senha, candidato.senha_hash))
        ...
        const visaoGeral = await centralAtendimentosRepository.obterVisaoGeral(beneficiario.beneficiario_id, tenantId);
        ...
        return {
            ...
            indicadores: [
                ...
                { label: "Pendências", valor: formatarValor((visaoGeral.alertas?.length ?? 0) + documentosPendentes.length + visaoGeral.encaminhamentos.filter((item) => String(item.status ?? "").toLowerCase() !== "concluído").length) },
            ...
            cards: [
                ...
                { titulo: "Pendências importantes", texto: `${formatarValor(documentosPendentes.length)} documento(s) pendente(s) e ${formatarValor(visaoGeral.encaminhamentos.filter((item) => String(item.status ?? "").toLowerCase() !== "concluído").length)} encaminhamento(s) em acompanhamento.` }
            ],
            linhaDoTempo: [
            ...
            grupoFamiliar: visaoGeral.grupoFamiliar,
            cestasPendentes,
            faltasCursos,
            movimentacoes,
            itens: []
    ...
    async listarCestasPendentesPortal(beneficiarioId, familiaId, tenantId) {
        try {
            const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async listarFaltasCursosPortal(cpf, nomeCompleto, tenantId) {
        try {
    ...
    async listarAgendamentosPortal(beneficiarioId, familiaId, tenantId) {
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT
        a.id,
        a.data_agendamento,
    ...
    async listarDocumentosPendentesPortal(beneficiarioId, tenantId) {
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT
        d.id,
        d.nome_documento,
    ...
    async acessarParceiro(identificador) {
    ...
    `);
        if (!projetos.length)
            throw new AppError("Nenhum projeto de parceiro encontrado para os dados informados.", 404);
