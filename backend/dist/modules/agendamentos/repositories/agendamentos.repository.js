...
function formatarHoraExibicao(value) {
    if (!value)
        return "---";
...
function formatarHoraEntrada(value) {
    if (!value)
        return undefined;
    if (value instanceof Date) {
        const horas = String(value.getUTCHours()).padStart(2, "0");
...
function chaveParticipantePorNome(nome) {
    return `nome:${normalizarTextoComparacao(nome)}`;
}
function normalizarTextoSql(campo) {
    return Prisma.sql `
    LOWER(
      TRANSLATE(
...
function formatarHoraMensagem(value) {
    if (!value)
        return "";
    if (value instanceof Date) {
        const horas = String(value.getUTCHours()).padStart(2, "0");
...
function sqlTelefoneLimpo(campo) {
    return Prisma.sql `
    NULLIF(
...
function extrairEmailContato(contato) {
    if (!contato)
        return undefined;
...
function lerInteiroParticipante(value) {
...
function lerTextoParticipante(value) {
    if (typeof value !== "string")
        return null;
    const normalizado = value.trim();
...
export class AgendamentosRepository {
    ...
    async registrarLog(agendamentoId, acao, usuario, tenantId, anterior, novo, db = prisma) {
    ...
    async registrarHistoricoFamilia(familiaId, descricao, dadosNovos, tenantId, db = prisma) {
        if (!familiaId)
            return;
        await db.$executeRaw(Prisma.sql `
    ...
    async verificarDuplicidadeAgenda(payload, tenantId, idIgnorar, db = prisma) {
        const criterios = [
    ...
    async buscarAgendaOperacionalExistente(payload, tenantId, agruparPorData = false) {
    ...
    chaveParticipante(nome, beneficiarioId) {
        if (beneficiarioId) {
            return `id:${beneficiarioId.toString()}`;
        }
        return chaveParticipantePorNome(nome);
    }
    async listarBeneficiariosAgendamentoComContato(agendamentoIds, tenantId) {
    ...
    async listarContatosPorParticipantes(participantes, tenantId) {
        const ids = Array.from(new Set(participantes
            .map((participante) => participante.beneficiarioId)
            .filter((valor) => Number.isInteger(valor) && Number(valor) > 0)));
        const nomes = Array.from(new Set(participantes
    ...
    async listarContatosPorNomes(nomesEntrada, tenantId) {
        const nomes = Array.from(new Set(nomesEntrada
    ...
    async listarContatosPorMatriculas(matriculaIds, tenantId) {
        const ids = Array.from(new Set(matriculaIds.filter((valor) => Number.isInteger(valor) && Number(valor) > 0)));
        if (!ids.length) {
            return [];
        }
        return prisma.$queryRaw(Prisma.sql `
    ...
    async listarContatosPorItensOperacionais(itemIds, tenantId) {
    ...
    async hidratarParticipantesAgendamentos(rows, tenantId) {
        if (!rows.length) {
            return rows;
        }
        const contatos = await this.listarBeneficiariosAgendamentoComContato(rows.map((row) => row.id), tenantId);
        const contatosPorAgendamento = new Map();
        ...
        const contatosFallback = await this.listarContatosPorParticipantes(rows.flatMap((row) => Array.isArray(row.participantes)
            ? row.participantes.map((participante) => ({
            ...
            : []), tenantId);
        const contatosPorNomeExato = await this.listarContatosPorNomes(rows.flatMap((row) => Array.isArray(row.participantes)
            ? row.participantes
                .map((participante) => lerTextoParticipante(participante.beneficiarioNome ?? participante.beneficiario_nome))
                .filter((valor) => typeof valor === "string" && valor.trim().length > 0)
            : []), tenantId);
        const contatosPorMatricula = await this.listarContatosPorMatriculas(rows.flatMap((row) => Array.isArray(row.participantes)
            ? row.participantes
                .map((participante) => lerInteiroParticipante(participante.matriculaId ?? participante.matricula_id))
                .filter((valor) => Number.isInteger(valor) && Number(valor) > 0)
            : []), tenantId);
        const contatosItensOperacionais = await this.listarContatosPorItensOperacionais(rows
            .map((row) => (row.item_origem_id ? Number(row.item_origem_id) : null))
            .filter((valor) => Number.isInteger(valor) && Number(valor) > 0), tenantId);
        ...
        for (const contato of contatosItensOperacionais) {
            ...
            contatosItensPorChave.set(`item:${itemOrigemId}:participante:${this.chaveParticipante(contato.beneficiario_nome, contato.beneficiario_id)}`, {
                beneficiario_nome: contato.beneficiario_nome,
                codigo: contato.codigo,
                data_nascimento: contato.data_nascimento,
                telefone: contato.telefone,
        ...
        for (const contato of contatosFallback) {
            contatosFallbackPorChave.set(this.chaveParticipante(contato.beneficiario_nome, contato.beneficiario_id), contato);
        ...
        return rows.map((row) => {
            ...
            for (const contato of contatosDoAgendamento) {
                contatosPorChave.set(this.chaveParticipante(contato.beneficiario_nome, contato.beneficiario_id), contato);
            }
            const participantesOriginais = Array.isArray(row.participantes)
                ? row.participantes
                : [];
            const participantes = participantesOriginais.length > 0
                ? participantesOriginais.map((participante) => {
                    ...
                    const contatoItemBruto = itemOrigemId && matriculaId
                        ...
                        : itemOrigemId
                            ...
                            : undefined;
                    const contatoItem = contatoItemBruto &&
                    ...
                    const telefoneParticipante = typeof participante.telefone === "string" && normalizeDigits(participante.telefone)
                        ? participante.telefone
                        : undefined;
                    const emailParticipante = typeof participante.email === "string" && participante.email.trim().length
                        ? participante.email
                        : undefined;
                    const dataNascimentoParticipante = typeof participante.dataNascimento === "string" && participante.dataNascimento.trim().length
                    ...
                    return {
                        ...participante,
                        dataNascimento: formatarDataEntrada(contatoNomeExato?.data_nascimento) ??
                        ...
                        telefone: extrairTelefoneContato(contatoTelefone) ??
                        ...
                        codigo: (typeof contatoNomeExato?.codigo === "string" && contatoNomeExato.codigo.trim().length
                            ...
                            (typeof contatoMatricula?.codigo === "string" && contatoMatricula.codigo.trim().length
                                ...
                                : typeof contatoCodigo?.codigo === "string" && contatoCodigo.codigo.trim().length
                                    ...
                                    : undefined) ?? undefined,
                        email: typeof contatoNomeExato?.email === "string" && contatoNomeExato.email.trim().length
            ...
            const contatoPrincipal = contatosDoAgendamento.find((contato) => contato.beneficiario_id && row.beneficiario_id && contato.beneficiario_id === row.beneficiario_id) ??
    ...
    async sincronizarBeneficiariosAgendamento(agendamentoId, tenantId, participantes, db = prisma) {
        await db.$executeRaw(Prisma.sql `
    ...
    `);
        ...
        const valores = participantes.map((participante) => Prisma.sql `(
        ${tenantId}::uuid,
        ${agendamentoId},
    ...
    async listarItensOperacionais(tipo, busca, tenantId) {
    ...
    async obterItemOperacional(itemId, tenantId) {
        await this.ensureEstrutura();
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT
        c.id,
    ...
    async listarBeneficiariosOperacionais(itemId, tenantId, matriculasIds) {
        await this.ensureEstrutura();
    ...
    async listarConflitos(payload, db = prisma) {
    ...
    formatarMensagemConflito(conflitos) {
    ...
    async criar(input, usuario, tenantId) {
        await this.ensureEstrutura();
    ...
    async atualizar(id, input, usuario, tenantId) {
        await this.ensureEstrutura();
    ...
    async copiar(id, dataDestino, usuario, tenantId) {
        await this.ensureEstrutura();
        ...
        const payload = {
            ...
            equipeApoio: Array.isArray(origem.equipe_apoio) ? origem.equipe_apoio : undefined,
            data: novaData,
            ...
            recorrencia: origem.recorrencia ? origem.recorrencia : undefined,
            ...
            itemTipo: (origem.item_tipo ?? undefined),
        ...
        INSERT INTO agendamento (
          ...
          ${trimOrUndefined(payload.inscricaoOrigemId) ? BigInt(trimOrUndefined(payload.inscricaoOrigemId)) : null},
      ...
      `);
            ...
            if (!idNovo)
                throw new AppError("Nao foi possivel copiar a agenda.", 500);
    ...
    async cancelar(id, motivo, usuario, tenantId) {
        const anterior = await this.obter(id, tenantId);
        if (!anterior)
            throw new AppError("Agendamento nao encontrado.", 404);
    ...
    async excluir(id, usuario, tenantId) {
        const anterior = await this.obter(id, tenantId);
        if (!anterior)
            throw new AppError("Agendamento nao encontrado.", 404);
        await prisma.$executeRaw(Prisma.sql `
    ...
    async remarcar(id, input, usuario, tenantId) {
        const anterior = await this.obter(id, tenantId);
        if (!anterior)
            throw new AppError("Agendamento nao encontrado.", 404);
        const conflitos = await this.listarConflitos({
    ...
    async confirmar(id, canal, observacao, usuario, tenantId) {
        const anterior = await this.obter(id, tenantId);
        if (!anterior)
            throw new AppError("Agendamento nao encontrado.", 404);
        await prisma.$executeRaw(Prisma.sql `
    ...
    async checkIn(id, input, usuario, tenantId) {
        const anterior = await this.obter(id, tenantId);
        if (!anterior)
            throw new AppError("Agendamento nao encontrado.", 404);
    ...
    async inserirCentralAtendimentoSeNecessario(agendamentoId, payload, usuario, tenantId) {
    ...
    async concluir(id, input, usuario, tenantId) {
        const anterior = await this.obter(id, tenantId);
        if (!anterior)
            throw new AppError("Agendamento nao encontrado.", 404);
    ...
    async listarListaEspera(tenantId) {
        await this.ensureEstrutura();
        return prisma.$queryRaw(Prisma.sql `
      SELECT *
    ...
    async criarListaEspera(input, tenantId) {
        await this.ensureEstrutura();
        const inserted = await prisma.$queryRaw(Prisma.sql `
    ...
    async converterListaEspera(id, input, usuario, tenantId) {
    ...
    async montarPayloadOperacional(input, tenantId) {
        ...
        if (profissionalSelecionado &&
            ...
            !profissionaisRelacionados.some((nome) => nome.toLocaleLowerCase("pt-BR") === profissionalSelecionado.toLocaleLowerCase("pt-BR"))) {
            throw new AppError("O profissional selecionado nao esta relacionado ao item.", 400);
        }
        const profissionalNome = profissionalSelecionado ?? profissionaisRelacionados[0] ?? trimOrUndefined(selecionados[0]?.profissional_nome) ?? "Equipe institucional";
    ...
    async criarOperacional(input, usuario, tenantId) {
        ...
        if (agendaExistenteId) {
            if (atendimentoPorHorario) {
                ...
                const participantesPorMatricula = new Map();
                for (const participante of participantesExistentes) {
                    const matriculaId = lerInteiroParticipante(participante.matriculaId ?? participante.matricula_id);
                    if (matriculaId)
                        participantesPorMatricula.set(String(matriculaId), participante);
                }
                ...
                return this.atualizar(agendaExistenteId, {
                    ...payload,
                    capacidadeMaxima: participantes.length,
    ...
    async atualizarOperacional(id, input, usuario, tenantId) {
        const payload = await this.montarPayloadOperacional(input, tenantId);
        return this.atualizar(id, payload, usuario, tenantId);
    }
    async notificar(id, canal, usuario, tenantId) {
        await this.ensureEstrutura();
        ...
        const contatosMatriculas = participantesAgendamento.length
            ? await this.listarContatosPorMatriculas(participantesAgendamento
                .map((participante) => lerInteiroParticipante(participante.matriculaId ?? participante.matricula_id))
                .filter((valor) => Number.isInteger(valor) && Number(valor) > 0), tenantId)
            : [];
        ...
        const fallbackParticipantes = !beneficiariosTabela.length && participantesAgendamento.length
            ? await this.listarContatosPorParticipantes(participantesAgendamento.map((participante) => ({
                beneficiarioId: lerInteiroParticipante(participante.beneficiarioId ?? participante.beneficiario_id),
                beneficiarioNome: lerTextoParticipante(participante.beneficiarioNome ?? participante.beneficiario_nome)
            })), tenantId)
            : [];
        ...
        for (const contato of fallbackParticipantes) {
            fallbackPorChave.set(this.chaveParticipante(contato.beneficiario_nome, contato.beneficiario_id), { telefone: contato.telefone, email: contato.email });
        }
        const beneficiarios = beneficiariosTabela.length
            ? beneficiariosTabela.map((beneficiario) => {
                ...
                const participante = participantesAgendamento.find((item) => {
                    ...
                    return this.chaveParticipante(lerTextoParticipante(item.beneficiarioNome ?? item.beneficiario_nome), (() => {
            ...
            : participantesAgendamento.map((participante, index) => {
                ...
                return {
                    ...
                    telefone: (typeof contatoMatricula?.telefone_principal === "string" && normalizeDigits(contatoMatricula.telefone_principal)
                        ? contatoMatricula.telefone_principal
                        : typeof fallback?.telefone === "string" && normalizeDigits(fallback.telefone)
                            ...
                            : typeof participante.telefone === "string" && normalizeDigits(participante.telefone)
                                ? participante.telefone
                                : null),
                    email: (typeof contatoMatricula?.email === "string" && contatoMatricula.email.trim().length
                        ? contatoMatricula.email
        ...
        `);
                ...
                const destinatarioFalho = canal === "EMAIL"
    ...
    async indicadores(filtros, tenantId) {
