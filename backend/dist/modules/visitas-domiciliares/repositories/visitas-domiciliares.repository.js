...
export class VisitasDomiciliaresRepository {
    ...
    async criar(input) {
        ...
        const valores = [
            ...
            Prisma.sql `${(input.endereco ?? {})}`,
            Prisma.sql `${input.observacoesIniciais ?? null}`,
            Prisma.sql `${(input.condicoes ?? {})}`,
            Prisma.sql `${(input.situacaoSocial ?? {})}`,
            Prisma.sql `${(input.registro ?? {})}`,
            ...(schema.possuiAnexos
                ? [Prisma.sql `${(input.anexos ?? [])}`]
                : []),
            Prisma.sql `NOW()`,
            Prisma.sql `NOW()`
        ];
    ...
    async atualizar(id, input) {
        ...
        const atribuicoes = [
            ...
            Prisma.sql `endereco = ${(input.endereco ?? {})}`,
            Prisma.sql `observacoes_iniciais = ${input.observacoesIniciais ?? null}`,
            Prisma.sql `condicoes = ${(input.condicoes ?? {})}`,
            Prisma.sql `situacao_social = ${(input.situacaoSocial ?? {})}`,
            Prisma.sql `registro = ${(input.registro ?? {})}`,
            ...(schema.possuiAnexos
                ? [Prisma.sql `anexos = ${(input.anexos ?? [])}`]
                : []),
