...
function mapRow(row) {
    ...
    for (const [key, value] of Object.entries(row)) {
        ...
        if (typeof value === "bigint")
            mapped[camelKey] = value.toString();
...
export class PrestacaoContasProfissionalService {
    ...
    async visaoGeral(actor) {
        const tenantId = this.parseTenant(actor);
        const [instrumentos, receitas, despesas, metas, documentos, conciliacoes, diligencias, aprovacoes] = await Promise.all([
    ...
    async listar(entidade, actor) {
        const tenantId = this.parseTenant(actor);
        const table = this.table(entidade);
        const rows = await prisma.$queryRawUnsafe(`SELECT * FROM ${table} WHERE tenant_id::text = $1 AND excluido_em IS NULL ORDER BY id DESC LIMIT 200`, tenantId);
        return rows.map(mapRow);
    }
    async criar(entidade, rawInput, actor, ip) {
        const tenantId = this.parseTenant(actor);
        switch (entidade) {
            case "concedentes":
    ...
    async obterConfiguracoesIa(actor) {
        const tenantId = this.parseTenant(actor);
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async salvarConfiguracaoIa(rawInput, actor, ip) {
        const tenantId = this.parseTenant(actor);
    ...
    async testarConfiguracaoIa(rawInput, actor, ip) {
        const tenantId = this.parseTenant(actor);
    ...
    async analisarDocumento(rawInput, actor, ip) {
        const tenantId = this.parseTenant(actor);
    ...
    async assistente(rawInput, actor, ip) {
        const tenantId = this.parseTenant(actor);
        ...
        return {
            ...
            resposta: "Sugestão gerada sem consultar provedor externo nesta etapa. Revise os dados reais do G3N, confira documentos e registre a validação humana antes de usar em relatório, diligência ou parecer."
        };
    }
    table(entidade) {
        const tables = {
    ...
    async audit(tenantId, instrumentoId, entidade, entidadeId, acao, actor, ip) {
        await prisma.$executeRaw(Prisma.sql `
    ...
    async criarConcedente(input, tenantId, actor, ip) {
    ...
    async criarInstrumento(input, tenantId, actor, ip) {
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async criarModelo(input, tenantId, actor, ip) {
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async criarMeta(input, tenantId, actor, ip) {
    ...
    async criarRubrica(input, tenantId, actor, ip) {
        const instrumentoId = idToBigInt(input.instrumentoId);
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async criarReceita(input, tenantId, actor, ip) {
        const instrumentoId = idToBigInt(input.instrumentoId);
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async criarDespesa(input, tenantId, actor, ip) {
        const instrumentoId = idToBigInt(input.instrumentoId);
    ...
    async criarDocumento(input, tenantId, actor, ip) {
        const instrumentoId = idToBigInt(input.instrumentoId);
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async criarConciliacao(input, tenantId, actor, ip) {
        const instrumentoId = idToBigInt(input.instrumentoId);
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async criarDiligencia(input, tenantId, actor, ip) {
        const instrumentoId = idToBigInt(input.instrumentoId);
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async criarAprovacao(input, tenantId, actor, ip) {
        const instrumentoId = idToBigInt(input.instrumentoId);
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async criarTransparenciaPublica(input, tenantId, actor, ip) {
        const instrumentoId = idToBigInt(input.instrumentoId);
        const rows = await prisma.$queryRaw(Prisma.sql `
