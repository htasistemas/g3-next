...
export class TransparenciasService {
    ...
    async listar(rawTenantId) {
        ...
        return registros.map((item) => mapTransparenciaToResponse(item.transparencia, item.recebimentos, item.destinacoes, item.comprovantes, item.timelines, item.checklist, item.despesas, item.parecerHistorico));
    }
    async obter(rawId, rawTenantId) {
        const id = this.parseId(rawId);
        const tenantId = this.parseTenant(rawTenantId);
        const registro = await this.repository.buscarPorIdOuFalhar(id, tenantId);
        return mapTransparenciaToResponse(registro.transparencia, registro.recebimentos, registro.destinacoes, registro.comprovantes, registro.timelines, registro.checklist, registro.despesas, registro.parecerHistorico);
    }
    ...
    async atualizar(rawId, rawInput, rawTenantId, actor) {
        const id = this.parseId(rawId);
        const input = transparenciaInputSchema.parse(this.normalizarPayload(rawInput));
        const tenantId = this.parseTenant(rawTenantId);
    ...
    async alterarWorkflow(rawId, rawInput, actor) {
        const input = transparenciaWorkflowSchema.parse(rawInput);
        const tenantId = this.parseTenant(actor?.tenant_id);
        const id = this.parseId(rawId);
        const registro = await this.repository.buscarPorIdOuFalhar(id, tenantId);
        const atual = (registro.transparencia.status_workflow ?? "RASCUNHO");
        const transicoes = {
            RASCUNHO: ["EM_ANALISE"],
        ...
        if (input.acao === "ENVIAR_ANALISE") {
            const erros = [];
            if (!registro.transparencia.periodo_inicio || !registro.transparencia.periodo_fim)
                erros.push("Informe o período da prestação.");
            if (!registro.transparencia.instrumento?.trim())
                erros.push("Informe o instrumento ou parceria.");
            if (!registro.transparencia.objeto?.trim())
                erros.push("Informe o objeto da parceria.");
            if (!registro.comprovantes.length)
                erros.push("Adicione ao menos um comprovante.");
            if (!registro.checklist.length || registro.checklist.some((item) => item.status !== "concluido"))
                erros.push("Conclua o checklist antes de enviar.");
            ...
            if (erros.length)
                throw new AppError(erros.join(" "), 422);
        }
        if (input.acao === "APROVAR" || input.acao === "APROVAR_RESSALVAS" || input.acao === "REJEITAR") {
            const erros = [];
            if (!registro.transparencia.parecer_texto?.trim())
                erros.push("Registre o parecer técnico antes da decisão.");
            if (registro.transparencia.parecer_conclusao !== input.acao)
                erros.push("A conclusão do parecer deve corresponder à decisão selecionada.");
            if (input.acao === "APROVAR_RESSALVAS" && !registro.transparencia.parecer_ressalvas?.trim())
                erros.push("Informe as ressalvas do parecer.");
            if (erros.length)
                throw new AppError(erros.join(" "), 422);
        }
