...
export class TransparenciasRepository {
    ...
    async criar(input, tenantId) {
      ...
      `);
            ...
            if (!transparenciaId)
                throw new AppError("Nao foi possivel criar registro de prestacao de contas.", 500);
    ...
    async atualizar(id, input, tenantId, usuarioId, usuarioNome) {
        await ensureTransparenciasEstrutura();
    ...
    async atualizarStatusWorkflow(id, status, tenantId, usuarioId, usuarioNome, observacao) {
        await ensureTransparenciasEstrutura();
    ...
    async validarUnidade(unidadeId, tenantId) {
        if (!unidadeId) {
            return null;
        }
    ...
    async sincronizarInstrumentosProfissionais(tenantId) {
        await prisma.$executeRaw(Prisma.sql `
      INSERT INTO transparencia (
        tenant_id,
        unidade_id,
    ...
    async listarRecebimentos(ids, tenantId) {
    ...
    async listarDestinacoes(ids, tenantId) {
        return prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        transparencia_id,
    ...
    async listarComprovantes(ids, tenantId) {
        return prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        transparencia_id,
    ...
    async listarTimelines(ids, tenantId) {
        return prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        transparencia_id,
    ...
    async listarChecklist(ids, tenantId) {
        return prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        transparencia_id,
    ...
    async listarDespesas(ids, tenantId) {
        return prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        transparencia_id,
    ...
    async listarParecerHistorico(ids, tenantId) {
        return prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        transparencia_id,
    ...
    async salvarRelacionamentos(tx, transparenciaId, input, tenantId, usuarioId, usuarioNome) {
        await tx.$executeRaw(Prisma.sql `
    ...
    async registrarParecerHistorico(tx, transparenciaId, input, tenantId, usuarioId, usuarioNome) {
        await tx.$executeRaw(Prisma.sql `
    ...
    async inserirRecebimentos(tx, transparenciaId, lista, tenantId) {
    ...
    async inserirDestinacoes(tx, transparenciaId, lista, tenantId) {
        for (let index = 0; index < lista.length; index += 1) {
            const item = lista[index];
            await tx.$executeRaw(Prisma.sql `
    ...
    async inserirComprovantes(tx, transparenciaId, lista, tenantId) {
        for (let index = 0; index < lista.length; index += 1) {
            const item = lista[index];
            await tx.$executeRaw(Prisma.sql `
    ...
    async inserirTimelines(tx, transparenciaId, lista, tenantId) {
        for (let index = 0; index < lista.length; index += 1) {
            const item = lista[index];
            await tx.$executeRaw(Prisma.sql `
    ...
    async inserirChecklist(tx, transparenciaId, lista, tenantId) {
        for (let index = 0; index < lista.length; index += 1) {
            const item = lista[index];
            await tx.$executeRaw(Prisma.sql `
    ...
    async inserirDespesas(tx, transparenciaId, lista, tenantId) {
        for (let index = 0; index < lista.length; index += 1) {
            const item = lista[index];
            await tx.$executeRaw(Prisma.sql `
