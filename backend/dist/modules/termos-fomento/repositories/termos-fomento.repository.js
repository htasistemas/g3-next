...
export class TermosFomentoRepository {
    ...
    async adicionarAditivo(termoId, input, tenantId) {
        await ensureTermosFomentoEstrutura();
    ...
    async listarAditivosPorTermos(termosIds, tenantId) {
    ...
    async listarDocumentosPorTermos(termosIds, tenantId) {
        return prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        termo_fomento_id,
    ...
    async salvarRelacionamentos(tx, termoId, input, tenantId) {
        await tx.$executeRaw(Prisma.sql `
    ...
    async inserirAditivo(tx, termoId, input, tenantId) {
        const rows = await tx.$queryRaw(Prisma.sql `
    ...
    async inserirDocumento(tx, termoId, tipoDocumento, input, aditivoId, tenantId) {
        await tx.$executeRaw(Prisma.sql `
