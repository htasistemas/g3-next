...
export class RegistroPontoRepository {
    ...
    async marcarPonto(input, ator, origem) {
        ...
        return prisma.$transaction(async (tx) => {
            const usuario = await this.buscarContextoUsuarioTx(tx, ator.id);
            if (!usuario) {
            ...
            const registroId = await this.ensureRegistroHojeTx(tx, ator.id, usuario.unidade_id ?? null);
            ...
            const [{ agora }] = await tx.$queryRaw(Prisma.sql `SELECT NOW() AS agora`);
    ...
    async ensureRegistroHojeTx(tx, usuarioId, unidadeId) {
    ...
    async buscarRegistroParaAtualizacaoTx(tx, registroId) {
        const rows = await tx.$queryRaw(Prisma.sql `
      SELECT
        id,
        usuario_id,
    ...
    async listarBatidasTx(tx, registroId) {
    ...
    async buscarListaRowPorIdTx(tx, registroId) {
        const rows = await tx.$queryRaw(Prisma.sql `
      SELECT
        r.id,
        r.usuario_id,
    ...
    validarOrigemMarcacao(usuario, origem, validarLocalizacao) {
        ...
        let geoDetalhes = { geo_aplicado: false };
    ...
    async recalcularTotaisTx(tx, registroId, horarioFuncionamento) {
        const registro = await this.buscarRegistroParaAtualizacaoTx(tx, registroId);
        ...
        const atrasos = entrada1Minutos === null ? 0 : Math.max(0, entrada1Minutos - atrasoReferencia);
    ...
    async reconstruirOcorrenciasSistemaTx(tx, registroId, contexto) {
        await tx.$executeRaw(Prisma.sql `
