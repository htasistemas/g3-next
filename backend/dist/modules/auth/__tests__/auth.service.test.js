...
function criarServiceComStubs(stubs) {
    ...
    service.repository = {
        buscarTenantsPorEmail: async () => stubs.tenantsPorEmail?.map((item, indice) => ({
    ...
    service.deveExigirMfa = (usuario) => Boolean(stubs.exigirMfa) ||
...
test("login informa quando o e-mail pertence a outra instituicao", async () => {
    ...
    await assert.rejects(() => service.login({
        ...
        assert.ok(error instanceof AppError);
        assert.equal(error.message, "O e-mail informado está vinculado a outra instituição. Verifique o CNPJ e o e-mail do administrador inicial cadastrado em Administração inicial.");
...
test("login bloqueia temporariamente por 15 segundos sem depender de status bloqueado", async () => {
    ...
    await assert.rejects(() => service.login({
        ...
        assert.ok(error instanceof AppError);
        assert.equal(error.message, "Usuario temporariamente bloqueado por tentativas invalidas. Aguarde 15 segundos e tente novamente.");
...
test("login direto com passkey respeita usuario com autenticacao segura por email", async () => {
    ...
    await assert.rejects(() => service.iniciarLoginPasskey({
        email: "admin@instituicao.org.br",
        cnpj: "11.111.111/0001-11",
        origin: "http://localhost:5173"
    }), (error) => {
        assert.ok(error instanceof AppError);
        assert.equal(error.message, "Este usuario exige autenticacao segura por e-mail. Entre com CNPJ, e-mail e senha para receber a contrassenha.");
        assert.equal(error.statusCode, 403);
        return true;
    });
});
