...
function formatDateWithHyphen(value) {
    const dataIso = formatDateOnly(value);
    const [ano, mes, dia] = dataIso.split("-");
    return `${dia}-${mes}-${ano}`;
}
export function calcularDiasDecorridosCarencia(dataAtual, ultimaEntrega) {
...
export function entregaEstaDentroDaCarencia(dataAtual, ultimaEntrega, diasCarencia) {
    if (diasCarencia <= 0)
        return false;
...
export class DoacaoRealizadaService {
    ...
    parseTermo(rawTermo) {
        ...
        if (Array.isArray(rawTermo)) {
            const primeiroTexto = rawTermo.find((item) => typeof item === "string");
    ...
    async aplicarRegraCarencia(input, ator, ignorarDoacaoRealizadaId) {
        ...
        for (const item of input.itens) {
            ...
            if (!autorizacao) {
                autorizacao = await this.validarAutorizacaoForaCarencia(input.senha_administrativa, ator);
            }
            itensProcessados.push({
                ...item,
                fora_carencia: true,
        ...
        if (itensBloqueados.length) {
            throw new AppError(this.montarMensagemItensBloqueadosCarencia(itensBloqueados, tempoCarenciaDias), 409);
        }
        return {
            ...input,
    ...
    montarMensagemItensBloqueadosCarencia(itensBloqueados, tempoCarenciaDias) {
    ...
    async validarAutorizacaoForaCarencia(senhaAdministrativa, ator) {
        if (!ator.id) {
            throw new AppError("Usuario autenticado invalido.", 401);
        }
        if (!ator.permissoes.includes("ADMINISTRADOR")) {
            throw new AppError("Somente administrador logado pode autorizar entrega fora da carencia.", 403);
        }
