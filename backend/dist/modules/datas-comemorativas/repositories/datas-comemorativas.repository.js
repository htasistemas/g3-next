...
function mapConfigRow(row) {
    ...
    return {
        ...
        frequenciaSincronizacao: String(row.frequencia_sincronizacao ?? defaultConfig.frequenciaSincronizacao),
        providerFeriadoPrincipal: String(row.provider_feriado_principal ?? defaultConfig.providerFeriadoPrincipal),
        providerFeriadoFallback: String(row.provider_feriado_fallback ?? defaultConfig.providerFeriadoFallback),
...
function buildWhere(filters, options) {
    const params = [];
    const parts = ["excluido_logico = FALSE"];
    const push = (value) => {
        params.push(value);
...
function buildOrder(filters, options) {
    const campo = filters.ordenarPor?.trim() || "data";
    const direcao = filters.ordem?.trim()?.toUpperCase() === "ASC" ? "ASC" : "DESC";
    if (campo === "titulo")
        return `ORDER BY titulo ${direcao}, id DESC`;
    if (campo === "tipo_evento")
        return `ORDER BY tipo_evento ${direcao}, titulo ASC`;
    if (campo === "abrangencia")
        return `ORDER BY abrangencia ${direcao}, titulo ASC`;
...
export class DatasComemorativasRepository {
    ...
    async listarParaExportacao(filters) {
        await this.ensureEstrutura();
        const year = Number(filters.ano ?? new Date().getFullYear()) || new Date().getFullYear();
        const { whereSql, params } = buildWhere(filters);
        const orderSql = buildOrder(filters, { year });
    ...
    async buscarDuplicidade(input, ignoreId) {
        await this.ensureEstrutura();
    ...
    async criar(input, usuarioId) {
        ...
        RETURNING id
      `, input.titulo.trim(), trimOrUndefined(input.descricao) ?? null, input.dia ?? 0, input.mes ?? 0, input.ano ?? null, input.dataEvento ?? null, input.tipoEvento, input.abrangencia, input.uf?.trim().toUpperCase() ?? null, normalizarMunicipio(input.municipio) ?? null, input.recorrenteAnual !== false, trimOrUndefined(input.fonteOrigem) ?? "manual", trimOrUndefined(input.origemReferencia) ?? null, trimOrUndefined(input.corExibicao) ?? null, trimOrUndefined(input.icone) ?? null, Number(input.prioridadePopup ?? 0), input.exibirNoPopup !== false, input.ativo !== false, usuarioId ?? null);
        const id = rows[0]?.id;
        if (!id) {
    ...
    async atualizar(id, input, usuarioId) {
        ...
        SET
          ...
          atualizado_em = NOW()
        WHERE id = $20
      `, input.titulo.trim(), trimOrUndefined(input.descricao) ?? null, input.dia ?? 0, input.mes ?? 0, input.ano ?? null, input.dataEvento ?? null, input.tipoEvento, input.abrangencia, input.uf?.trim().toUpperCase() ?? null, normalizarMunicipio(input.municipio) ?? null, input.recorrenteAnual !== false, trimOrUndefined(input.fonteOrigem) ?? "manual", trimOrUndefined(input.origemReferencia) ?? null, trimOrUndefined(input.corExibicao) ?? null, trimOrUndefined(input.icone) ?? null, Number(input.prioridadePopup ?? 0), input.exibirNoPopup !== false, input.ativo !== false, usuarioId ?? null, id);
        return this.buscarPorIdOuFalhar(id);
    }
    async excluirLogico(id, usuarioId) {
        await this.buscarPorIdOuFalhar(id);
        await prisma.$executeRawUnsafe(`
        UPDATE datas_comemorativas
        SET
    ...
    async alterarAtivo(id, ativo, usuarioId) {
        await this.buscarPorIdOuFalhar(id);
        await prisma.$executeRawUnsafe(`
        UPDATE datas_comemorativas
        SET
          ...
          atualizado_em = NOW()
        WHERE id = $3
      `, ativo, usuarioId ?? null, id);
        return this.buscarPorIdOuFalhar(id);
    }
    async listarPorData(data, contexto, somentePopup = false) {
        await this.ensureEstrutura();
        const filters = {};
        if (somentePopup) {
    ...
    async listarPorMes(year, month, filters, contexto) {
        await this.ensureEstrutura();
    ...
    async salvarImportado(item, usuarioId) {
        ...
        if (!existente) {
            ...
            return { registro: criado, acao: "inserido" };
        }
        const atualizado = await this.atualizar(existente.id, {
            ...item,
        ...
        return { registro: atualizado, acao: "atualizado" };
    }
    ...
    async salvarConfiguracoes(input, usuarioId) {
        await this.ensureEstrutura();
        ...
        SET
          ...
          atualizado_em = NOW()
        WHERE id = 1
      `, merged.popupHabilitado, merged.popupUmaVezPorDia, merged.popupMostrarFeriados, merged.popupMostrarComemorativas, merged.popupMostrarEventosInternos, merged.popupLimiteItens, merged.popupOrdenarPorPrioridade, merged.sincronizacaoAutomatica, merged.frequenciaSincronizacao, merged.providerFeriadoPrincipal, merged.providerFeriadoFallback, merged.cacheDias, merged.ativo, usuarioId ?? null);
        return this.buscarConfiguracoes();
    }
    async iniciarSyncLog(input) {
        await this.ensureEstrutura();
        const rows = await prisma.$queryRawUnsafe(`
        ...
        RETURNING id
      `, input.providerNome, input.tipoSync, JSON.stringify(input.parametrosExecucao ?? {}), input.executadoPor ?? null);
        return rows[0]?.id ?? BigInt(0);
    }
    async finalizarSyncLog(input) {
        await this.ensureEstrutura();
        await prisma.$executeRawUnsafe(`
        ...
        SET
          ...
          finalizado_em = NOW()
        WHERE id = $8
      `, input.quantidadeLidos, input.quantidadeInseridos, input.quantidadeAtualizados, input.quantidadeIgnorados, input.quantidadeErros, input.statusExecucao, input.detalhesErro ?? null, input.logId);
    }
    ...
    async listarPopupLogs(usuarioId) {
        await this.ensureEstrutura();
        return prisma.$queryRawUnsafe(`
        SELECT *
    ...
    async registrarAuditoria(acao, detalhes, usuarioId, eventoId) {
        await this.ensureEstrutura();
        await prisma.$executeRawUnsafe(`
      ...
      `, eventoId ?? null, acao, JSON.stringify(detalhes), usuarioId ?? null);
    }
    async jaRegistrouPopupNoDia(usuarioId, data) {
        await this.ensureEstrutura();
        const rows = await prisma.$queryRawUnsafe(`
        SELECT COUNT(*)::BIGINT AS total
        FROM datas_comemorativas_popup_log
    ...
    async registrarPopupVisualizacao(usuarioId, data, eventIds, acao, dispensado = false) {
        await this.ensureEstrutura();
        if (!eventIds.length) {
            await prisma.$executeRawUnsafe(`
        ...
        `, usuarioId, data, dispensado, acao ?? null);
            return;
        }
        ...
        `, usuarioId, data, BigInt(eventId), dispensado, acao ?? null);
        }
    }
    async listarLogsConsolidados() {
