import { dataComemorativaPopupRegistroSchema, dataComemorativaSyncRangeSchema, dataComemorativaSyncSchema } from "../datas-comemorativas.schema.js";
...
function obterContexto(request) {
    return {
        uf: typeof request.query.uf === "string"
            ? request.query.uf
            : typeof request.body?.uf === "string"
                ? request.body.uf
                : undefined,
        municipio: typeof request.query.municipio === "string"
...
export class DatasComemorativasController {
    ...
    async duplicar(request, response) {
        const usuarioId = request.authUser?.id;
        const resultado = await dateService.duplicate(request.params.id, usuarioId);
        return response.status(201).json(resultado);
    }
    async calendario(request, response) {
        const ano = Number(request.query.ano);
        const mes = Number(request.query.mes);
        const resultado = await dateService.getCalendarMatrix(ano, mes, request.query, obterContexto(request));
        return response.json(resultado);
    }
    async doDia(request, response) {
    ...
    async sincronizarFeriados(request, response) {
        const usuarioId = request.authUser?.id;
        const payload = dataComemorativaSyncSchema.parse(request.body ?? {});
        const resultado = await holidaySyncService.syncYear(payload.ano, payload.provider, usuarioId);
        return response.json(resultado);
    }
    async sincronizarIntervalo(request, response) {
        const usuarioId = request.authUser?.id;
        const payload = dataComemorativaSyncRangeSchema.parse(request.body ?? {});
        const resultado = await holidaySyncService.syncRange(payload.inicio, payload.fim, payload.provider, usuarioId);
        return response.json(resultado);
    }
    async importar(request, response) {
        const usuarioId = request.authUser?.id;
        const resultado = await importService.importar(request.body, usuarioId);
        return response.json(resultado);
    }
    async listarSyncLogs(_request, response) {
    ...
    async popupHoje(request, response) {
        const usuarioId = request.authUser?.id;
        if (!usuarioId) {
            return response.status(401).json({ message: "Nao autenticado." });
        }
        const data = typeof request.query.data === "string"
            ? request.query.data
            : new Date().toISOString().slice(0, 10);
        const resultado = await popupService.getPopupData(usuarioId, data, obterContexto(request));
        return response.json(resultado);
    }
    async registrarVisualizacao(request, response) {
        ...
        await popupService.registerPopupView(usuarioId, payload.data, payload.eventIds ?? [], payload.acao ?? null);
        return response.status(204).send();
    }
    async dispensarHoje(request, response) {
        const usuarioId = request.authUser?.id;
        if (!usuarioId) {
            return response.status(401).json({ message: "Nao autenticado." });
        }
