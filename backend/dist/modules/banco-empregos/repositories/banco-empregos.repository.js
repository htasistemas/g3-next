...
function faixaEtariaParaIntervalo(faixa) {
...
function whereCandidatos(filters) {
    ...
    if (termo) {
        const like = `%${termo}%`;
        condicoes.push(Prisma.sql `(c.nome_completo ILIKE ${like} OR COALESCE(c.email, '') ILIKE ${like} OR COALESCE(c.cargo_pretendido, '') ILIKE ${like})`);
    }
    ...
    if (filters.bairro?.trim())
        condicoes.push(Prisma.sql `c.bairro ILIKE ${`%${filters.bairro.trim()}%`}`);
    if (filters.cidade?.trim())
        condicoes.push(Prisma.sql `c.cidade ILIKE ${`%${filters.cidade.trim()}%`}`);
    if (filters.escolaridade?.trim())
        condicoes.push(Prisma.sql `c.escolaridade ILIKE ${`%${filters.escolaridade.trim()}%`}`);
    if (filters.areaInteresse?.trim())
        condicoes.push(Prisma.sql `c.area_interesse ILIKE ${`%${filters.areaInteresse.trim()}%`}`);
    if (filters.cargoPretendido?.trim())
        condicoes.push(Prisma.sql `c.cargo_pretendido ILIKE ${`%${filters.cargoPretendido.trim()}%`}`);
    if (filters.disponibilidade?.trim())
        condicoes.push(Prisma.sql `c.disponibilidade ILIKE ${`%${filters.disponibilidade.trim()}%`}`);
    if (filters.sexo?.trim())
        condicoes.push(Prisma.sql `c.sexo = ${filters.sexo.trim()}`);
    if (filters.situacao?.trim())
        condicoes.push(Prisma.sql `c.situacao = ${filters.situacao.trim()}`);
    if (typeof filters.possuiExperiencia === "boolean")
        condicoes.push(Prisma.sql `c.possui_experiencia = ${filters.possuiExperiencia}`);
    if (filters.dataCadastroDe)
        condicoes.push(Prisma.sql `c.criado_em::date >= ${filters.dataCadastroDe}::date`);
    if (filters.dataCadastroAte)
        condicoes.push(Prisma.sql `c.criado_em::date <= ${filters.dataCadastroAte}::date`);
    if (typeof filters.possuiCurriculo === "boolean") {
        condicoes.push(filters.possuiCurriculo
    ...
    if (typeof filters.possuiCertificados === "boolean") {
        condicoes.push(filters.possuiCertificados
...
function whereVagas(filters) {
    ...
    if (filters.titulo?.trim())
        condicoes.push(Prisma.sql `v.titulo ILIKE ${`%${filters.titulo.trim()}%`}`);
    if (filters.empresaNome?.trim())
        condicoes.push(Prisma.sql `v.empresa_nome ILIKE ${`%${filters.empresaNome.trim()}%`}`);
    if (filters.area?.trim())
        condicoes.push(Prisma.sql `v.area ILIKE ${`%${filters.area.trim()}%`}`);
    if (filters.cidade?.trim())
        condicoes.push(Prisma.sql `v.cidade ILIKE ${`%${filters.cidade.trim()}%`}`);
    if (filters.situacao?.trim())
        condicoes.push(Prisma.sql `v.situacao = ${filters.situacao.trim()}`);
    if (filters.dataAberturaDe)
        condicoes.push(Prisma.sql `v.data_abertura >= ${filters.dataAberturaDe}::date`);
...
export class BancoEmpregosRepository {
    ...
    async salvarCandidato(id, input) {
        ...
        return prisma.$transaction(async (tx) => {
          ...
          INSERT INTO banco_empregos_candidato (
            ...
            ${(toJson(formacoes) ?? [])},
            ${(toJson(habilidades) ?? [])},
            ${(toJson(input.curriculoExtraido ?? {}) ?? {})},
            NOW(),
            NOW()
          )
          RETURNING id
        `);
          ...
          SET
            ...
            situacao = ${situacao},
            ativo = ${ativo},
            experiencias_json = ${(toJson(experiencias) ?? [])},
            formacoes_json = ${(toJson(formacoes) ?? [])},
            habilidades_json = ${(toJson(habilidades) ?? [])},
    ...
    async adicionarDocumento(candidatoId, arquivoId, payload) {
      ...
      `);
          ...
          SET curriculo_versao = ${versao},
              ...
              curriculo_extraido_json = ${(toJson(payload.extraido ?? {}) ?? {})},
              atualizado_em = NOW()
    ...
    async salvarAvaliacao(processoId, input, usuarioId, usuarioNome) {
        ...
        return prisma.$transaction(async (tx) => {
            ...
            const existenteRows = await tx.$queryRaw(Prisma.sql `
        SELECT id
...
function whereProcessos(filters) {
    ...
    if (filters.candidatoId?.trim())
        condicoes.push(Prisma.sql `p.candidato_id = ${BigInt(filters.candidatoId.trim())}`);
    if (filters.etapa?.trim())
        condicoes.push(Prisma.sql `p.etapa = ${filters.etapa.trim()}`);
    if (filters.status?.trim())
        condicoes.push(Prisma.sql `p.status = ${filters.status.trim()}`);
    if (typeof filters.selecionado === "boolean")
        condicoes.push(Prisma.sql `p.selecionado = ${filters.selecionado}`);
    if (typeof filters.contratado === "boolean")
...
function whereHistorico(filters) {
    ...
    if (filters.candidatoId?.trim())
        condicoes.push(Prisma.sql `h.candidato_id = ${BigInt(filters.candidatoId.trim())}`);
    if (filters.vagaId?.trim())
        condicoes.push(Prisma.sql `h.vaga_id = ${BigInt(filters.vagaId.trim())}`);
    if (filters.processoId?.trim())
...
function mapSituacaoCandidatoPorProcesso(status, contratado, selecionado) {
    if (contratado || status === "CONTRATADO")
        return "CONTRATADO";
    if (status === "APROVADO")
        return "APROVADO";
    if (status === "REPROVADO")
    ...
    if (status === "ENCAMINHADO")
        return "ENCAMINHADO";
    if (status === "ENTREVISTA_MARCADA")
        return "EM_ENTREVISTA";
    if (selecionado)
        return "PRE_SELECIONADO";
    return "EM_ANALISE";
}
