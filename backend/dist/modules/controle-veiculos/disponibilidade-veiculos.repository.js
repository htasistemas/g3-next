...
let ensureDisponibilidadePromise = null;
...
function formatarNomeVeiculo(row) {
    const partes = [row.placa, row.marca, row.modelo].filter((item) => Boolean(item?.trim()));
    return partes.join(" - ") || "---";
}
function normalizarStatusRegistro(valor) {
    const texto = String(valor ?? "").trim().toUpperCase();
    if (texto === "CANCELADO" || texto === "ENCERRADO" || texto === "EXCLUIDO_LOGICAMENTE")
        return texto;
    return "ATIVO";
}
function intervalosSobrepoem(a, b) {
...
export class ControleVeiculosDisponibilidadeRepository {
    ...
    async travarVeiculo(tenantId, veiculoId) {
        await prisma.$executeRawUnsafe(`
      SELECT pg_advisory_xact_lock(hashtext($1), $2::int)
      `, tenantId, veiculoId);
    }
    async buscarVeiculoAtivoOuFalhar(tenantId, veiculoId) {
    ...
    async buscarConflito(tenantId, veiculoId, inicio, fim, ignorarId) {
        const rows = await prisma.$queryRawUnsafe(`
      SELECT
        d.*,
        v.placa,
      ...
      LIMIT 1
      `, tenantId, BigInt(veiculoId), inicio, fim, ignorarId ? BigInt(ignorarId) : null);
        return rows[0] ?? null;
    }
    async registrarHistorico(tx, id, acao, antes, depois, usuarioId, usuarioNome) {
      ...
      `, id, acao, antes ? JSON.stringify(antes) : null, depois ? JSON.stringify(depois) : null, usuarioId ?? null, usuarioNome ?? null);
    }
    mapRow(row) {
        return {
            id: Number(row.id),
    ...
    async listarDisponibilidades(tenantId) {
    ...
    async criar(input, tenantId, usuario) {
        await this.garantirEstrutura();
        return prisma.$transaction(async (tx) => {
            ...
            if (conflito) {
                throw new AppError(`Não foi possível concluir o lançamento. Este veículo já possui uma reserva ou indisponibilidade no período informado. Situação existente: ${conflito.tipo_situacao === "INDISPONIVEL" ? "Indisponível" : "Reservado"}.`, 409);
            }
        ...
        RETURNING *
        `, tenantId, BigInt(input.veiculoId), input.tipoSituacao, inicio, fim, input.motivo ?? null, input.motivoDetalhado ?? null, input.destino ?? null, input.responsavelNome ?? null, input.observacoes ?? null, usuario?.id ?? null, usuario?.nome ?? null);
            const salvo = rows[0];
            if (!salvo)
                throw new AppError("Não foi possível salvar a disponibilidade.", 500);
            await this.registrarHistorico(tx, salvo.id, "CRIADO", null, this.mapRow(salvo), usuario?.id, usuario?.nome);
            return this.mapRow(salvo);
        });
    }
    async atualizar(id, input, tenantId, usuario) {
        await this.garantirEstrutura();
        return prisma.$transaction(async (tx) => {
            await this.travarVeiculo(tenantId, input.veiculoId);
            await this.buscarVeiculoAtivoOuFalhar(tenantId, input.veiculoId);
            const atual = await this.obterPorId(id, tenantId);
            const conflito = await this.buscarConflito(tenantId, input.veiculoId, new Date(input.dataHoraInicio), new Date(input.dataHoraFim), id);
            if (conflito) {
                throw new AppError(`Não foi possível concluir o lançamento. Este veículo já possui uma reserva ou indisponibilidade no período informado. Situação existente: ${conflito.tipo_situacao === "INDISPONIVEL" ? "Indisponível" : "Reservado"}.`, 409);
            }
            const rows = await tx.$queryRawUnsafe(`
        ...
        WHERE tenant_id::text = $1
          AND id = $13
        RETURNING *
        `, tenantId, BigInt(input.veiculoId), input.tipoSituacao, new Date(input.dataHoraInicio), new Date(input.dataHoraFim), input.motivo ?? null, input.motivoDetalhado ?? null, input.destino ?? null, input.responsavelNome ?? null, input.observacoes ?? null, usuario?.id ?? null, usuario?.nome ?? null, BigInt(id));
            const salvo = rows[0];
            if (!salvo)
                throw new AppError("Não foi possível atualizar a disponibilidade.", 500);
            await this.registrarHistorico(tx, salvo.id, "ATUALIZADO", atual, this.mapRow(salvo), usuario?.id, usuario?.nome);
            return this.mapRow(salvo);
        });
    }
    async cancelar(id, tenantId, motivoCancelamento, usuario) {
        await this.garantirEstrutura();
        return prisma.$transaction(async (tx) => {
            const atual = await this.obterPorId(id, tenantId);
            const rows = await tx.$queryRawUnsafe(`
        ...
        RETURNING *
        `, tenantId, usuario?.id ?? null, usuario?.nome ?? null, motivoCancelamento, BigInt(id));
            const salvo = rows[0];
            if (!salvo)
                throw new AppError("Não foi possível cancelar o registro.", 500);
            await this.registrarHistorico(tx, salvo.id, "CANCELADO", atual, this.mapRow(salvo), usuario?.id, usuario?.nome);
            return this.mapRow(salvo);
        });
    }
    async encerrar(id, tenantId, usuario) {
        await this.garantirEstrutura();
        return prisma.$transaction(async (tx) => {
            const atual = await this.obterPorId(id, tenantId);
            const rows = await tx.$queryRawUnsafe(`
        ...
        WHERE tenant_id::text = $1
          ...
          AND status_registro = 'ATIVO'
        RETURNING *
        `, tenantId, usuario?.id ?? null, usuario?.nome ?? null, BigInt(id));
            const salvo = rows[0];
            if (!salvo)
                throw new AppError("Não foi possível encerrar o registro.", 500);
            await this.registrarHistorico(tx, salvo.id, "ENCERRADO", atual, this.mapRow(salvo), usuario?.id, usuario?.nome);
            return this.mapRow(salvo);
        });
    }
    async excluir(id, tenantId, usuario) {
        await this.garantirEstrutura();
        return prisma.$transaction(async (tx) => {
            const atual = await this.obterPorId(id, tenantId);
            const rows = await tx.$queryRawUnsafe(`
    ...
    async resumoDisponibilidade(tenantId, consulta) {
    ...
    async agendaVeiculo(tenantId, veiculoId, inicio, fim) {
        await this.garantirEstrutura();
      ...
      WHERE d.tenant_id::text = $1
        ...
        AND d.data_hora_fim > $3
      ORDER BY d.data_hora_inicio ASC, d.id ASC
      `, tenantId, BigInt(veiculoId), inicio, fim);
        return rows.map((row) => this.mapRow(row));
    }
    async proximaDisponibilidade(tenantId, veiculoId, referencia) {
        await this.garantirEstrutura();
        await this.buscarVeiculoAtivoOuFalhar(tenantId, veiculoId);
        const rows = await prisma.$queryRawUnsafe(`
      SELECT
      ...
      `, tenantId, BigInt(veiculoId), referencia);
