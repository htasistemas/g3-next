...
import { mapDiarioBordoToResponse, mapMotoristaAutorizadoToResponse, mapVeiculoToResponse } from "../controle-veiculos.mapper.js";
import { diarioBordoInputSchema, motoristaAutorizadoInputSchema, veiculoInputSchema } from "../controle-veiculos.schema.js";
...
export class ControleVeiculosService {
    ...
    async atualizarDiario(rawId, rawInput) {
        const id = this.parseId(rawId);
        const input = diarioBordoInputSchema.parse(this.normalizarPayload(rawInput));
        const registro = await this.repository.atualizarDiario(id, input);
        return mapDiarioBordoToResponse(registro);
    }
    async removerDiario(rawId) {
        const id = this.parseId(rawId);
        await this.repository.removerDiario(id);
    }
    async listarMotoristasDisponiveis(rawNome) {
    ...
    async listarMotoristasAutorizados(rawVeiculoId) {
        const veiculoId = typeof rawVeiculoId === "string" && rawVeiculoId.trim()
            ? Number(rawVeiculoId)
            : typeof rawVeiculoId === "number"
                ? rawVeiculoId
                : undefined;
        const registros = await this.repository.listarMotoristasAutorizados(Number.isInteger(veiculoId) && veiculoId > 0 ? veiculoId : undefined);
        return registros.map(mapMotoristaAutorizadoToResponse);
    }
    async criarMotoristaAutorizado(rawInput) {
    ...
    async atualizarMotoristaAutorizado(rawId, rawInput) {
        const id = this.parseId(rawId);
        const input = motoristaAutorizadoInputSchema.parse(this.normalizarPayload(rawInput));
        const registro = await this.repository.atualizarMotoristaAutorizado(id, input);
        return mapMotoristaAutorizadoToResponse(registro);
    }
    async removerMotoristaAutorizado(rawId) {
        const id = this.parseId(rawId);
