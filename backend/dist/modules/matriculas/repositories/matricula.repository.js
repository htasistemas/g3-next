...
function calcularVagasPorHorario(input) {
    if (!input.controle_horario_atendimento)
        return input.vagas_totais;
...
export class MatriculaRepository {
    ...
    async obterResumoCatalogo(tenantId) {
        await this.ensureEstrutura();
    ...
    async listarProfissionais(termo, tenantId) {
        await this.ensureEstrutura();
        const termoSanitizado = trimOrUndefined(termo);
        const termoLike = termoSanitizado ? `%${termoSanitizado}%` : undefined;
    ...
    async listarPresencaDatas(cursoId, tenantId, somentePendentes = false) {
        await this.ensureEstrutura();
    ...
    async sincronizarPresencaDatasRealizadas(cursoId, tenantId, tipoCurso) {
    ...
    async criarPresencaData(cursoId, input, tenantId) {
        await this.ensureEstrutura();
    ...
    async atualizarPresencaData(cursoId, presencaDataId, input, tenantId) {
        await this.ensureEstrutura();
    ...
    async listarPresencasPorData(cursoId, presencaDataId, tenantId) {
        await this.ensureEstrutura();
        const presencaData = await this.buscarPresencaDataOuFalhar(cursoId, presencaDataId, tenantId);
    ...
    async salvarPresencasPorData(cursoId, presencaDataId, input, tenantId, usuario) {
        await this.ensureEstrutura();
        const presencaData = await this.buscarPresencaDataOuFalhar(cursoId, presencaDataId, tenantId);
        ...
        `);
                ...
                const usuarioAnterior = await tx.$queryRaw(Prisma.sql `
          SELECT
            status,
            usuario_registro_id,
        ...
        `);
                ...
                if (presencaJaConfirmada && statusAnterior !== item.status && !input.senha_confirmacao) {
                    throw new AppError("A presença ou ausência deste beneficiário já foi salva. Confirme a alteração informando a senha do usuário logado.", 409);
                }
                await tx.$executeRaw(Prisma.sql `
    ...
    async buscarPresencaDataOuFalhar(cursoId, presencaDataId, tenantId) {
    ...
    async registrarAuditoriaPresencaTx(tx, payload) {
        try {
        ...
        `, auditoria.tenant_id, auditoria.beneficiario_id, auditoria.entidade_id, auditoria.acao, auditoria.descricao, JSON.stringify(auditoria.dados_novos), auditoria.usuario_id, auditoria.usuario_nome);
        }
        catch (error) {
            console.warn("[matriculas] falha ao registrar auditoria de presenca:", error);
        }
    }
    async registrarAuditoriaExclusaoPresencaDataTx(tx, payload) {
        try {
            await tx.$executeRawUnsafe(`
          INSERT INTO central_auditoria (
            tenant_id,
        ...
        `, payload.tenantId, payload.presencaDataId, `Data de presença excluída em ${payload.dataAula.toISOString().slice(0, 10)} com ${payload.totalExcluidos} registro(s) removido(s).`, JSON.stringify({
                tenantId: payload.tenantId,
                cursoId: payload.cursoId.toString(),
                presencaDataId: payload.presencaDataId.toString(),
                dataAula: payload.dataAula.toISOString(),
                totalExcluidos: payload.totalExcluidos
            }), payload.usuarioId ?? null, payload.usuarioNome ?? null);
        }
        catch (error) {
            console.warn("[matriculas] falha ao registrar auditoria de exclusao de presenca:", error);
        }
    }
    async inserirMatriculas(tx, cursoId, matriculas, tenantId) {
    ...
    async inserirFilaEspera(tx, cursoId, filaEspera, tenantId) {
