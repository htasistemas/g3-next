...
export class ProfissionalController {
    ...
    async criar(request, response) {
        const profissional = await service.criar(request.body, request.authUser?.id, request.authUser?.tenant_id);
        return response.status(201).json({ profissional });
    }
    async atualizar(request, response) {
        const profissional = await service.atualizar(request.params.id, request.body, request.authUser?.id, request.authUser?.tenant_id);
        return response.json({ profissional });
    }
