...
export class OcorrenciasCriancaController {
    ...
    async atualizar(request, response) {
        const ocorrencia = await service.atualizar(request.params.id, request.body, request.authUser?.tenant_id);
        return response.json(ocorrencia);
    }
    ...
    async removerAnexo(request, response) {
        await service.removerAnexo(request.params.id, request.params.anexoId, request.authUser?.id, request.authUser?.tenant_id);
        return response.status(204).send();
    }
