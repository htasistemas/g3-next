...
import { projetoFiltersSchema, projetoInputSchema, projetoTarefaInputSchema, projetoTarefaStatusSchema } from "../projeto.schema.js";
import { ProjetoRepository } from "../repositories/projeto.repository.js";
import { AppError } from "../../../shared/errors/app-error.js";
export class ProjetoService {
    repository = new ProjetoRepository();
    async listar(rawFilters, rawTenantId, contexto) {
        const tenantId = this.parseTenant(rawTenantId);
        const filters = projetoFiltersSchema.parse({ ...(rawFilters ?? {}), ...(contexto?.projeto_id ? { id: contexto.projeto_id } : {}) });
    ...
    async dashboard(rawFilters, rawTenantId, contexto) {
        const tenantId = this.parseTenant(rawTenantId);
        const filters = projetoFiltersSchema.parse({ ...(rawFilters ?? {}), ...(contexto?.projeto_id ? { id: contexto.projeto_id } : {}) });
        return this.repository.dashboard(filters, tenantId);
    }
    async buscarPorId(rawId, rawTenantId, contexto) {
    ...
    async atualizar(rawId, rawInput, actor) {
        const id = this.parseId(rawId, "Projeto");
        const tenantId = this.parseTenant(actor.tenant_id);
    ...
    async remover(rawId, actor) {
        const id = this.parseId(rawId, "Projeto");
        const tenantId = this.parseTenant(actor.tenant_id);
        this.validarProjetoContexto(id, actor.contexto);
        await this.repository.inativar(id, idActor(actor), actor.nome, tenantId);
    }
    async listarHistorico(rawProjetoId, rawTenantId, contexto) {
    ...
    async criarTarefa(rawProjetoId, rawInput, actor) {
        const projetoId = this.parseId(rawProjetoId, "Projeto");
        const tenantId = this.parseTenant(actor.tenant_id);
        this.validarProjetoContexto(projetoId, actor.contexto);
        const input = projetoTarefaInputSchema.parse(rawInput);
        const tarefa = await this.repository.criarTarefa(projetoId, idActor(actor), actor.nome, input, tenantId);
        return mapProjetoTarefaToResponse(tarefa);
    }
    async atualizarTarefa(rawProjetoId, rawTarefaId, rawInput, actor) {
        const projetoId = this.parseId(rawProjetoId, "Projeto");
        const tarefaId = this.parseId(rawTarefaId, "Tarefa");
        const tenantId = this.parseTenant(actor.tenant_id);
        this.validarProjetoContexto(projetoId, actor.contexto);
        const input = projetoTarefaInputSchema.parse(rawInput);
        const tarefa = await this.repository.atualizarTarefa(projetoId, tarefaId, idActor(actor), actor.nome, input, tenantId);
        return mapProjetoTarefaToResponse(tarefa);
    }
    async moverTarefa(rawProjetoId, rawTarefaId, rawInput, actor) {
        const projetoId = this.parseId(rawProjetoId, "Projeto");
        const tarefaId = this.parseId(rawTarefaId, "Tarefa");
        const tenantId = this.parseTenant(actor.tenant_id);
        this.validarProjetoContexto(projetoId, actor.contexto);
        const parsed = projetoTarefaStatusSchema.safeParse(rawInput?.status);
        if (!parsed.success) {
            throw new AppError("Informe o status de destino da tarefa.", 400);
        }
        const tarefa = await this.repository.moverTarefa(projetoId, tarefaId, idActor(actor), actor.nome, parsed.data, tenantId);
        return mapProjetoTarefaToResponse(tarefa);
    }
    ...
    validarProjetoContexto(projetoId, contexto) {
