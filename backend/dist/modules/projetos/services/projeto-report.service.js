...
import { RelatorioTemplatePadrao } from "../../reports/templates/relatorio-template-padrao.js";
import { HtmlPdfRenderer } from "../../reports/services/html-pdf-renderer.js";
import { formatProjetoAreaLabel, formatProjetoPrioridadeLabel, formatProjetoStatusLabel, formatProjetoTarefaTipoLabel } from "../projeto.mapper.js";
...
export class ProjetoReportService {
    ...
    async gerar(tipo, rawFilters, authUser) {
    ...
    async gerarRelatorioLista(tipo, filters, tenantId, contexto, authUser) {
    ...
    async gerarRelatorioIndividual(projetoId, tenantId, contexto, authUser) {
        if (!projetoId?.trim()) {
            ...
            secoes: [
                {
                    ...
                    conteudo: this.parseEquipe(registro.projeto.equipe_envolvida).map((item) => `• ${item}`).join("\n") ||
                        "Não informada."
                },
                {
                    titulo: "Observações",
