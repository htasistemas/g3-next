...
export function formatProjetoStatusLabel(value) {
    return value ? projetoStatusLabels[value] ?? value : "---";
}
export function formatProjetoPrioridadeLabel(value) {
    return value ? prioridadeLabels[value] ?? value : "---";
}
export function formatProjetoAreaLabel(value) {
    return value ? areaLabels[value] ?? value : "---";
}
export function formatProjetoTarefaTipoLabel(value) {
    return value ? tarefaTipoLabels[value] ?? value : "---";
}
export function mapProjetoTarefaToResponse(tarefa) {
    const hoje = new Date();
    const dataPrevista = toIsoDate(tarefa.data_prevista);
    const atrasada = tarefa.status !== "CONCLUIDO" &&
...
export function mapProjetoHistoricoToResponse(item) {
    return {
...
export function mapProjetoToResponse(projeto, tarefas, historico) {
    const totalTarefas = Number(projeto.total_tarefas ?? tarefas.length);
    const tarefasConcluidas = Number(projeto.tarefas_concluidas ?? tarefas.filter((item) => item.status === "CONCLUIDO").length);
    const percentual = projeto.percentual_evolucao !== null && projeto.percentual_evolucao !== undefined
        ...
        : totalTarefas > 0
            ...
            : projeto.status === "CONCLUIDO"
                ...
                : 0;
    const hoje = new Date();
    const atraso = projeto.status !== "CONCLUIDO" &&
        projeto.status !== "CANCELADO" &&
        projeto.prazo_previsto.getTime() < new Date(hoje.toDateString()).getTime();
    return {
        id: toStringId(projeto.id),
        nome: projeto.nome,
        descricaoCurta: (projeto.descricao_completa ?? "").trim().slice(0, 160) || "Sem descrição informada.",
