...
function criarServiceComRepositorioStub() {
    ...
    service.repository = {
        ...
        criarUsuario: async (id, input, nomeUsuarioAtor, idAtor) => ({
            id,
            input,
            nomeUsuarioAtor,
            idAtor
        }),
        atualizarUsuario: async (id, usuarioId, input, nomeUsuarioAtor, idAtor) => ({
            id,
            usuarioId,
            input,
            nomeUsuarioAtor,
        ...
        resetarSenhaUsuario: async (id, usuarioId, input, nomeUsuarioAtor, idAtor) => ({
            id,
            usuarioId,
            input,
            nomeUsuarioAtor,
...
test("criarUsuario valida e delega o cadastro do usuario do tenant", async () => {
    const service = criarServiceComRepositorioStub();
    const resultado = await service.criarUsuario("instituicao-1", {
        nome_completo: "Usuário Teste",
        nome_usuario: "usuario.teste",
        email: "usuario@teste.org.br",
        senha: "Senha#123",
    ...
    }, "master@htasistemas.com.br", "7");
    assert.equal(resultado.id, "instituicao-1");
    assert.equal(resultado.input.nome_usuario, "usuario.teste");
...
test("atualizarUsuario valida e delega a edicao do usuario do tenant", async () => {
    const service = criarServiceComRepositorioStub();
    const resultado = await service.atualizarUsuario("instituicao-1", "usuario-1", {
    ...
    assert.equal(resultado.input.nome_usuario, "usuario.editado");
    assert.equal(resultado.nomeUsuarioAtor, "master@htasistemas.com.br");
    assert.equal(resultado.idAtor, "7");
});
test("resetarSenhaUsuario valida e delega a redefinicao da senha do usuario do tenant", async () => {
    const service = criarServiceComRepositorioStub();
    const resultado = await service.resetarSenhaUsuario("instituicao-1", "usuario-1", {
    ...
    assert.equal(resultado.id, "instituicao-1");
    assert.equal(resultado.usuarioId, "usuario-1");
    assert.equal(resultado.input.nova_senha, "SenhaNova#123");
    assert.equal(resultado.nomeUsuarioAtor, "master@htasistemas.com.br");
    assert.equal(resultado.idAtor, "7");
});
