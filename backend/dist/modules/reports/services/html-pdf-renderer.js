...
export class HtmlPdfRenderer {
    ...
    async renderEstruturado(layout, rodape) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const garantirEspaco = (alturaNecessaria) => {
                if (doc.y + alturaNecessaria <= bottomLimite())
                    return;
                doc.addPage();
            };
            ...
            const escreverParagrafo = (texto, config) => {
            ...
            const desenharTabela = () => {
                ...
                if (!tabela || tabela.colunas.length === 0)
                    return;
                const paddingX = 6;
                const paddingY = 4;
    ...
    async renderLegado(html, rodape) {
