...
portalInscricoesRoutes.post("/publico/:slug/pre-inscricoes/:protocolo/documentos", publico, documentoUpload, asyncHandler(async (req, res) => { const file = req.file; if (!file)
    throw new AppError("Selecione um documento para anexar.", 400); return res.status(201).json(await service.anexarDocumento(req.params.slug, req.params.protocolo, String(req.body?.cpf ?? ""), file)); }));
const interno = ["ADMINISTRADOR", "OPERADOR", "LEITURA_APENAS"];
portalInscricoesRoutes.get("/admin", ensureAuthenticated, ensurePermissions(interno), asyncHandler(async (req, res) => res.json({ preInscricoes: await service.adminList(String(req.authUser?.tenant_id), req.query) })));
portalInscricoesRoutes.get("/admin/resumo", ensureAuthenticated, ensurePermissions(interno), asyncHandler(async (req, res) => res.json({ resumo: await service.adminResumo(String(req.authUser?.tenant_id)) })));
portalInscricoesRoutes.get("/admin/:id", ensureAuthenticated, ensurePermissions(interno), asyncHandler(async (req, res) => res.json({ preInscricao: await service.adminDetalhe(req.params.id, String(req.authUser?.tenant_id)) })));
portalInscricoesRoutes.put("/admin/oportunidades/:id/publicacao", ensureAuthenticated, ensurePermissions(["ADMINISTRADOR", "OPERADOR"]), asyncHandler(async (req, res) => res.json({ oportunidade: await service.configurarPublicacao(req.params.id, publicacaoSchema.parse(req.body), String(req.authUser?.tenant_id)) })));
portalInscricoesRoutes.post("/admin/:id/:acao", ensureAuthenticated, ensurePermissions(["ADMINISTRADOR", "OPERADOR"]), asyncHandler(async (req, res) => { const parsed = acaoPreInscricaoSchema.parse(req.body ?? {}); return res.json(await service.acao(req.params.id, req.params.acao, parsed.motivo, { tenantId: String(req.authUser?.tenant_id), usuarioId: req.authUser?.id, usuarioNome: req.authUser?.nome, ip: req.ip })); }));
