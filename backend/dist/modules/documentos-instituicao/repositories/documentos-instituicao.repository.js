...
export class DocumentosInstituicaoRepository {
    ...
    async listarAnexos(documentoId, tenantId) {
        await this.garantirSchemaAnexos();
    ...
    async buscarAnexoPorId(documentoId, anexoId, tenantId) {
        await this.garantirSchemaAnexos();
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
    ...
    async buscarAnexoPorIdOuFalhar(documentoId, anexoId, tenantId) {
    ...
    async adicionarAnexo(documentoId, input, tenantId) {
        await this.garantirSchemaAnexos();
        await this.buscarPorIdOuFalhar(documentoId, tenantId);
        const inserted = await prisma.$queryRaw(Prisma.sql `
    ...
    async atualizarAnexo(documentoId, anexoId, input, tenantId) {
        await this.garantirSchemaAnexos();
    ...
    async excluirAnexo(documentoId, anexoId, tenantId) {
        await this.garantirSchemaAnexos();
    ...
    async existeHistoricoAlertaEmail(documentoId, observacao, tenantId) {
        await this.garantirEstrutura();
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT EXISTS (
        SELECT 1
    ...
    async listarHistorico(documentoId, tenantId) {
        await this.garantirEstrutura();
        await this.buscarPorIdOuFalhar(documentoId, tenantId);
        return prisma.$queryRaw(Prisma.sql `
      SELECT
    ...
    async adicionarHistorico(documentoId, input, tenantId) {
        await this.garantirEstrutura();
        await this.buscarPorIdOuFalhar(documentoId, tenantId);
