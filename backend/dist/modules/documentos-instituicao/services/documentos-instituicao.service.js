...
export class DocumentosInstituicaoService {
    ...
    async adicionarAnexo(rawDocumentoId, rawInput, rawUsuarioId) {
        ...
        try {
            ...
            await this.registrarHistoricoAutomatico(documentoId, input.usuario, "Anexo enviado", `Arquivo ${anexo.nome_arquivo} anexado ao documento.`);
    ...
    async substituirAnexo(rawDocumentoId, rawAnexoId, rawInput, rawUsuarioId) {
        const documentoId = this.parseId(rawDocumentoId);
        ...
        try {
            ...
            await this.registrarHistoricoAutomatico(documentoId, input.usuario, "Anexo substituido", `Arquivo ${anexoAnterior.nome_arquivo} substituido por ${anexo.nome_arquivo}.`);
            return mapDocumentoInstituicaoAnexoToResponse(anexo);
        }
        catch (error) {
            await storageService.rollbackArquivos([arquivo.registro ? arquivo.caminhoArquivo : undefined]);
    ...
    async excluirAnexo(rawDocumentoId, rawAnexoId, rawUsuarioId) {
        const documentoId = this.parseId(rawDocumentoId);
        const anexoId = this.parseId(rawAnexoId);
        const usuarioId = this.parseIdOpcional(rawUsuarioId);
        ...
        await this.registrarHistoricoAutomatico(documentoId, "Sistema", "Anexo excluido", `Arquivo ${anexo.nome_arquivo} removido do documento.`);
    }
    ...
    async processarAlertasEmailPendentes(documentoIds) {
        ...
        const registros = documentoIds?.length
            ? (await Promise.all(documentoIds.map((id) => this.repository.buscarPorId(id)))).filter(Boolean)
        ...
        for (const documento of documentos) {
            ...
            const jaEnviado = await this.repository.existeHistoricoAlertaEmail(documentoId, observacaoHistorico);
            if (!jaEnviado) {
        ...
        for (const item of pendentes) {
            await this.registrarHistoricoAutomatico(item.documentoId, "Sistema", "Alerta por e-mail", item.observacaoHistorico);
        }
        return {
            emailsEnviados: 1,
    ...
    async registrarHistoricoAutomatico(documentoId, usuario, tipoAlteracao, observacao) {
        try {
    ...
    dispararProcessamentoAlertaEmail(documentoIds) {
