...
import { deveEnviarAlertaEmailDocumento, montarMensagemAlertaEmailDocumentos, montarObservacaoHistoricoAlertaEmailDocumento } from "../documentos-instituicao.alertas.js";
test("deveEnviarAlertaEmailDocumento considera apenas documentos com alerta ativo e status critico", () => {
    ...
    assert.equal(deveEnviarAlertaEmailDocumento({ gerarAlerta: true, situacao: "vence_em_breve" }), true);
...
test("montarObservacaoHistoricoAlertaEmailDocumento gera texto deterministico por data e situacao", () => {
    assert.equal(montarObservacaoHistoricoAlertaEmailDocumento({ situacao: "vencido", validade: "2026-03-10" }, "2026-03-18"), "Alerta automatico de documento vencido enviado em 18-03-2026. Validade registrada: 10-03-2026.");
});
test("montarMensagemAlertaEmailDocumentos agrega os documentos no corpo do email", () => {
    const mensagem = montarMensagemAlertaEmailDocumentos("Casa Sede", [
        {
            id: "1",
