...
export class OficiosController {
    ...
    async obterContextoDocumento(request, response) {
    ...
    async criar(request, response) {
        const oficio = await service.criar(request.body, request.authUser?.id, request.authUser?.tenant_id);
        return response.status(201).json(oficio);
    }
    async atualizar(request, response) {
        const oficio = await service.atualizar(request.params.id, request.body, request.authUser?.id, request.authUser?.tenant_id);
        return response.json(oficio);
    }
    ...
    async salvarPdfAssinado(request, response) {
        const oficio = await service.salvarPdfAssinado(request.params.id, request.body, request.authUser?.id, request.authUser?.tenant_id);
        return response.json(oficio);
    }
    ...
    async removerPdfAssinado(request, response) {
        await service.removerPdfAssinado(request.params.id, request.authUser?.id, request.authUser?.tenant_id);
        return response.status(204).send();
    }
    ...
    async adicionarImagem(request, response) {
        const imagem = await service.adicionarImagem(request.params.id, request.body, request.authUser?.id, request.authUser?.tenant_id);
        return response.status(201).json(imagem);
    }
    async removerImagem(request, response) {
        await service.removerImagem(request.params.id, request.params.imagemId, request.authUser?.id, request.authUser?.tenant_id);
        return response.status(204).send();
    }
