...
function normalizarSalas(salas) {
    ...
    for (const sala of salas) {
        ...
        if ((chaveId && idsUnicos.has(chaveId)) || nomesUnicos.has(chaveNome))
            continue;
        if (chaveId)
            idsUnicos.add(chaveId);
...
export class UnidadeAssistencialRepository {
    async listar(filters, tenantId, contexto) {
    ...
    async buscarPorIdDoTenant(id, tenantId) {
        if (!tenantId) {
    ...
    async buscarPorIdOuFalhar(id, tenantId, contexto) {
    ...
    async buscarAtual(tenantId, contexto) {
      ...
      `, tenantId);
        ...
        if (!unidadeId)
            return null;
        if (contexto && !(await this.unidadePermitidaNoContexto(unidadeId, tenantId, contexto)))
            return null;
        return prisma.unidadeAssistencial.findUnique({
            where: { id: unidadeId },
            include: unidadeInclude
          ...
          `, tenantId, id, now);
            ...
            await tx.unidadeAssistencial.update({
                ...
                data: {
                    ...
                    accuracyMaxPontoMetros: input.accuracy_max_ponto_metros ?? existing.accuracyMaxPontoMetros,
                    ipValidacaoPonto: trimOrUndefined(input.ip_validacao_ponto) ?? existing.ipValidacaoPonto,
                    ipsPublicosPonto: trimOrUndefined(input.ips_publicos_ponto) ?? existing.ipsPublicosPonto,
                    redesLocaisPonto: trimOrUndefined(input.redes_locais_ponto) ?? existing.redesLocaisPonto,
                    modoValidacaoPonto: trimOrUndefined(input.modo_validacao_ponto) ?? existing.modoValidacaoPonto,
            ...
            if (input.salas) {
                await this.garantirColunaSalaAtivo(tx);
                const salas = normalizarSalas(input.salas);
                const idsRecebidos = salas.map((sala) => sala.id).filter((salaId) => !!salaId);
                const salasExistentes = existing.salas ?? [];
                const salasRemovidas = salasExistentes.filter((sala) => !idsRecebidos.some((salaId) => salaId === sala.id));
                for (const sala of salasRemovidas) {
                    const possuiVinculo = await this.salaPossuiVinculo(sala.id, tx);
                    if (possuiVinculo) {
                        throw new AppError(`A sala "${sala.nome}" possui vínculo de uso no sistema. Não é possível remover; inative a sala.`, 409);
                    }
    ...
    async verificarVinculosSala(salaId, tenantId) {
        await this.garantirColunaSalaAtivo();
        const rows = await prisma.$queryRawUnsafe(`
      SELECT s.id
    ...
    async garantirColunaSalaAtivo(db = prisma) {
        await db.$executeRawUnsafe("ALTER TABLE salas_unidade ADD COLUMN IF NOT EXISTS ativo BOOLEAN NOT NULL DEFAULT TRUE");
        await db.$executeRawUnsafe("ALTER TABLE salas_unidade ADD COLUMN IF NOT EXISTS capacidade_maxima INTEGER NOT NULL DEFAULT 0");
    }
    async criarSala(tx, unidadeId, sala, now) {
        await tx.$executeRawUnsafe(`
      INSERT INTO salas_unidade (unidade_id, nome, capacidade_maxima, ativo, criado_em, atualizado_em)
      VALUES ($1, $2, $3, $4, $5, $6)
      `, unidadeId, sala.nome, sala.capacidade_maxima, sala.ativo, now, now);
    }
    async atualizarSala(tx, unidadeId, sala, now) {
        await tx.$executeRawUnsafe(`
      ...
      WHERE id = $1
        AND unidade_id = $2
      `, sala.id, unidadeId, sala.nome, sala.capacidade_maxima, sala.ativo, now);
    }
    async anexarStatusSalas(entrada, db = prisma) {
      ...
      `, ids.map((id) => id.toString()));
        ...
        for (const unidade of unidades) {
            for (const sala of unidade.salas ?? []) {
                sala.ativo = statusPorId.get(sala.id.toString()) ?? true;
            }
        }
        return entrada;
    }
    async salaPossuiVinculo(salaId, db = prisma) {
        return (await this.contarVinculosSala(salaId, db)) > 0;
    }
    async contarVinculosSala(salaId, db = prisma) {
        const tabelaRows = await db.$queryRawUnsafe("SELECT to_regclass('public.cursos_atendimentos')::text AS existe");
        if (!tabelaRows[0]?.existe)
            return 0;
        const rows = await db.$queryRawUnsafe(`
    ...
    async listarIdsPorTenant(filters, tenantId, contexto) {
        ...
        if (nome) {
            params.push(`%${nome}%`);
            condicoes.push(`(COALESCE(ua.nome_fantasia, '') ILIKE $${params.length} OR COALESCE(ua.razao_social, '') ILIKE $${params.length})`);
        }
        const cnpj = normalizeDigits(filters.cnpj);
        if (cnpj) {
            params.push(`%${cnpj}%`);
    ...
    async unidadePertenceAoTenant(id, tenantId, db = prisma) {
        const rows = await db.$queryRawUnsafe(`
      SELECT id
      FROM unidade_assistencial
      WHERE id = $1
    ...
    async unidadePermitidaNoContexto(id, tenantId, contexto) {
        if (contexto.unidade_id) {
            const rows = await prisma.$queryRawUnsafe(`SELECT ua.id FROM unidade_assistencial ua JOIN unidades_organizacionais uo ON uo.unidade_assistencial_id = ua.id
        ...
        if (contexto.entidade_juridica_id) {
            const rows = await prisma.$queryRawUnsafe(`SELECT id FROM unidade_assistencial WHERE id = $1 AND tenant_id::text = $2 AND entidade_juridica_id = $3::bigint LIMIT 1`, id, tenantId, contexto.entidade_juridica_id);
            return !!rows[0];
        }
        return true;
    }
