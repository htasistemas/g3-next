...
export class CarteiraEventoRepository {
    async listarEventos(filters, tenantId) {
    ...
    async listarParticipantes(filters, tenantId) {
        await ensureCarteiraEventoEstrutura(prisma);
        const where = [];
    ...
    async buscarParticipantePorIdOuFalhar(id, tenantId, db = prisma) {
        await ensureCarteiraEventoEstrutura(prisma);
    ...
    async criarParticipante(input, tenantId) {
        await ensureCarteiraEventoEstrutura(prisma);
        const evento = await this.buscarEventoConfigPorIdOuFalhar(BigInt(input.evento_id), prisma, tenantId);
        const numeroCarteira = trimOrUndefined(input.numero_carteira) ?? (await this.gerarNumeroCarteira(BigInt(input.evento_id), tenantId));
    ...
    async atualizarParticipante(id, input, tenantId) {
        await ensureCarteiraEventoEstrutura(prisma);
        ...
        const numeroCarteira = trimOrUndefined(input.numero_carteira) ?? (await this.gerarNumeroCarteira(BigInt(input.evento_id), tenantId, id));
        await prisma.$executeRaw(Prisma.sql `
    ...
    async listarBarracas(filters, tenantId) {
        await ensureCarteiraEventoEstrutura(prisma);
        const where = [];
        if (filters.evento_id) {
    ...
    async criarBarraca(input, tenantId) {
        await ensureCarteiraEventoEstrutura(prisma);
        await this.buscarEventoConfigPorIdOuFalhar(BigInt(input.evento_id), prisma, tenantId);
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async atualizarBarraca(id, input, tenantId) {
        await ensureCarteiraEventoEstrutura(prisma);
    ...
    async listarItens(filters, tenantId) {
        await ensureCarteiraEventoEstrutura(prisma);
        const where = [];
        if (filters.evento_id) {
    ...
    async criarItem(input, tenantId) {
        await ensureCarteiraEventoEstrutura(prisma);
        await this.buscarEventoConfigPorIdOuFalhar(BigInt(input.evento_id), prisma, tenantId);
        if (input.barraca_id) {
    ...
    async atualizarItem(id, input, tenantId) {
        await ensureCarteiraEventoEstrutura(prisma);
    ...
    async consultarToken(eventoId, token, tenantId) {
        await ensureCarteiraEventoEstrutura(prisma);
    ...
    async realizarVenda(input, ator) {
      ...
      `);
            ...
            const participante = await this.buscarParticipantePorTokenOuFalhar(evento.id, input.token.trim(), tx, tenantId, true);
            if (participante.status !== "ATIVO") {
    ...
    async listarExtrato(filters, tenantId) {
        await ensureCarteiraEventoEstrutura(prisma);
    ...
    async obterFechamento(filters, tenantId) {
    ...
    async obterRelatorio(filters, tenantId) {
        const eventoId = BigInt(filters.evento_id);
    ...
    async buscarEventoPorIdOuFalhar(id, tenantId) {
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        nome_evento,
    ...
    async buscarEventoConfigPorIdOuFalhar(id, tx = prisma, tenantId) {
    ...
    async buscarBarracaPorIdOuFalhar(id, tenantId) {
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT
        b.id,
        b.evento_id,
    ...
    async buscarBarracaOperacaoPorIdOuFalhar(id, tx, tenantId) {
        const rows = await tx.$queryRaw(Prisma.sql `
    ...
    async buscarItemPorIdOuFalhar(id, tenantId) {
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT
        i.id,
        i.evento_id,
    ...
    async buscarParticipanteSaldoPorIdOuFalhar(id, tx = prisma, tenantId, forUpdate = false) {
    ...
    async buscarParticipantePorTokenOuFalhar(eventoId, token, tx, tenantId, forUpdate = false) {
        const identificador = token.trim();
        const rows = await tx.$queryRaw(Prisma.sql `
      SELECT id, evento_id, nome, status, saldo_atual, qr_code_token_unico
      FROM carteira_evento_participante
    ...
    async gerarNumeroCarteira(eventoId, tenantId, ignorarId) {
        const rows = await prisma.$queryRaw(Prisma.sql `
