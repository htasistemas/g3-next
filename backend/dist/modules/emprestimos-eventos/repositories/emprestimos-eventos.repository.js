...
import { adicionarDiasLocal, calcularDiaDevolucaoApoio, calcularDiaRetiradaApoio, formatDateLocal, formatDateTimeLocal, inicioDoDiaLocal, parseDateTimeLocal } from "../emprestimos-eventos-datetime.js";
...
function toDbNullableText(value) {
...
export class EmprestimosEventosRepository {
    ...
    async listarEmprestimos(filtros, tenantId) {
        await this.ensureEstrutura();
        const where = [tenantSql("e", tenantId), tenantSql("ev", tenantId)];
    ...
    async buscarEmprestimoPorId(id, tenantId) {
        await this.ensureEstrutura();
        const registros = await prisma.$queryRaw(Prisma.sql `
      SELECT
        e.id,
    ...
    async buscarEmprestimoPorIdOuFalhar(id, tenantId) {
    ...
    async criarEmprestimo(input, tenantId) {
      ...
      `);
            ...
            await this.registrarMovimentacao(tx, emprestimoId, "CRIACAO", "Emprestimo criado.", tenantId);
            return emprestimoId;
        });
    ...
    async atualizarEmprestimo(id, input, tenantId) {
      ...
      `);
            ...
            await this.registrarMovimentacao(tx, id, "ATUALIZACAO", "Emprestimo atualizado.", tenantId);
        });
        return this.buscarEmprestimoPorIdOuFalhar(id, tenantId);
    }
    ...
    async alterarStatus(id, status, tenantId, usuarioId) {
        await this.ensureEstrutura();
        const atual = await this.buscarEmprestimoPorIdOuFalhar(id, tenantId);
        this.validarTransicaoStatus(atual.registro.status, status);
        await prisma.$transaction(async (tx) => {
        ...
        WHERE id = ${id}
          AND tenant_id::text = ${tenantId}
      `);
            await this.registrarMovimentacao(tx, id, `STATUS_${status}`, `Status alterado para ${status}.`, tenantId, usuarioId);
        });
        return this.buscarEmprestimoPorIdOuFalhar(id, tenantId);
    }
    async listarEventos(tenantId) {
        await this.ensureEstrutura();
    ...
    async listarResponsaveis(tenantId) {
        await this.ensureEstrutura();
        return prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
    ...
    async buscarResponsavelPorId(id, tenantId) {
        await this.ensureEstrutura();
    ...
    async criarResponsavel(input, tenantId) {
        await this.ensureEstrutura();
    ...
    async atualizarResponsavel(id, input, tenantId) {
        await this.ensureEstrutura();
    ...
    async excluirResponsavel(id, tenantId) {
        await this.ensureEstrutura();
        const atual = await this.buscarResponsavelPorId(id, tenantId);
        if (!atual) {
            throw new AppError("Responsavel nao encontrado.", 404);
    ...
    async buscarEventoPorId(id, tenantId) {
        await this.ensureEstrutura();
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
    ...
    async criarEvento(input, tenantId) {
        await this.ensureEstrutura();
        const inserted = await prisma.$queryRaw(Prisma.sql `
    ...
    async atualizarEvento(id, input, tenantId) {
        await this.ensureEstrutura();
    ...
    async excluirEvento(id, tenantId) {
        await this.ensureEstrutura();
        const evento = await this.buscarEventoPorId(id, tenantId);
        if (!evento) {
            throw new AppError("Evento nao encontrado.", 404);
    ...
    async listarAgendaResumo(inicio, fim, tenantId) {
        await this.ensureEstrutura();
    ...
    `);
        ...
        const registrarDia = (data, tipo, registro) => {
            const dia = formatDateLocal(data) ?? "";
            if (!dia || dia < formatDateLocal(inicio) || dia > formatDateLocal(fim))
                return;
    ...
    async listarAgendaDia(data, tenantId) {
        await this.ensureEstrutura();
    ...
    `);
        const registrosDoDia = registros.filter((registro) => {
            ...
            return ((formatDateLocal(inicioEvento) <= dia && formatDateLocal(fimEvento) >= dia) ||
        ...
        return registrosDoDia.map((registro) => {
            ...
            const devolucaoApoio = calcularDiaDevolucaoApoio(fimEvento, feriados);
            const dia = diaSelecionado ?? "";
            const tipoDia = formatDateLocal(retiradaApoio) === dia
                ? "RETIRADA"
                : formatDateLocal(devolucaoApoio) === dia
                    ? "DEVOLUCAO"
                    : "EVENTO";
    ...
    async consultarDisponibilidade(input, tenantId) {
        await this.ensureEstrutura();
    ...
    async listarMovimentacoes(emprestimoId, tenantId) {
        await this.ensureEstrutura();
    ...
    async obterNomeInstituicao(tenantId) {
        await this.ensureEstrutura();
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async validarEventoExiste(tx, eventoId, tenantId) {
    ...
    async validarResponsavelExiste(tx, responsavelId, tenantId) {
        const rows = await tx.$queryRaw(Prisma.sql `
      SELECT id
      FROM emprestimos_eventos_responsaveis
    ...
    async listarItensPorEmprestimos(emprestimoIds, tenantId) {
    ...
    async salvarItens(tx, emprestimoId, itens, tenantId) {
    ...
    async listarFeriadosPeriodo(inicio, fim) {
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT data
      FROM feriados
    ...
    async validarItensEmprestimo(tx, itens, contexto, tenantId) {
        ...
        for (const item of agrupados.values()) {
            ...
            if (!disponibilidade.disponivel) {
                throw new AppError(`Item indisponivel para o periodo informado: ${item.tipoItem} ${item.itemId}.`, 409);
            }
        }
    }
    async validarItemExiste(tx, itemId, tipoItem, tenantId) {
    ...
    async registrarMovimentacao(tx, emprestimoId, acao, descricao, tenantId, usuarioId) {
        await tx.$executeRaw(Prisma.sql `
    ...
    validarTransicaoStatus(statusAtual, proximoStatus) {
