...
export class ImportacaoDadosRepository {
    ...
    async listarInstituicoes(busca) {
    ...
    async obter(id) {
        await this.garantirEstrutura();
        const rows = await prisma.$queryRaw `SELECT *, id::text FROM importacao_dados WHERE id = ${id}::uuid LIMIT 1`;
        return rows[0];
    }
    async atualizar(id, status, linhas, counts) {
        await this.garantirEstrutura();
    ...
    async listarHistorico() {
        await this.garantirEstrutura();
        return prisma.$queryRaw `SELECT id::text, tipo, instituicao_nome, instituicao_cnpj, usuario_master_nome, nome_arquivo, total_registros, prontos, existentes, duplicidades, erros, ignorados, status, criado_em, processado_em FROM importacao_dados ORDER BY criado_em DESC LIMIT 100`;
    }
}
