...
function chave(valor) { return valor.normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().toLowerCase().replace(/[._-]+/g, " ").replace(/\s+/g, " "); }
function valorTexto(valor) { return valor === null || valor === undefined ? "" : String(valor).trim(); }
function removerAcentos(valor) {
    return typeof valor === "string" ? valor.normalize("NFD").replace(/[\u0300-\u036f]/g, "") : valor;
}
function normalizarTextoImportado(dados) {
...
function normalizarBool(valor) { return ["sim", "s", "true", "1", "yes"].includes(chave(valorTexto(valor))); }
function cadastroEstaCompleto(dados) {
...
function sugestaoMapeamento(colunas) { return Object.fromEntries(colunas.map((coluna) => [coluna, ALIASES[chave(coluna)] ?? ""])); }
function nomeInstituicao(instituicao) { return instituicao.nome_fantasia?.trim() || instituicao.razao_social; }
export class ImportacaoDadosService {
    repository = new ImportacaoDadosRepository();
    beneficiarioService = new BeneficiarioService();
    listarInstituicoes(busca) { return this.repository.listarInstituicoes(busca); }
    listarHistorico() { return this.repository.listarHistorico(); }
    async validarArquivo(input) {
        const instituicao = await this.repository.obterInstituicao(input.instituicaoId);
        if (!instituicao)
            throw new AppError("Instituição de destino não encontrada.", 404);
        if (!['ativo', 'ATIVA', 'ativa'].includes(instituicao.status))
            throw new AppError("A instituição de destino não está ativa.", 422);
        ...
        for (let index = 0; index < parsed.linhas.length; index += 1) {
            ...
            const dados = { status: "EM_ANALISE", aceite_lgpd: undefined };
            for (const [coluna, campo] of Object.entries(mapeamento)) {
                if (campo && CAMPOS.has(campo))
                    dados[campo] = valorTexto(original[coluna]);
            }
            dados.data_nascimento = normalizarData(dados.data_nascimento);
            if (dados.cpf)
                dados.cpf = normalizeDigits(valorTexto(dados.cpf));
            if (dados.cep)
                dados.cep = normalizeDigits(valorTexto(dados.cep));
            for (const campo of ["telefone_principal", "telefone_secundario", "telefone_recado_numero"])
                if (dados[campo])
                    dados[campo] = normalizeDigits(valorTexto(dados[campo]));
            for (const campo of ["opta_receber_cesta_basica", "apto_receber_cesta_basica", "telefone_principal_whatsapp", "permite_contato_tel", "permite_contato_whatsapp", "permite_contato_sms", "permite_contato_email", "mora_com_familia", "responsavel_legal", "acompanhamento_cras", "acompanhamento_saude", "sabe_ler_escrever", "estuda_atualmente", "possui_deficiencia", "usa_medicacao_continua", "recebe_beneficio", "aceite_lgpd"])
                if (dados[campo] !== undefined && dados[campo] !== "")
                    dados[campo] = normalizarBool(dados[campo]);
            for (const campo of ["criancas_adolescentes", "idosos"])
                if (dados[campo] !== undefined && dados[campo] !== "")
                    dados[campo] = Number(valorTexto(dados[campo]).replace(",", "."));
            if (typeof dados.beneficios_recebidos === "string")
                dados.beneficios_recebidos = valorTexto(dados.beneficios_recebidos).split(/[;,]/).map((item) => item.trim()).filter(Boolean);
            if (typeof dados.aceite_lgpd === "string")
                dados.aceite_lgpd = normalizarBool(dados.aceite_lgpd);
            const problemas = [];
            const camposObrigatorios = [["nome_completo", "Nome completo"], ["data_nascimento", "Data de nascimento"], ["nome_mae", "Nome da mãe"], ["cep", "CEP"], ["telefone_principal", "Telefone principal"], ["cpf", "CPF"]];
            for (const [campo, rotulo] of camposObrigatorios)
                if (!trimOrUndefined(valorTexto(dados[campo])))
                    problemas.push({ campo, mensagem: `${rotulo} não informado.`, orientacao: `Mapeie ou informe o campo ${rotulo}.` });
            if (dados.cpf && !isValidCpf(valorTexto(dados.cpf)))
                problemas.push({ campo: "cpf", valor: valorTexto(dados.cpf), mensagem: "CPF inválido.", orientacao: "Informe 11 dígitos válidos." });
            if (dados.cep && !isValidCep(valorTexto(dados.cep)))
                problemas.push({ campo: "cep", valor: valorTexto(dados.cep), mensagem: "CEP inválido.", orientacao: "Informe 8 dígitos." });
            if (dados.telefone_principal && !isValidPhone(valorTexto(dados.telefone_principal)))
                problemas.push({ campo: "telefone_principal", valor: valorTexto(dados.telefone_principal), mensagem: "Telefone inválido.", orientacao: "Informe DDD e número com 10 ou 11 dígitos." });
            const parsedInput = beneficiarioInputSchema.safeParse(dados);
            if (!parsedInput.success)
                for (const issue of parsedInput.error.issues)
                    if (issue.message !== "Required" && !problemas.some((item) => item.campo === issue.path[0]))
                        problemas.push({ campo: String(issue.path[0] ?? ""), valor: valorTexto(dados[String(issue.path[0] ?? "")]), mensagem: issue.message, orientacao: "Revise o valor ou o mapeamento da coluna." });
            ...
            let status = problemas.length ? "INCOMPLETO" : "PRONTO";
            ...
            if (cpfNormalizado && isValidCpf(cpfNormalizado)) {
                ...
                const encontrados = await prisma.$queryRaw `
          SELECT b.id
        ...
        `;
                if (encontrados.length) {
                    status = "EXISTENTE";
                    beneficiarioId = String(encontrados[0].id);
                }
            }
            ...
            if (!beneficiarioId && nomeParaDuplicidade && /^\d{4}-\d{2}-\d{2}$/.test(dataParaDuplicidade)) {
                const possiveis = await prisma.$queryRaw `
        ...
        `;
                if (possiveis.length) {
                    status = "DUPLICIDADE";
                    beneficiarioId = String(possiveis[0].id);
                }
            }
    ...
    async confirmar(id, rawAcoes, rawCorrecoes, usuarioId) {
        const registro = await this.repository.obter(id);
        if (!registro)
            throw new AppError("Importação não encontrada.", 404);
        if (registro.usuario_master_id && String(registro.usuario_master_id) !== usuarioId)
            throw new AppError("Somente o MASTER responsável pode confirmar esta importação.", 403);
        const statusAtual = String(registro.status ?? "");
        if (statusAtual === "PROCESSANDO")
            return { id, status: statusAtual, resumo: this.resumo(registro.linhas), linhas: registro.linhas };
        await this.repository.atualizar(id, "PROCESSANDO", registro.linhas, this.resumo(registro.linhas));
        void this.processarConfirmacao(id, rawAcoes, rawCorrecoes, usuarioId).catch(async (error) => {
            const linhas = (await this.repository.obter(id))?.linhas;
            if (linhas)
                await this.repository.atualizar(id, "FALHOU", linhas, this.resumo(linhas));
            console.error("Falha no processamento assíncrono da importação", error);
        });
        return { id, status: "PROCESSANDO", resumo: this.resumo(registro.linhas), linhas: registro.linhas };
    }
    async processarConfirmacao(id, rawAcoes, rawCorrecoes, usuarioId) {
        const registro = await this.repository.obter(id);
        if (!registro)
            throw new AppError("Importação não encontrada.", 404);
        if (registro.usuario_master_id && String(registro.usuario_master_id) !== usuarioId)
        ...
        if (!instituicao || instituicao.tenant_id !== String(registro.tenant_id))
            throw new AppError("A instituição da importação não é válida.", 422);
        const acoes = rawAcoes && typeof rawAcoes === "object" ? rawAcoes : {};
        const correcoes = rawCorrecoes && typeof rawCorrecoes === "object" ? rawCorrecoes : {};
        const linhas = registro.linhas.map((linha) => ({ ...linha, dados: { ...linha.dados, ...(correcoes[String(linha.linha)] ?? {}) }, acao: acoes[String(linha.linha)], alteradoManualmente: Boolean(correcoes[String(linha.linha)]) }));
        ...
        for (let inicio = 0; inicio < linhas.length; inicio += 50) {
            ...
            for (const linha of lote) {
                if (linha.acao === "IGNORAR" || linha.status === "DUPLICIDADE" || linha.status === "EXISTENTE" && linha.acao !== "ATUALIZAR") {
                ...
                try {
                    if (["INCOMPLETO", "INVALIDO"].includes(linha.status)) {
                        ...
                        if (linha.acao === "ATUALIZAR" && linha.beneficiarioId) {
                            await this.beneficiarioService.atualizar(linha.beneficiarioId, parsed, usuarioId, instituicao.tenant_id);
                            linha.status = "ATUALIZADO";
                            atualizados += 1;
                        }
                        else {
                            await this.beneficiarioService.criar(parsed, usuarioId, instituicao.tenant_id);
                            linha.status = "IMPORTADO";
                            importados += 1;
                        }
                    }
                    ...
                    linha.status = "ERRO";
                    linha.problemas = [{ mensagem: error instanceof Error ? error.message : "Falha ao processar a linha." }];
                }
            }
            await this.repository.atualizar(id, "PROCESSANDO", linhas, this.resumo(linhas));
        }
        const resumo = this.resumo(linhas);
        const status = resumo.erros ? (importados || atualizados ? "CONCLUIDA_COM_PENDENCIAS" : "FALHOU") : "CONCLUIDA";
    ...
    async obter(id) { const registro = await this.repository.obter(id); if (!registro)
        throw new AppError("Importação não encontrada.", 404); return registro; }
    resumo(linhas) { return { prontos: linhas.filter((l) => l.status === "PRONTO").length, existentes: linhas.filter((l) => l.status === "EXISTENTE").length, duplicidades: linhas.filter((l) => l.status === "DUPLICIDADE").length, erros: linhas.filter((l) => ["ERRO", "INVALIDO", "INCOMPLETO"].includes(l.status)).length, ignorados: linhas.filter((l) => l.status === "IGNORADO").length }; }
}
