...
export class ImportacaoDadosController {
    async instituicoes(request, response) { return response.json({ instituicoes: await service.listarInstituicoes(typeof request.query.busca === "string" ? request.query.busca : undefined) }); }
    async validar(request, response) {
        if (!request.file)
            throw new AppError("Selecione um arquivo para validar.", 400);
    ...
    async obter(request, response) { return response.json({ importacao: await service.obter(request.params.id) }); }
    async confirmar(request, response) { return response.json(await service.confirmar(request.params.id, request.body.acoes, request.body.correcoes, request.authUser?.id ?? "")); }
    async historico(_request, response) { return response.json({ importacoes: await service.listarHistorico() }); }
    async relatorio(request, response) { const registro = await service.obter(request.params.id); const csv = gerarRelatorioCsv(registro.linhas); response.setHeader("Content-Type", "text/csv; charset=utf-8"); response.setHeader("Content-Disposition", `attachment; filename="relatorio-importacao-${request.params.id}.csv"`); return response.send(csv); }
}
