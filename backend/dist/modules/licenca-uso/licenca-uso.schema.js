...
const emailArraySchema = z.preprocess((value) => {
    if (!Array.isArray(value))
        return [];
    return value;
}, z.array(z.string().trim().min(1)).max(10)).transform((items) => items.map((item) => normalizarEmail(item)).filter(Boolean));
...
export const licencaUsoConfiguracaoSchema = z.object({
    ...
    vigenciaInicialDias: z.number().int().min(1).max(3650).optional(),
...
export const atualizarLicencaUsoPayloadSchema = z.object({
    configuracao: z.object({
        ...
        vigenciaInicialDias: optionalInteger.optional(),
