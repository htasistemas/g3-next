...
function textoDaQuery(query) {
    if (typeof query === "string")
        return query;
    if (!query || typeof query !== "object")
        return "";
    const valor = query;
    if (typeof valor.sql === "string")
        return valor.sql;
    if (typeof valor.text === "string")
        return valor.text;
    return String(query);
}
function criarConfiguracaoBase() {
    return {
...
test("salvarConfiguracao e registrarPagamentoPendente usam cast de data nas vigencias", async () => {
    ...
    prismaMock.$executeRawUnsafe = async (query) => {
        queries.push(textoDaQuery(query));
        return 0;
    };
    prismaMock.$executeRaw = async (query) => {
        queries.push(textoDaQuery(query));
        return 0;
    };
    prismaMock.$queryRawUnsafe = async (query) => {
        queries.push(textoDaQuery(query));
