...
import { formatarEndereco, formatarNomeInstituicao, formatarNomePessoa, formatarTextoCurto, normalizarEspacos } from "../../utils/text-formatter.js";
...
export function normalizarPlanoTrabalhoInput(input) {
    return {
        ...
        responsavelColetaDados: input.responsavelColetaDados
            ...
            : undefined,
        instrumentosMonitoramento: uniq(input.instrumentosMonitoramento ?? []).map((item) => formatarTextoCurto(item)),
...
export function validarConformidadePlano(input) {
    const erros = [];
    if (!input.metas.length)
        erros.push("Cadastre ao menos uma meta antes de enviar para análise.");
...
export function garantirConformidadeParaEnvio(input) {
