...
export class PlanosTrabalhoRepository {
    ...
    async criar(input) {
      ...
      `);
            ...
            if (!planoId)
                throw new AppError("Nao foi possivel criar plano de trabalho.", 500);
    ...
    async listarAtividades(metaIds) {
        return prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        meta_id,
    ...
    async listarEtapas(atividadeIds) {
        return prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        atividade_id,
    ...
    async listarCronograma(planosIds) {
        return prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        plano_trabalho_id,
    ...
    async inserirMetas(tx, planoId, metas) {
        for (let metaIndex = 0; metaIndex < metas.length; metaIndex += 1) {
            ...
            const insertedMeta = await tx.$queryRaw(Prisma.sql `
    ...
    async inserirAtividades(tx, metaId, atividades) {
      ...
      `);
            ...
            if (!atividadeId)
                throw new AppError("Nao foi possivel salvar atividade da meta.", 500);
            await this.inserirEtapas(tx, atividadeId, atividade.etapas ?? []);
        }
    }
    async inserirEtapas(tx, atividadeId, etapas) {
    ...
    async inserirCronograma(tx, planoId, cronograma) {
    ...
    async inserirEquipe(tx, planoId, equipe) {
