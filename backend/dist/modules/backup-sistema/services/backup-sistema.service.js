...
function criarIdBackup(tipo) {
    return `${tipo.toLowerCase()}-${new Date().toISOString().replace(/[-:]/g, "").replace(/\..+/, "")}-${randomUUID().slice(0, 8)}`;
}
function normalizarErroComando(error, comando) {
    if (error instanceof Error) {
        const detalhes = error.message.trim();
        return new AppError(detalhes ? `Falha ao executar ${comando}: ${detalhes}` : `Falha ao executar ${comando}.`, 500);
    }
    return new AppError(`Falha ao executar ${comando}.`, 500);
}
function garantirDentroDaRaiz(raiz, alvo) {
...
async function lerMetadata(caminho) {
    const conteudo = await readFile(caminho, "utf-8");
    return JSON.parse(conteudo);
}
async function salvarMetadata(caminho, metadata) {
...
export class BackupSistemaService {
    ...
    async executarComando(comando, args, options) {
        try {
    ...
    async listarDiretorioBackups(tipo) {
    ...
    async obterBackupPorId(backupId) {
    ...
    async atualizarMetadataRestauracao(backup, usuario) {
    ...
    async criarBackupBancoEm(pastaDestino, usuarioResponsavel) {
    ...
    async criarBackupImagensEm(pastaDestino, usuarioResponsavel) {
    ...
    async gerarBackupBanco(usuarioResponsavel) {
    ...
    async gerarBackupImagens(usuarioResponsavel) {
    ...
    async restaurarBackupBanco(rawPayload, usuarioResponsavel) {
    ...
    async restaurarBackupImagens(rawPayload, usuarioResponsavel) {
        const payload = backupSistemaRestaurarSchema.parse(rawPayload ?? {});
        const backup = await this.obterBackupPorId(payload.backupId);
    ...
    async baixarBackup(backupId) {
        ...
        return {
            backup,
            contentType: backup.tipo === "BANCO" ? "application/octet-stream" : "application/gzip",
