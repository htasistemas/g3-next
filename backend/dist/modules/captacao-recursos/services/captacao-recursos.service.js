...
import { mapCaptacaoCampanha, mapCaptacaoComprovante, mapCaptacaoConfiguracoes, mapCaptacaoDoacao, mapCaptacaoDoador, mapCaptacaoLog, mapCaptacaoRecorrencia, mapCaptacaoTarefaRelacionamento } from "../captacao-recursos.mapper.js";
import { captacaoCampanhaInputSchema, captacaoConfiguracoesSchema, captacaoDoacaoInputSchema, captacaoDoadorInputSchema, captacaoListFiltersSchema, captacaoPortalLoginSchema, captacaoPortalTokenSchema, captacaoTarefaRelacionamentoInputSchema } from "../captacao-recursos.schema.js";
...
function parseTenant(rawTenantId) {
...
export class CaptacaoRecursosService {
    ...
    async getDashboard(rawFilters, rawTenantId) {
    ...
    async listDoadores(rawFilters, rawTenantId) {
        const filters = captacaoListFiltersSchema.parse(rawFilters ?? {});
        const tenantId = parseTenant(rawTenantId);
    ...
    async inativarDoador(rawId, userId, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
    ...
    async listTarefasRelacionamento(rawDoadorId, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
    ...
    async saveTarefaRelacionamento(rawDoadorId, rawInput, userId, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
        const doadorId = parseId(rawDoadorId);
        await this.repository.buscarDoadorPorIdOuFalhar(doadorId, tenantId);
        ...
        await this.repository.registrarLog("doador", doadorId, "TAREFA_RELACIONAMENTO_CRIADA", `Tarefa de relacionamento criada: ${payload.titulo}`, { doadorId: rawDoadorId, tarefaId: String(row.id), titulo: payload.titulo }, parseUserId(userId), tenantId);
        return { tarefa: mapCaptacaoTarefaRelacionamento(row) };
    }
    async concluirTarefaRelacionamento(rawId, userId, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
        const row = await this.repository.concluirTarefaRelacionamento(parseId(rawId), parseUserId(userId), tenantId);
        await this.repository.registrarLog("doador", BigInt(String(row.doador_id)), "TAREFA_RELACIONAMENTO_CONCLUIDA", `Tarefa de relacionamento concluída: ${String(row.titulo ?? "")}`, { tarefaId: rawId, doadorId: String(row.doador_id ?? "") }, parseUserId(userId), tenantId);
        return { tarefa: mapCaptacaoTarefaRelacionamento(row) };
    }
    async listCampanhas(rawFilters, rawTenantId) {
        const filters = captacaoListFiltersSchema.parse(rawFilters ?? {});
        const tenantId = parseTenant(rawTenantId);
    ...
    async getCampanha(rawId, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
    ...
    async saveCampanha(rawInput, userId, rawId, rawTenantId) {
    ...
    async alterarStatusCampanha(rawId, status, userId, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
    ...
    async listDoacoes(rawFilters, rawTenantId) {
        const filters = captacaoListFiltersSchema.parse(rawFilters ?? {});
        const tenantId = parseTenant(rawTenantId);
    ...
    async getDoacao(rawId, rawTenantId) {
    ...
    async saveDoacao(rawInput, userId, rawId, rawTenantId) {
        ...
        const recorrencia = payload.recorrencia
            ? await this.repository.salvarRecorrencia(currentId ? BigInt(String(payload.recorrenciaId ?? 0)) || null : null, randomUUID(), payload.doadorId ? parseId(payload.doadorId) : null, payload.campanhaId ? parseId(payload.campanhaId) : null, payload.recorrencia, undefined, parseUserId(userId), tenantId)
            : null;
    ...
    async gerarCobranca(rawId, userId, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
    ...
    async confirmarDoacao(rawId, userId, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
        const row = await this.repository.buscarDoacaoPorIdOuFalhar(parseId(rawId), tenantId);
        const doacao = mapCaptacaoDoacao(row);
    ...
    async cancelarDoacao(rawId, userId, observacao, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
        const row = await this.repository.buscarDoacaoPorIdOuFalhar(parseId(rawId), tenantId);
        const doacao = mapCaptacaoDoacao(row);
    ...
    async estornarDoacao(rawId, userId, observacao, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
        const row = await this.repository.buscarDoacaoPorIdOuFalhar(parseId(rawId), tenantId);
        const doacao = mapCaptacaoDoacao(row);
    ...
    async gerarBufferComprovante(doacao, mensagemAgradecimento, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
    ...
    async emitirComprovante(rawDoacaoId, userId, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
    ...
    async listComprovantes(rawFilters, rawTenantId) {
        const filters = captacaoListFiltersSchema.parse(rawFilters ?? {});
        const tenantId = parseTenant(rawTenantId);
    ...
    async reenviarComprovante(rawDoacaoId, userId, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
    ...
    async getConfiguracoes(rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
        return { configuracoes: mapCaptacaoConfiguracoes(await this.repository.obterConfiguracoes(tenantId)) };
    }
    async saveConfiguracoes(rawInput, userId, rawTenantId) {
        const payload = captacaoConfiguracoesSchema.parse(rawInput ?? {});
        const tenantId = parseTenant(rawTenantId);
        const row = await this.repository.salvarConfiguracoes(payload, parseUserId(userId), tenantId);
        await this.repository.registrarLog("configuracoes", null, "EDICAO", "Configurações de pagamento atualizadas.", payload, parseUserId(userId), tenantId);
        return { configuracoes: mapCaptacaoConfiguracoes(row) };
    }
    async listLogs(rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
        const rows = await this.repository.listarLogs(200, tenantId);
        return { logs: rows.map(mapCaptacaoLog) };
    }
    async exportarRelatorio(rawFilters, formato, rawTenantId) {
        const tenantId = parseTenant(rawTenantId);
