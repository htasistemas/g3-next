...
export class BackupArquivosService {
    ...
    diretorioBackups = resolve(process.cwd(), env.APP_STORAGE_ROOT, "backups", "arquivos");
    async criarPacoteLocal() {
        ...
        const arquivoCompactado = resolve(this.diretorioBackups, `g3n_arquivos-${timestamp}-${randomUUID().slice(0, 8)}.tar.gz`);
        ...
        for (const caminho of caminhos) {
            try {
                ...
                throw new AppError(error instanceof Error
                    ...
                    : `Falha ao preparar backup do arquivo ${caminho}.`, 500);
            }
        }
