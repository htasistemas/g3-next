...
const estruturaPromise = new Map();
...
function valorSql(campo, valor) {
    if (valor === null)
        return Prisma.sql `NULL`;
    if (valor === undefined)
        return Prisma.sql `NULL`;
    if (camposJsonb.has(campo))
        return Prisma.sql `${JSON.stringify(valor)}::jsonb`;
    ...
    if (camposData.has(campo))
        return Prisma.sql `${valor}::date`;
    if (camposHorario.has(campo))
        return Prisma.sql `${valor}::time`;
    if (campo === "emitido_em")
        return Prisma.sql `${valor}::timestamp`;
...
export class EducacionalRepository {
    ...
    tabela(recurso) { return tabelaPorRecurso[recurso]; }
    async resumo(filtros, tenantId) {
        await this.garantirEstrutura();
        const condicoes = [Prisma.sql `m.tenant_id::text = ${tenantId}`, Prisma.sql `m.situacao = 'ATIVA'`];
        if (filtros.ano_letivo_id)
            condicoes.push(Prisma.sql `m.ano_letivo_id = ${filtros.ano_letivo_id}`);
        if (filtros.unidade_id)
            condicoes.push(Prisma.sql `m.unidade_id = ${filtros.unidade_id}`);
        if (filtros.etapa_id)
            condicoes.push(Prisma.sql `m.etapa_id = ${filtros.etapa_id}`);
        if (filtros.turma_id)
            condicoes.push(Prisma.sql `m.turma_id = ${filtros.turma_id}`);
        if (filtros.turno)
            condicoes.push(Prisma.sql `COALESCE(t.turno, '') = ${filtros.turno}`);
        ...
        if (filtros.ano_letivo_id || filtros.unidade_id || filtros.etapa_id || filtros.turma_id || filtros.turno) {
            ...
            if (filtros.ano_letivo_id)
                alunoMatriculaCondicoes.push(Prisma.sql `mx.ano_letivo_id = ${filtros.ano_letivo_id}`);
            if (filtros.unidade_id)
                alunoMatriculaCondicoes.push(Prisma.sql `mx.unidade_id = ${filtros.unidade_id}`);
            if (filtros.etapa_id)
                alunoMatriculaCondicoes.push(Prisma.sql `mx.etapa_id = ${filtros.etapa_id}`);
            if (filtros.turma_id)
                alunoMatriculaCondicoes.push(Prisma.sql `mx.turma_id = ${filtros.turma_id}`);
            if (filtros.turno)
                alunoMatriculaCondicoes.push(Prisma.sql `tx.turno = ${filtros.turno}`);
    ...
    `);
        const turmasSemProfessor = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        const turmasProximasCapacidade = await prisma.$queryRaw(Prisma.sql `
      SELECT COUNT(*)::bigint AS total
      FROM (
    ...
    async listarPendencias(tipo, filtros, tenantId) {
        await this.garantirEstrutura();
        const condicoesMatricula = [Prisma.sql `m.tenant_id::text = ${tenantId}`, Prisma.sql `m.situacao = 'ATIVA'`];
        if (filtros.ano_letivo_id)
            condicoesMatricula.push(Prisma.sql `m.ano_letivo_id = ${filtros.ano_letivo_id}`);
        if (filtros.unidade_id)
            condicoesMatricula.push(Prisma.sql `m.unidade_id = ${filtros.unidade_id}`);
        if (filtros.etapa_id)
            condicoesMatricula.push(Prisma.sql `m.etapa_id = ${filtros.etapa_id}`);
        if (filtros.turma_id)
            condicoesMatricula.push(Prisma.sql `m.turma_id = ${filtros.turma_id}`);
        if (filtros.turno)
            condicoesMatricula.push(Prisma.sql `COALESCE(t.turno, '') = ${filtros.turno}`);
    ...
    async listar(recurso, tenantId) {
        await this.garantirEstrutura();
        const tabela = this.tabela(recurso);
        const rows = recurso === "alunos"
            ? await prisma.$queryRaw(Prisma.sql `SELECT a.*, b.nome_completo, b.codigo AS codigo_beneficiario, b.data_nascimento, b.nome_mae FROM educacional_aluno a INNER JOIN cadastro_beneficiario b ON b.id = a.beneficiario_id AND b.tenant_id::text = ${tenantId} WHERE a.tenant_id::text = ${tenantId} ORDER BY a.id DESC LIMIT 500`)
            : recurso === "matriculas"
                ? await prisma.$queryRaw(Prisma.sql `SELECT m.*, a.beneficiario_id, b.nome_completo AS aluno_nome, t.nome AS turma_nome, u.nome_fantasia AS unidade_nome, s.nome AS sala_nome FROM educacional_matricula m INNER JOIN educacional_aluno a ON a.id = m.aluno_id AND a.tenant_id::text = ${tenantId} INNER JOIN cadastro_beneficiario b ON b.id = a.beneficiario_id AND b.tenant_id::text = ${tenantId} LEFT JOIN educacional_turma t ON t.id = m.turma_id AND t.tenant_id::text = ${tenantId} LEFT JOIN unidade_assistencial u ON u.id = m.unidade_id AND u.tenant_id::text = ${tenantId} LEFT JOIN salas_unidade s ON s.id = m.sala_id AND s.unidade_id = m.unidade_id WHERE m.tenant_id::text = ${tenantId} ORDER BY m.id DESC LIMIT 500`)
                : recurso === "profissionais"
                    ? await prisma.$queryRaw(Prisma.sql `SELECT v.*, p.nome_completo AS profissional_nome, p.categoria AS profissional_categoria FROM educacional_profissional_vinculo v INNER JOIN cadastro_profissionais p ON p.id = v.profissional_id AND p.tenant_id::text = ${tenantId} WHERE v.tenant_id::text = ${tenantId} ORDER BY v.id DESC LIMIT 500`)
                    : recurso === "ocorrencias"
                        ? await prisma.$queryRaw(Prisma.sql `SELECT o.*, b.nome_completo AS aluno_nome, b.codigo AS codigo_beneficiario, m.numero_matricula, t.nome AS turma_nome FROM educacional_ocorrencia o INNER JOIN educacional_aluno a ON a.id = o.aluno_id AND a.tenant_id::text = ${tenantId} INNER JOIN cadastro_beneficiario b ON b.id = a.beneficiario_id AND b.tenant_id::text = ${tenantId} LEFT JOIN educacional_matricula m ON m.id = o.matricula_id AND m.tenant_id::text = ${tenantId} LEFT JOIN educacional_turma t ON t.id = m.turma_id AND t.tenant_id::text = ${tenantId} WHERE o.tenant_id::text = ${tenantId} ORDER BY o.data_ocorrencia DESC, o.id DESC LIMIT 500`)
                        : await prisma.$queryRaw(Prisma.sql `SELECT * FROM ${Prisma.raw(tabela)} WHERE tenant_id::text = ${tenantId} ORDER BY id DESC LIMIT 500`);
        return rows.map((row) => this.serializar(row));
    }
    async proximoNumeroMatricula(tenantId) {
        await this.garantirEstrutura();
    ...
    async buscarBeneficiarios(termo, tenantId) {
        await this.garantirEstrutura();
    ...
    async buscarAlunos(termo, tenantId) {
        await this.garantirEstrutura();
        const busca = `%${termo.trim()}%`;
        return prisma.$queryRaw(Prisma.sql `
    ...
    async listarUnidadesEnsino(tenantId) {
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        ...
        for (const row of rows) {
            ...
            if (!unidades.has(unidadeId))
                unidades.set(unidadeId, { id: unidadeId, nome: String(row.unidade_nome ?? ""), salas: [] });
    ...
    async listarAlunosAgrupados(filtros, tenantId) {
        await this.garantirEstrutura();
        const condicoes = [
            Prisma.sql `a.tenant_id::text = ${tenantId}`,
            Prisma.sql `b.tenant_id::text = ${tenantId}`
        ];
        if (filtros.aluno_ativo !== undefined)
            condicoes.push(Prisma.sql `a.status = ${filtros.aluno_ativo ? "ATIVO" : "INATIVO"}`);
        if (filtros.instituicao_id || filtros.unidade_id)
            condicoes.push(Prisma.sql `m.unidade_id = ${filtros.instituicao_id ?? filtros.unidade_id}`);
        if (filtros.ano_letivo_id)
            condicoes.push(Prisma.sql `m.ano_letivo_id = ${filtros.ano_letivo_id}`);
        if (filtros.sala_id)
            condicoes.push(Prisma.sql `m.sala_id = ${filtros.sala_id}`);
        if (filtros.turma_id)
            condicoes.push(Prisma.sql `m.turma_id = ${filtros.turma_id}`);
        if (filtros.etapa_id)
            condicoes.push(Prisma.sql `m.etapa_id = ${filtros.etapa_id}`);
        if (filtros.serie_id)
            condicoes.push(Prisma.sql `m.serie_id = ${filtros.serie_id}`);
        if (filtros.turno)
            condicoes.push(Prisma.sql `COALESCE(m.turno, t.turno, '') = ${filtros.turno}`);
        if (filtros.situacao)
            condicoes.push(Prisma.sql `COALESCE(m.situacao, 'PENDENTE') = ${filtros.situacao}`);
        if (filtros.sem_sala)
            condicoes.push(Prisma.sql `m.sala_id IS NULL`);
        if (filtros.sem_instituicao)
            condicoes.push(Prisma.sql `m.unidade_id IS NULL`);
        if (filtros.matricula_pendente)
            condicoes.push(Prisma.sql `COALESCE(m.situacao, 'PENDENTE') = 'PENDENTE'`);
        if (filtros.busca) {
            ...
            if (digitosBusca) {
                ...
                condicoesBusca.push(Prisma.sql `regexp_replace(COALESCE(cpf_doc.numero_documento, ''), '[^0-9]', '', 'g') LIKE ${buscaNumerica}`, Prisma.sql `EXISTS (SELECT 1 FROM contato_beneficiario cb WHERE cb.beneficiario_id = b.id AND regexp_replace(COALESCE(cb.telefone_principal, ''), '[^0-9]', '', 'g') LIKE ${buscaNumerica})`);
            }
    ...
    `;
      ...
      al.id AS ano_letivo_id, al.ano AS ano_letivo, e.nome AS etapa_nome, se.nome AS serie_nome ${from}`;
        ...
        const rows = await prisma.$queryRaw(Prisma.sql `${base} ORDER BY COALESCE(u.nome_fantasia, 'Sem instituição'), COALESCE(s.nome, 'Sem sala'), b.nome_completo LIMIT ${filtros.limite} OFFSET ${offset}`);
        const totalRows = await prisma.$queryRaw(Prisma.sql `SELECT COUNT(*)::bigint AS total, COUNT(DISTINCT unidade_id)::bigint AS instituicoes, COUNT(DISTINCT sala_id)::bigint AS salas, COUNT(*) FILTER (WHERE aluno_status = 'ATIVO')::bigint AS alunos_ativos, COUNT(*) FILTER (WHERE sala_id IS NULL)::bigint AS alunos_sem_sala, COUNT(*) FILTER (WHERE unidade_id IS NULL)::bigint AS alunos_sem_instituicao FROM (${base}) agrupados`);
        const rowsSerializadas = rows.map((row) => this.serializar(row)).filter((row) => !!row);
        ...
        for (const row of rowsSerializadas) {
            ...
            if (!grupos.has(instituicaoId))
                grupos.set(instituicaoId, { instituicao: { id: row.unidade_id, nome: row.unidade_nome ?? "Sem instituição", cnpj: row.unidade_cnpj, alunos_ativos: 0, alunos_inativos: 0, salas: 0 }, salas: [] });
            const grupo = grupos.get(instituicaoId);
            let sala = grupo.salas.find((item) => String(item.id) === salaId);
            if (!sala) {
                sala = { id: row.sala_id, nome: row.sala_nome ?? "Sem sala", turma_nome: row.turma_nome, etapa_nome: row.etapa_nome, serie_nome: row.serie_nome, turno: row.matricula_turno ?? row.turma_turno, capacidade: row.sala_id ? Number(row.capacidade_maxima ?? row.turma_capacidade ?? 0) : 0, alunos: [], vagas_disponiveis: null };
                grupo.salas.push(sala);
                grupo.instituicao.salas = Number(grupo.instituicao.salas ?? 0) + 1;
            }
            sala.alunos.push(row);
            if (row.aluno_status === "ATIVO")
                grupo.instituicao.alunos_ativos = Number(grupo.instituicao.alunos_ativos ?? 0) + 1;
            else
                grupo.instituicao.alunos_inativos = Number(grupo.instituicao.alunos_inativos ?? 0) + 1;
        }
        for (const grupo of grupos.values())
            for (const sala of grupo.salas)
                sala.vagas_disponiveis = Number(sala.capacidade) > 0 ? Math.max(Number(sala.capacidade) - sala.alunos.filter((item) => ["ATIVA", "PENDENTE"].includes(String(item.situacao))).length, 0) : null;
    ...
    async vidaAcademicaAluno(alunoId, tenantId) {
        await this.garantirEstrutura();
        const id = BigInt(alunoId);
        const aluno = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        const endereco = await prisma.$queryRaw(Prisma.sql `
      ...
      LIMIT 1
    `);
        const contatos = await prisma.$queryRaw(Prisma.sql `
      ...
      LIMIT 5
    `);
        const documentosBeneficiario = await prisma.$queryRaw(Prisma.sql `
      ...
      LIMIT 30
    `);
        const responsaveis = await prisma.$queryRaw(Prisma.sql `
      ...
      LIMIT 10
    `);
        const frequencia = await prisma.$queryRaw(Prisma.sql `
      SELECT COUNT(*)::bigint AS total,
    ...
    `);
        const notas = await prisma.$queryRaw(Prisma.sql `
      ...
      LIMIT 50
    `);
        const ocorrencias = await prisma.$queryRaw(Prisma.sql `
      SELECT *
    ...
    `);
        const movimentacoes = await prisma.$queryRaw(Prisma.sql `
      SELECT mv.*
      ...
      LIMIT 30
    `);
        const frequenciaSerializada = this.serializar(frequencia[0] ?? {});
    ...
    async obterChamadaRapida(diarioId, tenantId) {
        await this.garantirEstrutura();
        const id = BigInt(diarioId);
        const diario = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        if (!diario[0])
            throw new AppError("Aula do diário não encontrada nesta instituição.", 404);
        const turmaId = BigInt(String(diario[0].turma_id));
        const alunos = await prisma.$queryRaw(Prisma.sql `
    ...
    async salvarChamadaRapida(diarioId, input, tenantId, actor) {
        await this.garantirEstrutura();
        const id = BigInt(diarioId);
        const diario = await prisma.$queryRaw(Prisma.sql `
      SELECT turma_id
    ...
    `);
        ...
        const permitidas = await prisma.$queryRaw(Prisma.sql `
      SELECT m.id
      FROM educacional_matricula m
      WHERE m.tenant_id::text = ${tenantId}
        AND m.situacao = 'ATIVA'
    ...
    `);
        ...
        if (invalidas.length)
            throw new AppError("A chamada contém aluno que não pertence à turma da aula selecionada.", 400);
    ...
    `);
        if (!matricula[0])
            throw new AppError("Matrícula não encontrada nesta instituição.", 404);
        if (matricula[0].ano_letivo_id !== anoLetivoId)
            throw new AppError("O ano letivo informado não corresponde à matrícula selecionada.", 400);
        const notas = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
      ...
      RETURNING *
    `);
        await this.auditar("educacional_boletim", rows[0]?.id, "GERAR_AUTOMATICO", null, rows[0], tenantId, actor);
        return {
    ...
    async gerarHistoricoAutomatico(input, tenantId, actor) {
        await this.garantirEstrutura();
    ...
    `);
        ...
        if (!matricula)
            throw new AppError("Não há matrícula do aluno para o ano letivo informado.", 404);
        const matriculaId = BigInt(String(matricula.id));
        const resultadoFinal = await prisma.$queryRaw(Prisma.sql `
      ...
      LIMIT 1
    `);
        const boletins = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
      ...
      RETURNING *
    `);
        await this.auditar("educacional_historico_escolar", rows[0]?.id, "GERAR_AUTOMATICO", null, rows[0], tenantId, actor);
        return {
            item: this.serializar(rows[0]),
    ...
    async transferirMatricula(matriculaId, input, tenantId, actor) {
        await this.garantirEstrutura();
        const origem = await prisma.$queryRaw(Prisma.sql `SELECT * FROM educacional_matricula WHERE id = ${BigInt(matriculaId)} AND tenant_id::text = ${tenantId} LIMIT 1`);
        if (!origem[0])
            throw new AppError("Matrícula não encontrada nesta instituição.", 404);
        if (origem[0].situacao !== "ATIVA" || origem[0].ativo === false)
            throw new AppError("Somente matrículas ativas podem ser transferidas.", 409);
    ...
    async rematricular(matriculaId, input, tenantId, actor) {
        await this.garantirEstrutura();
        const id = BigInt(matriculaId);
        const origem = await prisma.$queryRaw(Prisma.sql `
      SELECT *
      FROM educacional_matricula
      WHERE id = ${id} AND tenant_id::text = ${tenantId}
      LIMIT 1
    `);
        if (!origem[0])
            throw new AppError("Matrícula de origem não encontrada nesta instituição.", 404);
    ...
    `);
        if (existente[0])
            throw new AppError("Este aluno já possui matrícula ativa ou pendente para o ano letivo selecionado.", 409);
      ...
      `);
        ...
        await this.auditar("educacional_matricula", nova.id, "REMATRICULAR", origem[0], nova, tenantId, actor);
        return { origem: this.serializar(origem[0]), destino: this.serializar(nova) };
    }
    async rematricularLote(input, tenantId, actor) {
        ...
        for (const matriculaId of matriculasIds) {
            try {
                ...
                criadas.push(resultado.destino);
            }
            catch (erro) {
    ...
    async sugerirRecuperacoes(input, tenantId) {
        await this.garantirEstrutura();
        const anoLetivoId = BigInt(input.ano_letivo_id);
    ...
    async editarVinculoMatricula(matriculaId, input, tenantId, actor) {
        await this.garantirEstrutura();
        const id = BigInt(matriculaId);
        const origem = await prisma.$queryRaw(Prisma.sql `SELECT * FROM educacional_matricula WHERE id = ${id} AND tenant_id::text = ${tenantId} LIMIT 1`);
        if (!origem[0])
            throw new AppError("Matrícula não encontrada nesta instituição.", 404);
        if (origem[0].situacao !== "ATIVA" || origem[0].ativo === false)
            throw new AppError("Somente matrículas ativas podem ter o vínculo alterado.", 409);
        ...
        const atualizado = await prisma.$transaction(async (tx) => {
            const rows = await tx.$queryRaw(Prisma.sql `UPDATE educacional_matricula SET unidade_id = ${BigInt(input.instituicao_id)}, sala_id = ${BigInt(input.sala_id)}, turma_id = ${input.turma_id ?? null}, atualizado_em = NOW() WHERE id = ${id} AND tenant_id::text = ${tenantId} RETURNING *`);
    ...
    async criarVinculoAluno(alunoId, input, tenantId, actor) {
        await this.garantirEstrutura();
        const aluno = await prisma.$queryRaw(Prisma.sql `SELECT * FROM educacional_aluno WHERE id = ${BigInt(alunoId)} AND tenant_id::text = ${tenantId} LIMIT 1`);
        if (!aluno[0])
            throw new AppError("Aluno não encontrado nesta instituição.", 404);
        const ativa = await prisma.$queryRaw(Prisma.sql `SELECT id FROM educacional_matricula WHERE aluno_id = ${BigInt(alunoId)} AND tenant_id::text = ${tenantId} AND situacao IN ('ATIVA', 'PENDENTE') AND COALESCE(ativo, TRUE) = TRUE LIMIT 1`);
        if (ativa[0])
            throw new AppError("Este aluno já possui matrícula ativa.", 409);
        ...
        const referencias = await prisma.$queryRaw(Prisma.sql `SELECT id FROM educacional_ano_letivo WHERE id = ${BigInt(input.ano_letivo_id)} AND tenant_id::text = ${tenantId} LIMIT 1`);
        const etapa = await prisma.$queryRaw(Prisma.sql `SELECT id FROM educacional_etapa WHERE id = ${BigInt(input.etapa_id)} AND tenant_id::text = ${tenantId} LIMIT 1`);
        const serie = await prisma.$queryRaw(Prisma.sql `SELECT id FROM educacional_serie WHERE id = ${BigInt(input.serie_id)} AND tenant_id::text = ${tenantId} LIMIT 1`);
        if (!referencias[0] || !etapa[0] || !serie[0])
            throw new AppError("Ano letivo, etapa ou série não pertence à instituição atual.", 400);
        ...
        await this.auditar("educacional_matricula", nova[0]?.id, "CRIAR_VINCULO", null, nova[0], tenantId, actor);
        return this.serializar(nova[0]);
    }
    async listarHistoricoMatricula(matriculaId, tenantId) {
        await this.garantirEstrutura();
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async validarDestinoTransferencia(input, tenantId, matriculaId) {
        const sala = await prisma.$queryRaw(Prisma.sql `SELECT s.capacidade_maxima FROM salas_unidade s INNER JOIN unidade_assistencial u ON u.id = s.unidade_id WHERE s.id = ${BigInt(input.sala_destino_id)} AND s.unidade_id = ${BigInt(input.instituicao_destino_id)} AND u.tenant_id::text = ${tenantId} AND u.tipo_unidade = 'ENSINO' AND COALESCE(s.ativo, TRUE) = TRUE LIMIT 1`);
        if (!sala[0])
            throw new AppError("A sala de destino não pertence a uma unidade escolar desta instituição.", 400);
        if (input.turma_destino_id) {
            const turma = await prisma.$queryRaw(Prisma.sql `SELECT id FROM educacional_turma WHERE id = ${BigInt(input.turma_destino_id)} AND tenant_id::text = ${tenantId} AND unidade_id = ${BigInt(input.instituicao_destino_id)} AND (sala_id IS NULL OR sala_id = ${BigInt(input.sala_destino_id)}) AND status = 'ATIVA' LIMIT 1`);
            if (!turma[0])
                throw new AppError("A turma de destino não pertence à instituição e à sala selecionadas.", 400);
        }
        const ocupacao = await prisma.$queryRaw(Prisma.sql `SELECT COUNT(*)::bigint AS total FROM educacional_matricula WHERE tenant_id::text = ${tenantId} AND unidade_id = ${BigInt(input.instituicao_destino_id)} AND sala_id = ${BigInt(input.sala_destino_id)} AND situacao IN ('ATIVA', 'PENDENTE') AND id <> ${matriculaId}`);
        if (Number(sala[0].capacidade_maxima ?? 0) > 0 && Number(ocupacao[0]?.total ?? 0) >= Number(sala[0].capacidade_maxima))
            throw new AppError("A sala de destino atingiu sua capacidade máxima.", 409);
    }
    async criarAluno(input, tenantId, actor) {
        const beneficiario = await prisma.$queryRaw(Prisma.sql `SELECT id FROM cadastro_beneficiario WHERE id = ${BigInt(input.beneficiario_id)} AND tenant_id::text = ${tenantId} LIMIT 1`);
        if (!beneficiario[0])
            throw new AppError("Beneficiário não encontrado nesta instituição.", 404);
        const rows = await prisma.$queryRaw(Prisma.sql `INSERT INTO educacional_aluno (tenant_id, beneficiario_id, numero_aluno, observacoes) VALUES (${tenantId}::uuid, ${BigInt(input.beneficiario_id)}, ${input.numero_aluno ?? null}, ${input.observacoes ?? null}) ON CONFLICT (tenant_id, beneficiario_id) DO UPDATE SET observacoes = EXCLUDED.observacoes, atualizado_em = NOW() RETURNING *`);
        await this.auditar("educacional_aluno", rows[0]?.id, "CRIAR", null, rows[0], tenantId, actor);
        return this.serializar(rows[0]);
    }
    async salvar(recurso, rawId, input, tenantId, actor) {
        const tabela = this.tabela(recurso);
        await this.validarReferencias(recurso, input, tenantId, rawId);
        if (recurso === "lista-espera" && input.beneficiario_id)
            await this.validarBeneficiario(String(input.beneficiario_id), tenantId);
        if (recurso === "recuperacoes" && input.valor !== null && input.valor !== undefined && Number(input.valor) > Number(input.valor_maximo))
            throw new AppError("A nota da recuperação não pode ser maior que o valor máximo.", 400);
        if (recurso === "horarios")
            await this.validarConflitoHorario(input, rawId, tenantId);
        if (recurso === "diarios" || recurso === "frequencias" || recurso === "notas")
            await this.validarVinculosAcademicos(recurso, input, tenantId);
        if (recurso === "enturmacoes")
            await this.validarEnturmacao(input, rawId, tenantId);
        if (recurso === "notas")
            await this.validarNota(input, tenantId);
        if (recurso === "matriculas")
            await this.validarNumeroMatricula(input, rawId, tenantId);
        const id = rawId ? BigInt(rawId) : undefined;
        const permitido = Object.entries(input).filter(([key, value]) => key !== "id" && value !== undefined).map(([key, value]) => [key, value]);
        if (!permitido.length)
            throw new AppError("Informe dados para salvar.", 400);
        if (id) {
            const anterior = await prisma.$queryRaw(Prisma.sql `SELECT * FROM ${Prisma.raw(tabela)} WHERE id = ${id} AND tenant_id::text = ${tenantId} LIMIT 1`);
            if (!anterior[0])
                throw new AppError("Registro educacional não encontrado.", 404);
            if (recurso === "anos-letivos" && anterior[0].status === "ENCERRADO")
                throw new AppError("Ano letivo encerrado não pode ser alterado sem reabertura autorizada.", 409);
            ...
            const rows = await prisma.$queryRaw(Prisma.sql `UPDATE ${Prisma.raw(tabela)} SET ${query}, atualizado_em = NOW() WHERE id = ${id} AND tenant_id::text = ${tenantId} RETURNING *`);
        ...
        const rows = await prisma.$queryRaw(Prisma.sql `INSERT INTO ${Prisma.raw(tabela)} (tenant_id, ${colunasSql}) VALUES (${tenantId}::uuid, ${valoresSql}) RETURNING *`);
        await this.auditar(tabela, rows[0]?.id, "CRIAR", null, rows[0], tenantId, actor);
        return this.serializar(rows[0]);
    }
    async validarReferencias(recurso, input, tenantId, rawId) {
        const referencias = [];
        if (recurso === "profissionais") {
            const profissional = await prisma.$queryRaw(Prisma.sql `SELECT id FROM cadastro_profissionais WHERE id = ${BigInt(String(input.profissional_id))} AND tenant_id::text = ${tenantId} LIMIT 1`);
            if (!profissional[0])
                throw new AppError("Profissional não encontrado nesta instituição.", 404);
            if (input.unidade_id)
                referencias.push(["unidade_assistencial", "unidade_id"]);
            if (input.disciplina_id)
                referencias.push(["educacional_disciplina", "disciplina_id"]);
            if (input.turma_id)
                referencias.push(["educacional_turma", "turma_id"]);
        }
        if (recurso === "series" && input.etapa_id)
            referencias.push(["educacional_etapa", "etapa_id"]);
        if (recurso === "turmas") {
            if (input.ano_letivo_id)
                referencias.push(["educacional_ano_letivo", "ano_letivo_id"]);
            if (input.etapa_id)
                referencias.push(["educacional_etapa", "etapa_id"]);
            if (input.serie_id)
                referencias.push(["educacional_serie", "serie_id"]);
            if (input.unidade_id)
                referencias.push(["unidade_assistencial", "unidade_id"]);
            if (input.professor_responsavel_id)
                referencias.push(["cadastro_profissionais", "professor_responsavel_id"]);
        }
        if (recurso === "matriculas") {
            if (input.aluno_id)
            ...
            if (input.responsavel_id) {
                const responsavel = await prisma.$queryRaw(Prisma.sql `
          SELECT id
        ...
        `);
                if (!responsavel[0])
                    throw new AppError("Responsável da matrícula não encontrado nesta instituição.", 404);
            }
        }
        if (recurso === "enturmacoes") {
            if (input.matricula_id)
                referencias.push(["educacional_matricula", "matricula_id"]);
            if (input.turma_id)
                referencias.push(["educacional_turma", "turma_id"]);
        }
        ...
        if (recurso === "horarios") {
            ...
            if (input.professor_id)
                referencias.push(["cadastro_profissionais", "professor_id"]);
        }
        if (recurso === "diarios") {
            if (input.turma_id)
                referencias.push(["educacional_turma", "turma_id"]);
        ...
        if (recurso === "frequencias") {
            if (input.diario_aula_id)
                referencias.push(["educacional_diario_aula", "diario_aula_id"]);
            if (input.matricula_id)
                referencias.push(["educacional_matricula", "matricula_id"]);
        }
        ...
        if (recurso === "notas") {
            if (input.avaliacao_id)
                referencias.push(["educacional_avaliacao", "avaliacao_id"]);
            if (input.matricula_id)
                referencias.push(["educacional_matricula", "matricula_id"]);
        }
        if (recurso === "boletins") {
        ...
        if (recurso === "calendario" && input.ano_letivo_id)
            referencias.push(["educacional_ano_letivo", "ano_letivo_id"]);
        if ((recurso === "turmas" || recurso === "matriculas" || recurso === "lista-espera" || recurso === "calendario") && input.unidade_id) {
            const unidade = await prisma.$queryRaw(Prisma.sql `SELECT tipo_unidade FROM unidade_assistencial WHERE id = ${BigInt(String(input.unidade_id))} AND tenant_id::text = ${tenantId} LIMIT 1`);
            if (!unidade[0])
                throw new AppError("Unidade de ensino não encontrada nesta instituição.", 400);
            if (unidade[0].tipo_unidade !== "ENSINO")
                throw new AppError("Selecione uma unidade classificada como unidade de ensino.", 400);
        }
        if (recurso === "turmas" && input.sala_id) {
            if (!input.unidade_id)
                throw new AppError("Informe a unidade de ensino antes de selecionar a sala da turma.", 400);
            const sala = await prisma.$queryRaw(Prisma.sql `
        SELECT s.id
      ...
      `);
            if (!sala[0])
                throw new AppError("A sala selecionada não pertence à unidade de ensino atual.", 400);
        }
        if (recurso === "turmas" || recurso === "grade-curricular") {
            ...
            if (etapaId && serieId) {
                const serie = await prisma.$queryRaw(Prisma.sql `
          SELECT etapa_id
        ...
        if (recurso === "horarios" && input.sala_id && input.turma_id) {
            const compatibilidade = await prisma.$queryRaw(Prisma.sql `
        SELECT t.unidade_id
        FROM educacional_turma t
      ...
      `);
            if (!sala[0])
                throw new AppError("A sala do horário não é compatível com a unidade da turma.", 400);
        }
        if (recurso === "matriculas")
            await this.validarVagaSala(input, tenantId, rawId);
        for (const [tabelaReferencia, campo] of referencias) {
            const valor = BigInt(String(input[campo]));
            const encontrado = await prisma.$queryRaw(Prisma.sql `SELECT id FROM ${Prisma.raw(tabelaReferencia)} WHERE id = ${valor} AND tenant_id::text = ${tenantId} LIMIT 1`);
            if (!encontrado[0])
                throw new AppError("A referência informada não pertence à instituição atual.", 400);
        }
        if (recurso === "matriculas")
            await this.validarCompatibilidadeMatricula(input, tenantId);
    }
    async validarNumeroMatricula(input, rawId, tenantId) {
    ...
    `);
        if (existente[0])
            throw new AppError("Já existe uma matrícula com este número. Informe outro número para continuar.", 409);
    }
    async validarCompatibilidadeMatricula(input, tenantId) {
        const etapaId = input.etapa_id ? BigInt(String(input.etapa_id)) : null;
        const serieId = input.serie_id ? BigInt(String(input.serie_id)) : null;
        if (etapaId && serieId) {
            const serie = await prisma.$queryRaw(Prisma.sql `
      ...
      `);
        ...
        const turma = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        if (!turma[0])
            throw new AppError("Turma não encontrada nesta instituição.", 400);
        if (turma[0].status !== "ATIVA")
            throw new AppError("A turma selecionada não está ativa para receber matrículas.", 400);
        const incompatibilidades = [];
        if (input.ano_letivo_id && turma[0].ano_letivo_id !== BigInt(String(input.ano_letivo_id)))
            incompatibilidades.push("ano letivo");
        if (input.etapa_id && turma[0].etapa_id !== BigInt(String(input.etapa_id)))
            incompatibilidades.push("etapa");
        if (input.serie_id && turma[0].serie_id !== BigInt(String(input.serie_id)))
            incompatibilidades.push("série");
        if (input.unidade_id && turma[0].unidade_id && turma[0].unidade_id !== BigInt(String(input.unidade_id)))
            incompatibilidades.push("unidade de ensino");
        if (input.sala_id && turma[0].sala_id && turma[0].sala_id !== BigInt(String(input.sala_id)))
            incompatibilidades.push("sala");
    ...
    async validarVagaSala(input, tenantId, rawId) {
    ...
    `);
        if (!sala[0])
            throw new AppError("A sala selecionada não pertence à unidade de ensino ou está inativa.", 400);
        const capacidade = Number(sala[0].capacidade_maxima ?? 0);
        if (capacidade < 1)
            throw new AppError("Configure a capacidade da sala antes de matricular alunos.", 400);
        const matriculaId = rawId ? BigInt(rawId) : null;
        const ocupacao = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        ...
        if (total >= capacidade)
            throw new AppError(`A sala está lotada. Capacidade: ${capacidade}; ocupadas: ${total}.`, 409);
    }
    async validarEnturmacao(input, rawId, tenantId) {
      ...
      LIMIT 1
    `);
        if (!matricula[0] || matricula[0].situacao !== "ATIVA")
            throw new AppError("A matrícula precisa estar ativa para ser alocada em uma turma.", 400);
        const atual = await prisma.$queryRaw(Prisma.sql `SELECT id FROM educacional_enturmacao WHERE tenant_id::text = ${tenantId} AND matricula_id = ${matriculaId} AND data_fim IS NULL AND (${id}::bigint IS NULL OR id <> ${id}) LIMIT 1`);
        if (atual[0])
            throw new AppError("Este aluno já está alocado em uma turma ativa.", 409);
        const turma = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        if (!turma[0])
            throw new AppError("Turma ativa não encontrada nesta instituição.", 404);
        ...
        if (Number(turma[0].capacidade_maxima) > 0) {
            const ocupacao = await prisma.$queryRaw(Prisma.sql `SELECT COUNT(*)::bigint AS total FROM educacional_enturmacao e INNER JOIN educacional_matricula m ON m.id = e.matricula_id WHERE e.tenant_id::text = ${tenantId} AND e.turma_id = ${turmaId} AND e.data_fim IS NULL AND m.situacao = 'ATIVA' AND (${id}::bigint IS NULL OR e.id <> ${id})`);
            if (Number(ocupacao[0]?.total ?? 0) >= Number(turma[0].capacidade_maxima))
                throw new AppError("A capacidade máxima da turma foi atingida.", 409);
        }
    }
    async validarNota(input, tenantId) {
        if (input.valor === null || input.valor === undefined)
            return;
        const avaliacao = await prisma.$queryRaw(Prisma.sql `SELECT valor_maximo FROM educacional_avaliacao WHERE id = ${BigInt(String(input.avaliacao_id))} AND tenant_id::text = ${tenantId} LIMIT 1`);
        if (!avaliacao[0])
            throw new AppError("Avaliação não encontrada nesta instituição.", 404);
        if (Number(input.valor) > Number(avaliacao[0].valor_maximo))
            throw new AppError(`A nota não pode ser maior que ${avaliacao[0].valor_maximo}.`, 400);
    }
    async validarVinculosAcademicos(recurso, input, tenantId) {
        if (recurso === "diarios") {
            const turma = await prisma.$queryRaw(Prisma.sql `
        SELECT status
        FROM educacional_turma
      ...
      `);
            if (!turma[0])
                throw new AppError("A turma do diário não pertence à instituição atual.", 400);
            if (turma[0].status !== "ATIVA")
                throw new AppError("Somente turmas ativas podem receber aulas no diário.", 400);
            return;
        }
        ...
        `))[0]?.turma_id;
        if (!turmaId)
            throw new AppError(recurso === "frequencias" ? "O diário da frequência não foi encontrado nesta instituição." : "A avaliação não foi encontrada nesta instituição.", 404);
        const matriculaId = BigInt(String(input.matricula_id));
        const matricula = await prisma.$queryRaw(Prisma.sql `
      SELECT m.id
      FROM educacional_matricula m
    ...
    `);
        if (!matricula[0])
            throw new AppError("A matrícula selecionada não pertence à turma deste registro.", 400);
    }
    async validarBeneficiario(rawId, tenantId) {
        const encontrado = await prisma.$queryRaw(Prisma.sql `SELECT id FROM cadastro_beneficiario WHERE id = ${BigInt(rawId)} AND tenant_id::text = ${tenantId} LIMIT 1`);
        if (!encontrado[0])
            throw new AppError("Beneficiário não encontrado nesta instituição.", 404);
    }
    async validarConflitoHorario(input, rawId, tenantId) {
        const turmaId = BigInt(String(input.turma_id));
        ...
        const id = rawId ? BigInt(rawId) : null;
        const conflito = await prisma.$queryRaw(Prisma.sql `
      SELECT h.id,
        CASE
    ...
    `);
        if (conflito[0])
            throw new AppError(`Conflito de horário: ${conflito[0].tipo}.`, 409);
    }
    serializar(row) { if (!row)
        return null; return Object.fromEntries(Object.entries(row).map(([key, value]) => [key, typeof value === "bigint" ? value.toString() : value instanceof Date ? value.toISOString().slice(0, 10) : value])); }
    async auditar(entidade, id, acao, anterior, novo, tenantId, actor) { await prisma.$executeRaw(Prisma.sql `INSERT INTO educacional_auditoria (tenant_id, entidade, entidade_id, acao, dados_anteriores, dados_novos, usuario_id, usuario_nome) VALUES (${tenantId}::uuid, ${entidade}, ${id}, ${acao}, ${jsonSeguro(anterior)}::jsonb, ${jsonSeguro(novo)}::jsonb, ${actor.id ? BigInt(actor.id) : null}, ${actor.nome ?? actor.nomeUsuario ?? "Usuário autenticado"})`); }
}
