...
const numeroNaoNegativo = (mensagem) => z.preprocess((value) => {
    if (value === "" || value === null || value === undefined)
        return 0;
    if (typeof value === "string")
        return numeroBrasileiro(value);
    return value;
}, z.number({ invalid_type_error: mensagem }).finite(mensagem).min(0, mensagem));
export function idBigInt(valor, nome = "Identificador") {
    if (!/^\d+$/.test(String(valor ?? "")))
        throw new AppError(`${nome} invalido.`, 422);
    try {
        return BigInt(valor);
    }
    catch {
...
export function validarIntervalo(inicio, fim, nome = "O periodo") {
    if (inicio && fim && fim < inicio)
        throw new AppError(`${nome} final nao pode ser anterior ao inicio.`, 422);
}
...
const entidadesFilhas = ["metas", "rubricas", "receitas", "despesas", "documentos"];
function tenant(actor) {
...
export class TermosParceriaService {
    async dashboard(actor) {
      ...
      `)
        ...
        const financeiro = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        if (!rows[0])
            throw new AppError("Parceria nao encontrada no ambiente atual.", 404);
    ...
    async criar(raw, actor, ip) {
        ...
        if (input.valorGlobal > 0 && fontes > input.valorGlobal)
            throw new AppError("A soma dos recursos nao pode ultrapassar o valor global da parceria.", 422);
        ...
        if (input.planoTrabalhoId)
            await this.validarPlanoTrabalho(input.planoTrabalhoId, tenantId);
        if (input.termoFomentoId)
      ...
      WHERE id = ${BigInt(String(registro.id))} AND tenant_id::text = ${tenantId}
    `);
        await this.timeline(BigInt(String(registro.id)), tenantId, "CRIACAO", "Parceria criada", actor, ip);
        return this.obter(String(registro.id), actor);
    }
    async atualizar(id, raw, actor, ip) {
        const tenantId = tenant(actor);
        const instrumentoId = idBigInt(id, "Termo");
        ...
        if (valorGlobal > 0 && fontes > valorGlobal)
            throw new AppError("A soma dos recursos nao pode ultrapassar o valor global da parceria.", 422);
        if (input.numeroInstrumento)
            await this.validarNumeroUnico(input.numeroInstrumento, tenantId, instrumentoId);
        if (input.projetoId)
            await this.validarProjeto(input.projetoId, tenantId);
        if (input.unidadeId)
            await this.validarUnidade(input.unidadeId, tenantId);
    ...
    async criarFilho(id, entidade, raw, actor, ip) {
        const tenantId = tenant(actor);
        if (!entidadesFilhas.includes(entidade))
            throw new AppError("Entidade operacional nao suportada.", 404);
        const instrumentoId = idBigInt(id, "Termo");
        const instrumento = await this.instrumento(instrumentoId, tenantId);
        const input = { ...raw, instrumentoId: id };
        await this.validarFilho(entidade, input, instrumento, instrumentoId, tenantId);
    ...
    `);
        if (!rows[0])
            throw new AppError("Termo nao encontrado no ambiente atual.", 404);
        await this.timeline(instrumentoId, tenantId, "EXCLUSAO", "Parceria excluida logicamente", actor, ip);
    }
    async excluirFilho(id, entidade, itemId, actor, ip) {
        const tenantId = tenant(actor);
        if (!entidadesFilhas.includes(entidade))
            throw new AppError("Entidade operacional nao suportada.", 404);
        const instrumentoId = idBigInt(id, "Termo");
        ...
        await this.instrumento(instrumentoId, tenantId);
        const tabela = { metas: "prestacao_contas_meta", rubricas: "prestacao_contas_rubrica", receitas: "prestacao_contas_receita", despesas: "prestacao_contas_despesa", documentos: "prestacao_contas_documento" }[entidade];
        if (entidade === "rubricas") {
            const movimento = await prisma.$queryRaw(Prisma.sql `SELECT COUNT(*)::bigint AS total FROM prestacao_contas_despesa WHERE rubrica_id = ${registroId} AND instrumento_id = ${instrumentoId} AND tenant_id::text = ${tenantId} AND excluido_em IS NULL`);
            if (Number(movimento[0]?.total ?? 0) > 0)
                throw new AppError("A rubrica possui despesas vinculadas e nao pode ser excluida.", 409);
        }
        const rows = await prisma.$queryRaw(Prisma.sql `UPDATE ${Prisma.raw(tabela)} SET excluido_em = NOW(), excluido_por = ${actor?.id ?? null}, ativo = FALSE, atualizado_em = NOW(), atualizado_por = ${actor?.id ?? null}, versao = versao + 1 WHERE id = ${registroId} AND instrumento_id = ${instrumentoId} AND tenant_id::text = ${tenantId} AND excluido_em IS NULL RETURNING id`);
        if (!rows[0])
            throw new AppError("Registro nao encontrado no ambiente atual.", 404);
        await this.timeline(instrumentoId, tenantId, "EXCLUSAO", `Registro de ${entidade} excluido logicamente`, actor, ip);
    }
    async atualizarFilho(id, entidade, itemId, raw, actor, ip) {
        const tenantId = tenant(actor);
        if (!entidadesFilhas.includes(entidade))
            throw new AppError("Entidade operacional nao suportada.", 404);
        const instrumentoId = idBigInt(id, "Termo");
        ...
        for (const [entrada, coluna] of Object.entries(configuracao.campos)) {
            ...
            if (["quantidadePrevista", "valorTotal", "valorPrevisto", "valorRecebido", "valorBruto", "valorLiquido"].includes(entrada)) {
                ...
                if (!Number.isFinite(valor) || Number(valor) < 0)
                    throw new AppError("O valor informado deve ser valido e nao negativo.", 422);
            }
            valores.push(valor);
            sets.push(`${coluna} = $${valores.length}`);
        }
        if (!sets.length)
            throw new AppError("Nenhum campo valido foi informado para atualizacao.", 422);
        valores.push(actor?.id ?? null, registroId, instrumentoId, tenantId);
        const rows = await prisma.$queryRawUnsafe(`UPDATE ${configuracao.tabela} SET ${sets.join(", ")}, atualizado_em = NOW(), atualizado_por = $${valores.length - 3}, versao = versao + 1 WHERE id = $${valores.length - 2} AND instrumento_id = $${valores.length - 1} AND tenant_id::text = $${valores.length} AND excluido_em IS NULL RETURNING *`, ...valores);
        if (!rows[0])
            throw new AppError("Registro nao encontrado no ambiente atual.", 404);
        await this.timeline(instrumentoId, tenantId, "ALTERACAO", `Registro de ${entidade} atualizado`, actor, ip);
        return mapRow(rows[0]);
    }
    async instrumento(id, tenantId) {
        const rows = await prisma.$queryRaw(Prisma.sql `SELECT * FROM prestacao_contas_instrumento WHERE id = ${id} AND tenant_id::text = ${tenantId} AND excluido_em IS NULL LIMIT 1`);
        if (!rows[0])
            throw new AppError("Parceria nao encontrada no ambiente atual.", 404);
        return rows[0];
    }
    async validarProjeto(id, tenantId) { const rows = await prisma.$queryRaw(Prisma.sql `SELECT id FROM projetos WHERE id = ${idBigInt(id, "Projeto")} AND tenant_id::text = ${tenantId} AND ativo = TRUE LIMIT 1`); if (!rows[0])
        throw new AppError("O projeto nao pertence ao ambiente atual.", 403); }
    async validarPlanoTrabalho(id, tenantId) { const rows = await prisma.$queryRaw(Prisma.sql `SELECT id FROM plano_trabalho WHERE id = ${idBigInt(id, "Plano de trabalho")} AND tenant_id::text = ${tenantId} AND COALESCE(ativo, TRUE) = TRUE LIMIT 1`); if (!rows[0])
    ...
    async validarNumeroUnico(numero, tenantId, ignorarId) {
        ...
        if (!valor)
            return;
        const rows = await prisma.$queryRaw(Prisma.sql `SELECT id FROM prestacao_contas_instrumento WHERE tenant_id::text = ${tenantId} AND numero_instrumento = ${valor} AND excluido_em IS NULL AND (${ignorarId ?? null}::bigint IS NULL OR id <> ${ignorarId ?? null}::bigint) LIMIT 1`);
        if (rows[0])
            throw new AppError("Ja existe um termo com este numero no ambiente atual.", 409);
    }
    async validarUnidade(id, tenantId) { const rows = await prisma.$queryRaw(Prisma.sql `SELECT id FROM unidade_assistencial WHERE id = ${idBigInt(id, "Unidade executora")} AND tenant_id::text = ${tenantId} LIMIT 1`); if (!rows[0])
        throw new AppError("A unidade nao pertence ao ambiente atual.", 403); }
    async validarFilho(entidade, input, instrumento, instrumentoId, tenantId) {
        if (entidade === "despesas")
            await this.validarDespesa(input, instrumentoId, tenantId);
        if (entidade === "rubricas" && input.metaId)
            await this.validarMeta(String(input.metaId), instrumentoId, tenantId);
        if (entidade === "despesas" && input.metaId)
            await this.validarMeta(String(input.metaId), instrumentoId, tenantId);
        if (entidade === "despesas" && input.projetoId) {
            await this.validarProjeto(String(input.projetoId), tenantId);
            if (instrumento.projeto_id && String(input.projetoId) !== String(instrumento.projeto_id))
                throw new AppError("A despesa deve pertencer ao mesmo projeto do termo.", 422);
        }
        if (entidade === "rubricas") {
            const valor = numeroBrasileiro(input.valorTotal ?? 0);
            if (!Number.isFinite(valor) || valor < 0)
                throw new AppError("O valor da rubrica deve ser valido e nao negativo.", 422);
            const soma = await prisma.$queryRaw(Prisma.sql `SELECT COALESCE(SUM(valor_total), 0)::float8 AS total FROM prestacao_contas_rubrica WHERE instrumento_id = ${instrumentoId} AND tenant_id::text = ${tenantId} AND excluido_em IS NULL`);
            if (Number(soma[0]?.total ?? 0) + valor > Number(instrumento.valor_global ?? 0))
                throw new AppError("A soma das rubricas nao pode ultrapassar o valor global da parceria.", 422);
        }
    }
    async validarMeta(id, instrumentoId, tenantId) { const rows = await prisma.$queryRaw(Prisma.sql `SELECT id FROM prestacao_contas_meta WHERE id = ${idBigInt(id, "Meta")} AND instrumento_id = ${instrumentoId} AND tenant_id::text = ${tenantId} AND excluido_em IS NULL LIMIT 1`); if (!rows[0])
        throw new AppError("A meta nao pertence a esta parceria.", 403); }
    async validarDespesa(input, instrumentoId, tenantId) {
        const data = typeof input.dataEmissao === "string" ? input.dataEmissao : null;
        const rows = await prisma.$queryRaw(Prisma.sql `SELECT inicio_vigencia, termino_vigencia FROM prestacao_contas_instrumento WHERE id = ${instrumentoId} AND tenant_id::text = ${tenantId} LIMIT 1`);
        const vigencia = rows[0];
        if (data && ((vigencia?.inicio_vigencia && data < String(vigencia.inicio_vigencia).slice(0, 10)) || (vigencia?.termino_vigencia && data > String(vigencia.termino_vigencia).slice(0, 10))))
            throw new AppError("A despesa esta fora do periodo de vigencia da parceria.", 422);
        if (input.rubricaId) {
            const rubrica = await prisma.$queryRaw(Prisma.sql `SELECT id, valor_total FROM prestacao_contas_rubrica WHERE id = ${idBigInt(String(input.rubricaId), "Rubrica")} AND instrumento_id = ${instrumentoId} AND tenant_id::text = ${tenantId} AND excluido_em IS NULL LIMIT 1`);
            if (!rubrica[0])
                throw new AppError("A rubrica nao pertence a esta parceria.", 403);
            const valor = numeroBrasileiro(input.valorLiquido ?? input.valorBruto ?? 0);
            const executado = await prisma.$queryRaw(Prisma.sql `SELECT COALESCE(SUM(valor_liquido), 0)::float8 AS total FROM prestacao_contas_despesa WHERE rubrica_id = ${rubrica[0].id} AND instrumento_id = ${instrumentoId} AND tenant_id::text = ${tenantId} AND excluido_em IS NULL`);
            if (Number(executado[0]?.total ?? 0) + valor > Number(rubrica[0].valor_total ?? 0))
                throw new AppError("A despesa ultrapassa o saldo disponível da rubrica.", 422);
        }
    }
    async filhos(tabela, instrumentoId, tenantId) { const permitidas = ["prestacao_contas_meta", "prestacao_contas_rubrica", "prestacao_contas_receita", "prestacao_contas_despesa", "prestacao_contas_instrumento_unidade", "prestacao_contas_aditivo", "prestacao_contas_documento", "prestacao_contas_timeline"]; if (!permitidas.includes(tabela))
        throw new AppError("Tabela nao autorizada.", 500); const filtroExclusao = tabela === "prestacao_contas_timeline" ? "" : " AND excluido_em IS NULL"; const rows = await prisma.$queryRawUnsafe(`SELECT * FROM ${tabela} WHERE instrumento_id = $1 AND tenant_id::text = $2${filtroExclusao} ORDER BY id DESC LIMIT 500`, instrumentoId, tenantId); return rows.map(mapRow); }
    async timeline(instrumentoId, tenantId, tipo, titulo, actor, _ip) { await prisma.$executeRaw(Prisma.sql `INSERT INTO prestacao_contas_timeline (tenant_id, instrumento_id, tipo_evento, titulo, usuario_id, usuario_nome) VALUES (${tenantId}::uuid, ${instrumentoId}, ${tipo}, ${titulo}, ${actor?.id ?? null}, ${actor?.nomeUsuario ?? null})`); }
}
