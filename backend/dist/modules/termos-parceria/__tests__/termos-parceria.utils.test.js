...
test("rejeita periodo invertido e aceita periodo valido", () => {
    ...
    assert.throws(() => validarIntervalo("2026-12-31", "2026-01-01"), (error) => error instanceof AppError && error.statusCode === 422);
});
test("valida identificadores antes de converter para bigint", () => {
    assert.equal(idBigInt("123"), 123n);
    assert.throws(() => idBigInt("abc"), (error) => error instanceof AppError && error.statusCode === 422);
});
