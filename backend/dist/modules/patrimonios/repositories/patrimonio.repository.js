...
export class PatrimonioRepository {
    ...
    async listar(tenantId) {
        ...
        const patrimonios = await prisma.$queryRaw(Prisma.sql `
      SELECT
        p.id,
        p.numero_patrimonio,
        p.nome,
    ...
    `);
        const movimentos = await prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        patrimonio_id,
        tipo,
    ...
    async criarCategoria(input, tenantId) {
        await this.garantirEstrutura();
    ...
    async atualizarCategoria(id, input, tenantId) {
        await this.garantirEstrutura();
    ...
    async removerCategoria(id, tenantId) {
        await this.garantirEstrutura();
        const categoria = await this.buscarCategoriaOuFalhar(id, tenantId);
        const usos = await prisma.$queryRaw(Prisma.sql `
    ...
    async registrarMovimento(id, input, tenantId) {
        await this.garantirEstrutura();
        await this.buscarPorIdOuFalhar(id, tenantId);
        await prisma.$executeRaw(Prisma.sql `
    ...
    async validarNumeroUnico(tx, numeroPatrimonio, unidade, tenantId, idAtual) {
    ...
    async resolverUnidade(tx, input, tenantId) {
    ...
    normalizarSubcategorias(subcategorias) {
        const nomes = new Set();
        const resultado = [];
    ...
    async buscarCategoriaOuFalhar(id, tenantId) {
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        tenant_id::text AS tenant_id,
    ...
    async validarCategoriaUnica(nome, tenantId, idAtual) {
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT id
      FROM patrimonio_categoria
      WHERE tenant_id::text = ${tenantId}
    ...
    async semearCategoriasPadrao(tenantId) {
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT COUNT(*)::bigint AS total
      FROM patrimonio_categoria
      WHERE tenant_id::text = ${tenantId}
