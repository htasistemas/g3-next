...
export class ParametrosSistemaService {
    ...
    async atualizarObrigatoriedadeDocumentosBeneficiario(rawPayload, usuarioAtualizacao) {
        ...
        const salvo = await this.repository.salvarObrigatoriedadeDocumentosBeneficiario(normalizado, usuarioAtualizacao);
        return {
            obrigatoriedade: normalizado,
            atualizado_em: salvo.atualizado_em.toISOString()
        };
    ...
    async atualizarAlertasCentralAtendimentos(rawPayload, usuarioAtualizacao) {
        const payload = atualizarAlertasCentralAtendimentosPayloadSchema.parse(rawPayload);
        const normalizado = alertasCentralAtendimentosSchema.parse({
            ...alertasCentralAtendimentosPadrao,
            ...payload.alertas
        });
        const salvo = await this.repository.salvarAlertasCentralAtendimentos(normalizado, usuarioAtualizacao);
        return {
            alertas: normalizado,
            atualizado_em: salvo.atualizado_em.toISOString()
        };
