import { alertasCentralAtendimentosSchema, atualizarCarenciaDoacaoRealizadaPayloadSchema, atualizarAlertasCentralAtendimentosPayloadSchema, atualizarObrigatoriedadeDocumentosBeneficiarioPayloadSchema, atualizarPersonalizacaoPayloadSchema, carenciaDoacaoRealizadaSchema, obrigatoriedadeDocumentosBeneficiarioSchema, personalizacaoSistemaSchema } from "../parametros-sistema.schema.js";
...
const obrigatoriedadeDocumentosBeneficiarioPadrao = {
    documentos: [
...
const alertasCentralAtendimentosPadrao = {
...
function normalizarTipoIntegracao(tipo) {
    const valor = String(tipo ?? "").trim().toUpperCase();
    if (!valor)
        throw new AppError("Informe o tipo da integracao.", 422);
    return valor;
}
...
export class ParametrosSistemaService {
    ...
    async obterPersonalizacao(tenantId) {
    ...
    async atualizarPersonalizacao(rawPayload, usuarioAtualizacao, tenantId) {
        ...
        const salvo = await this.repository.salvarPersonalizacao(normalizado, usuarioAtualizacao, tenantId);
        return {
            personalizacao: normalizado,
    ...
    async obterCarenciaDoacaoRealizada(tenantId) {
    ...
    async atualizarCarenciaDoacaoRealizada(rawPayload, usuarioAtualizacao, tenantId) {
        const payload = atualizarCarenciaDoacaoRealizadaPayloadSchema.parse(rawPayload);
        const normalizado = carenciaDoacaoRealizadaSchema.parse({
            ...carenciaDoacaoRealizadaPadrao,
            ...payload.carencia
        });
        const salvo = await this.repository.salvarCarenciaDoacaoRealizada(normalizado, usuarioAtualizacao, tenantId);
        return {
            carencia: normalizado,
            atualizado_em: salvo.atualizado_em.toISOString()
        };
    ...
    async obterObrigatoriedadeDocumentosBeneficiario(tenantId) {
        ...
        const documentosSalvos = Array.isArray(registro.valor?.documentos)
            ...
            : [];
        const porId = new Map(documentosSalvos.map((documento) => [String(documento?.id ?? "").trim(), documento]));
    ...
    async atualizarObrigatoriedadeDocumentosBeneficiario(rawPayload, usuarioAtualizacao, tenantId) {
        ...
        const documentosRecebidos = Array.isArray(payload.obrigatoriedade?.documentos)
            ...
            : [];
        const porId = new Map(documentosRecebidos.map((documento) => [documento.id.trim(), documento]));
        const normalizado = obrigatoriedadeDocumentosBeneficiarioSchema.parse({
            documentos: obrigatoriedadeDocumentosBeneficiarioPadrao.documentos.map((documentoPadrao) => {
        ...
        const salvo = await this.repository.salvarObrigatoriedadeDocumentosBeneficiario(normalizado, usuarioAtualizacao, tenantId);
        return {
            obrigatoriedade: normalizado,
            atualizado_em: salvo.atualizado_em.toISOString()
        };
    ...
    async obterAlertasCentralAtendimentos(tenantId) {
    ...
    async atualizarAlertasCentralAtendimentos(rawPayload, usuarioAtualizacao, tenantId) {
        const payload = atualizarAlertasCentralAtendimentosPayloadSchema.parse(rawPayload);
        const normalizado = alertasCentralAtendimentosSchema.parse({
            ...alertasCentralAtendimentosPadrao,
            ...payload.alertas
        });
        const salvo = await this.repository.salvarAlertasCentralAtendimentos(normalizado, usuarioAtualizacao, tenantId);
        return {
            alertas: normalizado,
            atualizado_em: salvo.atualizado_em.toISOString()
        };
    ...
    async obterPreferenciaAgendamentosVisualizacao(usuarioId, tenantId) {
        const chave = this.montarChavePreferenciaUsuario(CHAVE_PREFERENCIA_AGENDAMENTOS_VISUALIZACAO, usuarioId);
        const registro = await this.repository.buscarPorChaveGenerica(chave, tenantId);
        return registro?.valor?.data_visualizacao?.trim() || null;
    }
    async salvarPreferenciaAgendamentosVisualizacao(dataVisualizacao, usuarioId, usuarioAtualizacao, tenantId) {
        const chave = this.montarChavePreferenciaUsuario(CHAVE_PREFERENCIA_AGENDAMENTOS_VISUALIZACAO, usuarioId);
        await this.repository.salvarPorChaveGenerica(chave, { data_visualizacao: dataVisualizacao }, usuarioAtualizacao, tenantId);
        return { data_visualizacao: dataVisualizacao };
    }
    async obterConfiguracaoCadastroBeneficiario(tenantId) {
    ...
    `);
      ...
      `);
            return { configuracao: configuracaoCadastroBeneficiarioPadrao, atualizado_em: null };
        }
        return {
            configuracao: {
    ...
    async atualizarConfiguracaoCadastroBeneficiario(rawPayload, usuarioAtualizacao, tenantId) {
        const tenant = parseTenantId(tenantId);
        const payload = (rawPayload && typeof rawPayload === "object" ? rawPayload : {});
        const configuracao = {
            ...configuracaoCadastroBeneficiarioPadrao,
            ...(payload.configuracao && typeof payload.configuracao === "object" ? payload.configuracao : payload)
        };
    ...
    async listarIntegracoes(tenantId) {
        const tenant = parseTenantId(tenantId);
        const rows = await prisma.$queryRaw(Prisma.sql `
    ...
    async salvarIntegracao(rawPayload, usuarioId, tenantId) {
        const tenant = parseTenantId(tenantId);
        const payload = (rawPayload && typeof rawPayload === "object" ? rawPayload : {});
    ...
    async testarIntegracao(rawPayload, usuarioId, tenantId) {
        const tenant = parseTenantId(tenantId);
        const payload = (rawPayload && typeof rawPayload === "object" ? rawPayload : {});
        const tipo = normalizarTipoIntegracao(payload.tipo);
        const usuario = Number(usuarioId);
    ...
    montarChavePreferenciaUsuario(chaveBase, usuarioId) {
