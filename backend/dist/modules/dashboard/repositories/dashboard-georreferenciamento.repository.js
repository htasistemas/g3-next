...
function serializarAuditoriaRegistro(registro) {
    if (!registro)
        return null;
    return Object.entries(registro).reduce((acc, [key, value]) => {
        if (typeof value === "bigint") {
...
async function registrarAuditoriaLocalizacao(localizacaoId, entidadeTipo, entidadeId, acao, dadosAntes, dadosDepois, usuario) {
...
async function registrarAuditoriaPontoManual(pontoManualId, acao, dadosAntes, dadosDepois, usuario) {
    await prisma.$executeRaw(Prisma.sql `
...
export class DashboardGeorreferenciamentoRepository {
    ...
    async listarOpcoesFiltros() {
        await this.garantirEstrutura();
        const [bairrosRows, regioesRows, sexoRows, vulnerabilidadeRows, unidadeRows, statusRows, diagnostico] = await Promise.all([
    ...
    async buscarPontoManualDetalhe(id) {
        const rows = await prisma.$queryRaw(Prisma.sql `
      SELECT
        id,
        categoria,
    ...
    async listarPendenciasGeocodificacao(limit = 20) {
        await this.garantirEstrutura();
        return prisma.$queryRaw(Prisma.sql `
      SELECT *
      FROM (
    ...
    async registrarLogGeocoding(entidadeTipo, entidadeId, origem, enderecoReferencia, status, mensagem) {
        await this.garantirEstrutura();
        await prisma.$executeRaw(Prisma.sql `
    ...
    async aplicarCoordenadasGeocodificadas(pendencia, latitude, longitude, usuario) {
        await this.garantirEstrutura();
    ...
    `);
      ...
      `);
            await registrarAuditoriaLocalizacao(existente[0].id, pendencia.entidade_tipo, pendencia.entidade_id, "GEOCODIFICACAO_ATUALIZADA", serializarAuditoriaRegistro(existente[0]), {
                origem: "GEOCODIFICADA",
                latitude,
                longitude,
      ...
      RETURNING id
    `);
        await registrarAuditoriaLocalizacao(inserted[0]?.id ?? null, pendencia.entidade_tipo, pendencia.entidade_id, "GEOCODIFICACAO_CRIADA", null, {
            origem: "GEOCODIFICADA",
            latitude,
            longitude,
            logradouro: pendencia.logradouro,
    ...
    async salvarMarcacaoVinculada(input, usuario) {
        await this.garantirEstrutura();
      ...
      LIMIT 1
    `);
        const enderecoReferencia = montarEnderecoResumo(input.logradouro, input.numero, input.bairro, input.cidade, input.uf);
        if (existente[0]) {
            await prisma.$executeRaw(Prisma.sql `
        UPDATE territorial_localizacao
        SET
        ...
        WHERE id = ${existente[0].id}
      `);
            await registrarAuditoriaLocalizacao(existente[0].id, input.entidadeTipo, entidadeId, "ATUALIZACAO_MANUAL", serializarAuditoriaRegistro(existente[0]), {
                origem: "MANUAL",
      ...
      RETURNING id
    `);
        await registrarAuditoriaLocalizacao(inserted[0]?.id ?? null, input.entidadeTipo, entidadeId, "CRIACAO_MANUAL", null, {
            origem: "MANUAL",
            latitude: input.latitude,
            longitude: input.longitude,
            logradouro: input.logradouro,
    ...
    async criarPontoManual(input, usuario) {
        await this.garantirEstrutura();
        const inserted = await prisma.$queryRaw(Prisma.sql `
    ...
    `);
        ...
        await registrarAuditoriaPontoManual(pontoManualId, "CRIACAO", null, {
    ...
    async montarDetalhePontoManual(id) {
        ...
        return {
            ...
            enderecoResumo: montarEnderecoResumo(ponto.logradouro, ponto.bairro, ponto.cidade, ponto.uf) || undefined,
