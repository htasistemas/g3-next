...
export class DashboardPowerBiRepository {
    ...
    async listarOpcoesFiltros() {
        const [unidades, municipios, bairros, programas, situacoesCadastro, generos, responsaveisTecnicos, tiposAtendimento, origensEncaminhamento, statusAcompanhamento] = await Promise.all([
    ...
    async listarOpcoesSituacoesCadastro() {
        return this.listarOpcoesComContagem(`
    ...
    async listarOpcoesResponsaveisTecnicos() {
        return this.listarOpcoesComContagem(`
      SELECT nome, COUNT(*)::bigint AS total
      FROM (
    ...
    async listarOpcoesTiposAtendimento() {
        return this.listarOpcoesComContagem(`
      SELECT nome, COUNT(*)::bigint AS total
      FROM (
    ...
    async listarOpcoesOrigensEncaminhamento() {
        return this.listarOpcoesComContagem(`
      SELECT nome, COUNT(*)::bigint AS total
      FROM (
    ...
    async listarOpcoesStatusAcompanhamento() {
        return this.listarOpcoesComContagem(`
      SELECT nome, COUNT(*)::bigint AS total
      FROM (
    ...
    async contarBeneficiarios(filters) {
    ...
    async contarBeneficiariosPorGrupoStatus(filters) {
        const query = this.montarFiltroCadastros(filters);
      ...
      `, query.params);
        return rows.reduce((acc, row) => {
            if (row.grupo === "inativos")
                acc.inativos = toNumber(row.total);
            if (row.grupo === "ativos")
                acc.ativos = toNumber(row.total);
            return acc;
        }, { ativos: 0, inativos: 0 });
    }
    async contarNovosCadastros(filters) {
        const query = this.montarFiltroCadastros(filters);
    ...
    async contarFamiliasAcompanhamento(filters) {
        const query = this.montarFiltroFamilias(filters);
        query.conditions.push(`(COALESCE(NULLIF(TRIM(vf.tecnico_responsavel), ''), '') <> '' OR COALESCE(NULLIF(TRIM(vf.servicos_acompanhamento), ''), '') <> '')`);
        return this.consultarTotal(`
      SELECT COUNT(DISTINCT vf.id)::bigint AS total
      FROM vinculo_familiar vf
      ${query.joins.join("\n")}
    ...
    async contarAtendimentos(filters) {
    ...
    async contarEncaminhamentos(filters) {
    ...
    async contarBeneficios(filters) {
    ...
    async contarVisitasDomiciliares(filters) {
        return this.contarAtendimentos(filters);
    }
    async contarPendencias(filters) {
        const query = this.criarQuery();
        this.adicionarFiltroLista(query, "LOWER(COALESCE(tp.responsavel, ''))", filters.tecnicoUsuario ? [filters.tecnicoUsuario] : []);
        return this.consultarTotal(`
    ...
    async contarCasosPrioritarios(filters) {
    ...
    async contarDocumentacoesPendentes(filters) {
    ...
    async contarProjetosAtivos(filters) {
        const query = this.montarFiltroProjetos(filters);
        query.conditions.push(`UPPER(COALESCE(ca.status, 'ATIVO')) IN ('ATIVO', 'ABERTO', 'EM_ANDAMENTO', 'EM ANDAMENTO')`);
        return this.consultarTotal(`
    ...
    async contarAcoesColetivas(filters) {
        const query = this.criarQuery();
    ...
    async contarParticipantesOficinas(filters) {
        const query = this.montarFiltroProjetos(filters);
    ...
    async contarInstituicoesParceiras(filters) {
        const query = this.montarFiltroProjetos(filters);
        const rows = await this.consultarRows(`
    ...
    async listarCadastrosPorMes(filters) {
        const query = this.montarFiltroCadastros(filters);
        this.adicionarFiltroPeriodo(query, "cb.criado_em", filters.startDate, filters.endDate);
    ...
    async listarAtendimentosPorMes(filters) {
        const query = this.montarFiltroVisitas(filters);
        this.adicionarFiltroPeriodo(query, "vd.data_visita", filters.startDate, filters.endDate);
        return this.listarSeriesMensais(`
    ...
    async listarBeneficiosPorMes(filters) {
        const query = this.montarFiltroBeneficios(filters);
        this.adicionarFiltroPeriodo(query, "dr.data_doacao", filters.startDate, filters.endDate);
        return this.listarSeriesMensais(`
    ...
    async listarEncaminhamentosPorMes(filters) {
        const query = this.montarFiltroEncaminhamentos(filters);
        this.adicionarFiltroPeriodo(query, "bee.data_encaminhamento", filters.startDate, filters.endDate);
        return this.listarSeriesMensais(`
    ...
    async listarParticipacaoPorMes(filters) {
        const query = this.montarFiltroProjetos(filters);
        query.joins.push("LEFT JOIN cursos_atendimentos_matriculas cam ON cam.curso_id = ca.id");
        this.adicionarFiltroPeriodo(query, "cam.data_matricula", filters.startDate, filters.endDate);
        return this.listarSeriesMensais(`
    ...
    async listarBeneficiariosPorStatus(filters) {
        const query = this.montarFiltroCadastros(filters);
    ...
    async listarBeneficiariosPorGenero(filters) {
        const query = this.montarFiltroCadastros(filters);
        return this.listarValoresNomeados(`
    ...
    async listarBeneficiariosPorFaixaEtaria(filters) {
        const query = this.montarFiltroCadastros(filters);
    ...
    async listarBeneficiariosPorTerritorio(filters) {
        const query = this.montarFiltroCadastros(filters);
        return this.listarValoresNomeados(`
    ...
    async listarFamiliasPorBairro(filters) {
        const query = this.montarFiltroFamilias(filters);
        return this.listarValoresNomeados(`
    ...
    async listarFamiliasPorFaixaRenda(filters) {
        const query = this.montarFiltroFamilias(filters);
        return this.listarValoresNomeados(`
    ...
    async listarAtendimentosPorTecnico(filters) {
        const query = this.montarFiltroVisitas(filters);
        this.adicionarFiltroPeriodo(query, "vd.data_visita", filters.startDate, filters.endDate);
        return this.listarValoresNomeados(`
    ...
    async listarAtendimentosPorUnidade(filters) {
        const query = this.montarFiltroVisitas(filters);
        this.adicionarFiltroPeriodo(query, "vd.data_visita", filters.startDate, filters.endDate);
        return this.listarValoresNomeados(`
    ...
    async listarAtendimentosPorTipo(filters) {
        const query = this.montarFiltroVisitas(filters);
        this.adicionarFiltroPeriodo(query, "vd.data_visita", filters.startDate, filters.endDate);
        return this.listarValoresNomeados(`
    ...
    async listarAtendimentosPresenciaisRemotos(filters) {
        const query = this.montarFiltroVisitas(filters);
        this.adicionarFiltroPeriodo(query, "vd.data_visita", filters.startDate, filters.endDate);
        return this.listarValoresNomeados(`
      SELECT
    ...
    async listarCasosAbertosEncerrados(filters) {
        const query = this.montarFiltroVisitas(filters);
        this.adicionarFiltroPeriodo(query, "vd.data_visita", filters.startDate, filters.endDate);
        return this.listarValoresNomeados(`
      SELECT
    ...
    async listarBeneficiosPorTipo(filters) {
        const query = this.montarFiltroBeneficios(filters);
        this.adicionarFiltroPeriodo(query, "dr.data_doacao", filters.startDate, filters.endDate);
        return this.listarValoresNomeados(`
    ...
    async listarBeneficiosDeferidosIndeferidos(filters) {
        const query = this.montarFiltroBeneficios(filters);
        this.adicionarFiltroPeriodo(query, "dr.data_doacao", filters.startDate, filters.endDate);
        return this.listarValoresNomeados(`
      SELECT
    ...
    async listarEncaminhamentosPorTipo(filters) {
        const query = this.montarFiltroEncaminhamentos(filters);
        this.adicionarFiltroPeriodo(query, "bee.data_encaminhamento", filters.startDate, filters.endDate);
        return this.listarValoresNomeados(`
    ...
    async listarEncaminhamentosPorInstituicao(filters) {
        const query = this.montarFiltroEncaminhamentos(filters);
        this.adicionarFiltroPeriodo(query, "bee.data_encaminhamento", filters.startDate, filters.endDate);
        return this.listarValoresNomeados(`
    ...
    async listarEncaminhamentosPendentesRetorno(filters) {
        const query = this.montarFiltroEncaminhamentos(filters);
        this.adicionarFiltroPeriodo(query, "bee.data_encaminhamento", filters.startDate, filters.endDate);
        return this.listarValoresNomeados(`
      SELECT
    ...
    async listarProjetosPorAdesao(filters) {
        const query = this.montarFiltroProjetos(filters);
        query.joins.push("LEFT JOIN cursos_atendimentos_matriculas cam ON cam.curso_id = ca.id");
        this.adicionarFiltroPeriodo(query, "cam.data_matricula", filters.startDate, filters.endDate);
        return this.listarValoresNomeados(`
    ...
    async listarParticipacaoPorFaixaEtariaProjeto(filters) {
        const query = this.montarFiltroProjetos(filters);
        return this.listarValoresNomeados(`
    ...
    async listarPendenciasCriticas(filters) {
        const [cadastrosIncompletos, documentacoesPendentes, tarefasAtrasadas, encaminhamentosPendentes] = await Promise.all([
    ...
    async calcularIndicadoresResumo(filters) {
        const [composicaoFamiliarMedia, familiasComCriancas, familiasComIdosos, familiasComPcd, familiasMonoparentais, tempoMedioEntreAtendimentosDias, casosSemAtualizacao, tempoMedioConcessaoDias, taxaRetornoRede, taxaPresenca] = await Promise.all([
    ...
    async contarCadastrosIncompletos(filters) {
        const query = this.montarFiltroCadastros(filters);
        query.conditions.push(`UPPER(COALESCE(cb.status, '')) IN ('INCOMPLETO', 'EM_ANALISE')`);
        return this.consultarTotal(`
      SELECT COUNT(DISTINCT cb.id)::bigint AS total
    ...
    async contarTarefasAtrasadas(filters) {
        const query = this.criarQuery();
        this.adicionarFiltroLista(query, "LOWER(COALESCE(tp.responsavel, ''))", filters.tecnicoUsuario ? [filters.tecnicoUsuario] : []);
        query.conditions.push(`tp.prazo IS NOT NULL`);
        query.conditions.push(`CAST(tp.prazo AS date) < CURRENT_DATE`);
        query.conditions.push(`UPPER(COALESCE(tp.status, 'PENDENTE')) NOT IN ('CONCLUIDO', 'CONCLUÍDO', 'FINALIZADO', 'ENCERRADO')`);
        return this.consultarTotal(`
      SELECT COUNT(*)::bigint AS total
      FROM tarefas_pendencias tp
      ${this.whereSql(query.conditions)}
    ...
    async contarEncaminhamentosPendentes(filters) {
        const query = this.montarFiltroEncaminhamentos(filters);
        query.conditions.push(`UPPER(COALESCE(bee.status, 'AGUARDANDO')) NOT IN ('RETORNADO', 'CONCLUIDO', 'CONCLUÍDO', 'CONTRATADO')`);
        return this.consultarTotal(`
      SELECT COUNT(DISTINCT bee.id)::bigint AS total
      FROM banco_empregos_encaminhamentos bee
      ${query.joins.join("\n")}
    ...
    async calcularComposicaoFamiliarMedia(filters) {
        const query = this.montarFiltroFamilias(filters);
        return this.consultarTotal(`
    ...
    async contarFamiliasComColunaMaiorQue(filters, coluna, limite) {
        const query = this.montarFiltroFamilias(filters);
    ...
    async contarFamiliasMonoparentais(filters) {
        const query = this.montarFiltroFamilias(filters);
    ...
    async calcularTempoMedioEntreAtendimentos(filters) {
        const query = this.montarFiltroVisitas(filters);
        this.adicionarFiltroPeriodo(query, "vd.data_visita", filters.startDate, filters.endDate);
        return this.consultarTotal(`
    ...
    async contarCasosSemAtualizacao(filters) {
        const query = this.montarFiltroFamilias(filters);
    ...
    async calcularTempoMedioConcessao(filters) {
        const query = this.montarFiltroBeneficios(filters);
        this.adicionarFiltroPeriodo(query, "dr.data_doacao", filters.startDate, filters.endDate);
        return this.consultarTotal(`
    ...
    async calcularTaxaRetornoRede(filters) {
        const query = this.montarFiltroEncaminhamentos(filters);
        this.adicionarFiltroPeriodo(query, "bee.data_encaminhamento", filters.startDate, filters.endDate);
        return this.consultarTotal(`
      SELECT
    ...
    async calcularTaxaPresenca(filters) {
        const query = this.montarFiltroProjetos(filters);
    ...
    async listarDetalhamentos(filters, mascararIdentificacao) {
        const [beneficiarios, familias, atendimentos, beneficios, encaminhamentos, alertas, convenios] = await Promise.all([
    ...
    async listarTabelaFamilias(filters, mascararIdentificacao) {
        const query = this.montarFiltroFamilias(filters);
        const rows = await this.consultarRows(`
      SELECT
    ...
    async listarTabelaAtendimentos(filters, mascararIdentificacao) {
        const query = this.montarFiltroVisitas(filters);
        this.adicionarFiltroPeriodo(query, "vd.data_visita", filters.startDate, filters.endDate);
        const rows = await this.consultarRows(`
      SELECT
    ...
    async listarTabelaBeneficios(filters, mascararIdentificacao) {
        const query = this.montarFiltroBeneficios(filters);
        this.adicionarFiltroPeriodo(query, "dr.data_doacao", filters.startDate, filters.endDate);
        const rows = await this.consultarRows(`
      SELECT
    ...
    async listarTabelaEncaminhamentos(filters, mascararIdentificacao) {
        const query = this.montarFiltroEncaminhamentos(filters);
        this.adicionarFiltroPeriodo(query, "bee.data_encaminhamento", filters.startDate, filters.endDate);
        const rows = await this.consultarRows(`
      SELECT
    ...
    async listarTabelaAlertas(filters) {
    ...
    async listarAlertasCadastros(filters) {
        const query = this.montarFiltroCadastros(filters);
    ...
    async listarAlertasTarefas(filters) {
        const query = this.criarQuery();
        this.adicionarFiltroLista(query, "LOWER(COALESCE(tp.responsavel, ''))", filters.tecnicoUsuario ? [filters.tecnicoUsuario] : []);
        query.conditions.push(`tp.prazo IS NOT NULL`);
        query.conditions.push(`CAST(tp.prazo AS date) < CURRENT_DATE`);
    ...
    async listarAlertasEncaminhamentos(filters) {
        const query = this.montarFiltroEncaminhamentos(filters);
        query.conditions.push(`UPPER(COALESCE(bee.status, 'AGUARDANDO')) NOT IN ('RETORNADO', 'CONCLUIDO', 'CONCLUÍDO', 'CONTRATADO')`);
        const rows = await this.consultarRows(`
      SELECT
    ...
    montarFiltroCadastros(filters, incluirSituacaoSocial = false) {
        const query = this.criarQuery();
        ...
        this.adicionarFiltroLista(query, "LOWER(COALESCE(en.bairro, vf.bairro, en.zona, vf.zona, ''))", filters.bairros);
        ...
        this.adicionarFiltroLista(query, "LOWER(COALESCE(vf.tecnico_responsavel, ''))", filters.responsaveisTecnicos);
    ...
    montarFiltroFamilias(filters) {
        ...
        this.adicionarFiltroLista(query, "LOWER(COALESCE(vf.bairro, en.bairro, vf.zona, en.zona, ''))", filters.bairros);
        this.adicionarFiltroLista(query, "LOWER(COALESCE(vf.status, ''))", filters.statusAcompanhamento);
        this.adicionarFiltroLista(query, "LOWER(COALESCE(vf.tecnico_responsavel, ''))", filters.responsaveisTecnicos);
        this.adicionarFiltroLista(query, "LOWER(COALESCE(cb.status, ''))", filters.situacoesCadastro);
        this.adicionarFiltroLista(query, "LOWER(COALESCE(cb.sexo_biologico, ''))", filters.generos);
    ...
    montarFiltroBeneficios(filters) {
        const query = this.criarQuery();
    ...
    montarFiltroEncaminhamentos(filters) {
        const query = this.criarQuery();
        ...
        this.adicionarFiltroLista(query, "LOWER(COALESCE(be.area, be.tipo, ''))", [...filters.origensEncaminhamento, ...filters.programas]);
        this.adicionarFiltroLista(query, "LOWER(COALESCE(en.cidade, vf.municipio, be.cidade, ''))", filters.municipios);
        this.adicionarFiltroLista(query, "LOWER(COALESCE(en.bairro, vf.bairro, be.bairro, vf.zona, en.zona, ''))", filters.bairros);
    ...
    montarFiltroProjetos(filters) {
        const query = this.criarQuery();
        this.adicionarFiltroLista(query, "LOWER(COALESCE(ca.instituicao_parceira, ca.nome, ''))", filters.unidades);
    ...
    adicionarFiltroPeriodo(query, coluna, startDate, endDate) {
        if (startDate) {
    ...
    adicionarFiltroLista(query, expressao, valores) {
        if (!valores.length)
            return;
    ...
    async listarOpcoesComContagem(sql, params) {
    ...
    async listarValoresNomeados(sql, params) {
        const rows = await this.consultarRows(sql, params);
        return rows.map((row) => ({
    ...
    async listarSeriesMensais(sql, params) {
        const rows = await this.consultarRows(sql, params);
        return rows.map((row) => {
