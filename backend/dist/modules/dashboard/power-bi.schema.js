...
function parseListaStrings(value) {
...
const listaStringsSchema = z.preprocess((value) => parseListaStrings(value), z.array(z.string().min(1)).default([]));
...
export const dashboardPowerBiFiltrosSchema = z
    ...
    familiaBeneficiario: z
        .preprocess((value) => (typeof value === "string" && value.trim().length ? value.trim() : undefined), z.string().max(200).optional())
        .optional(),
    tecnicoUsuario: z
        .preprocess((value) => (typeof value === "string" && value.trim().length ? value.trim() : undefined), z.string().max(200).optional())
        .optional()
